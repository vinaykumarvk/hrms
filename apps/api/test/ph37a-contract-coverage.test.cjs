...
test("PH-37A ratchet floor: coverage does not regress below the recorded baseline", () => {
  const record = coverage();
  // Ratchet floor recorded in docs/reviews/contract-coverage-20260703.md (PH-52A raised it to 495 / 37.4%).
  assert.ok(record.implementedTotal >= 495, `implemented routes ${record.implementedTotal} < floor 495`);
  assert.ok(record.totalPct >= 37.4, `coverage ${record.totalPct}% < floor 37.4%`);
});
