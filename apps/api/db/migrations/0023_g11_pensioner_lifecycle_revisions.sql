...
CREATE TABLE pen_family_members (
    id                  text PRIMARY KEY DEFAULT gen_random_uuid()::text,    -- family_member_id
...
CREATE TABLE pen_pensioners (
    id                    text PRIMARY KEY DEFAULT gen_random_uuid()::text,  -- pensioner_id
    tenant_id             text NOT NULL REFERENCES tenants(id) ON DELETE RESTRICT,
    entity_id             text REFERENCES entities(id) ON DELETE RESTRICT,
    pensioner_no          varchar(48) NOT NULL,
    case_id               text NOT NULL,                               -- logical ref g11 pension case
    employee_id           text REFERENCES employees(id) ON DELETE RESTRICT,
    ppo_id                text NOT NULL,                               -- authorising PPO (FR-12: never detached)
    ppo_no                varchar(48) NOT NULL,
    ppo_type              text NOT NULL DEFAULT 'SERVICE_PENSION',
    pensioner_type        text NOT NULL DEFAULT 'SELF',
    current_pension_basic bigint NOT NULL DEFAULT 0,
    current_da_relief     bigint NOT NULL DEFAULT 0,
    disbursement_model    text NOT NULL DEFAULT 'M11_COMPUTES_FULL',
    life_cert_valid_until date,                                        -- FR-12 AC1 yearly due date
    suspended_from        date,                                        -- set while SUSPENDED_NO_LC (AC2 arrear base)
    date_of_death         date,
    death_detected_source text,
    source_pensioner_id   text REFERENCES pen_pensioners(id) ON DELETE SET NULL,  -- FAMILY rows: converted-from (AC4)
    family_member_id      text REFERENCES pen_family_members(id) ON DELETE SET NULL,  -- E26 beneficiary (BR3)
    family_pension_ref    text REFERENCES pen_family_pension_records(id) ON DELETE SET NULL,  -- E10 computation
...
CREATE TABLE pen_life_certificates (
    id                  text PRIMARY KEY DEFAULT gen_random_uuid()::text,    -- life_certificate_id
    tenant_id           text NOT NULL REFERENCES tenants(id) ON DELETE RESTRICT,
    entity_id           text REFERENCES entities(id) ON DELETE RESTRICT,
    pensioner_id        text NOT NULL REFERENCES pen_pensioners(id) ON DELETE RESTRICT,
...
CREATE TABLE pen_revisions (
    id                  text PRIMARY KEY DEFAULT gen_random_uuid()::text,    -- revision_id
    tenant_id           text NOT NULL REFERENCES tenants(id) ON DELETE RESTRICT,
    entity_id           text REFERENCES entities(id) ON DELETE RESTRICT,
    ...
    batch_id            text REFERENCES pen_revisions(id) ON DELETE RESTRICT,  -- subset: line -> header
    pensioner_id        text REFERENCES pen_pensioners(id) ON DELETE RESTRICT, -- null on batch header
    job_run_ref         varchar(64),                                   -- JOB-G11-PENSION-RUN run_key
    da_rate_ref         text REFERENCES pen_da_relief_rates(id) ON DELETE RESTRICT,  -- BR1 snapshot (DA)
    fitment_factor      numeric(9,4),                                  -- pay-commission re-fix snapshot
    old_basic           bigint,
    new_basic           bigint,
    arrears_amount      bigint,                                 -- month-wise arrears total (AC2)
    application_order   integer,                                       -- IR18 §16.9 ordering
    calc_trace          jsonb,
    status              text NOT NULL DEFAULT 'DRAFT',  -- APPLIED = immutable (AC4/P05)
    created_at          timestamptz NOT NULL DEFAULT now(),
    updated_at          timestamptz NOT NULL DEFAULT now(),
    created_by          text,
    updated_by          text,
