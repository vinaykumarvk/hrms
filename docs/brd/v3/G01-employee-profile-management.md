# Employee Profile Management — Government HRMS Module BRD (G01, v3.0 · platform-grounded)
...
**Upstream contracts (authoritative, reused — not redefined):** `docs/brd/PLATFORM_FOUNDATION.md` (platform build contract — P01–P06, RBAC v1.7, VAL-*/JOB-*/MSG-*/ERR-*, multi-tenancy, API conventions, error table, NFR baseline) and `docs/brd/MODULE_RECONCILIATION.md` (gov↔PrimeSoft mapping, §C convention overrides, §D net-new entity register). The legacy `docs/brd/SHARED_FOUNDATION.md` is **superseded** by these where they conflict (`PLATFORM_FOUNDATION.md` §1.1).
**Grounded in:** Master BRD v2.1 · Product Vision v2.6 · Platform Specification v1.6 · RBAC Design v1.7 · Foundation FS v1.6.
**Council evidence:** `docs/evaluation/G01-employee-profile-management-council.md` (22 risks; 22 adopted improvements — all folded in; see §1.6)
...
Module 01 — Employee Profile Management (G01-EPM) — is the **canonical employee master** and the
**golden source of truth** for every person-, identity-, job-, and organisational-placement attribute
in the HRMS. All 13 other modules (G02–G14) **read** employee data from G01; none keep a competing
copy. When a payroll run needs a bank account, when the Digital Service Register (G12) needs a service
number, when the disciplinary module (G09) needs a reporting line, or when analytics (G14) needs
headcount by cadre — the authoritative answer comes from G01.

G01 owns the **full lifecycle of the employee profile**: created on hire, maintained through service,
...
> **Scope boundary with G02:** G01 owns the *data and the read/maintain surfaces*. **Edits to
> employee-initiated personal details are governed by Module 02 (G02-EPDM) via maker-checker
> workflow.** G01 *references* that workflow (it exposes the fields, the validation, and the
> commit-on-approval write path); it does **not** duplicate the approval engine. Direct HR/Admin
> corrections that are not employee-initiated follow G01's own authorisation and audit rules described
> here.
>
> **Scope boundary with G10 (Payroll):** G01 **never** blocks payroll. Completeness/data-quality is
> **advisory only**. The single legitimate disbursement precondition — a *verified primary salary
> account* — is owned and enforced by **G10** as a typed `NO_VERIFIED_BANK` precondition with a
...
analytics. G01 eliminates this by establishing **one authoritative, versioned, auditable employee
...
|---|---|---|
| G1 | Single source of truth | 100% of G02–G14 reads resolve via G01 consumption API; **0** competing master copies; identity always resolved through `employee_id_aliases` |
...
- Approval workflow for employee-initiated edits → **G02-EPDM**.
- Leave/attendance balances and applications → **G03/G04**.
- Transfers, relieving, joining mechanics → **G05** (G01 *records* the resulting placement).
- Promotions/seniority computation → **G06** (G01 *records* the resulting designation/position).
- Payroll computation, payslips, **disbursement-readiness gating** → **G10**; pension & **family-pension
  award** → **G11** (G01 *hands off* the heir/nominee linkage).
- Statutory service-event ledger semantics → **G12** (G01 *emits* events to G12).
- Binary document storage/encryption/versioning → **G13** (G01 stores only `document_id` references).
- Cross-module dashboards → **G14**.
...
| 3 | Isolate biometric/facial-photo processing — own purpose/consent/retention; no raw template in G01 (R1, **Critical**) | **FR-EPM-009 rewritten** (biometric split from profile photo, distinct consent + purpose); E11 `employee_photos` gains `processing_purpose`; consent recorded in **E27 `consent_records`**; §4.4 |
...
| 6 | DQ `BLOCKED` → advisory-only; disbursement gating moved to G10 (`NO_VERIFIED_BANK`) with non-electronic fallback (R3, **Critical**) | **FR-EPM-014 rewritten** (advisory only); `DQ_FLAG` enum drops `BLOCKED` semantics for pay (renamed values §5.5); §1.1 boundary; §8.4 integration; error catalog note; G10 owns `NO_VERIFIED_BANK` |
...
| 18 | DECEASED downstream: legal-heir/nominee linkage, family-pensioner hook to G11, data-rights succession (R16, Medium) | New **FR-EPM-024**; `employee_nominees`/`employee_dependents` gain heir linkage; SR event `DEATH`; G11 handoff (§8.4) |
...
G01 is an **EXTEND** of the PrimeSoft `M01` Employee Master (`MODULE_RECONCILIATION.md` §A). It does not re-author platform plumbing: it **consumes the platform engines by id** and **adds** only the public-sector statutory surface. This section is the mandated FR→service map (`PLATFORM_FOUNDATION.md` §9 rule 6).

### A1. Engine-consumption summary

| Platform service (id) | What G01 consumes it for (instead of inventing) | Replaces the invented… |
|---|---|---|
...
| **P06 — Migration Toolkit (ETL+V)** (Extract→Validate→Transform→Load→Verify; **3 mandatory dry runs**; **waves**; `migration_runs`; permanent **source-id** column) | Legacy paper-service-book / register migration (FR-017). The `source_system`/`legacy_id` columns follow the `<gov>_source_id` pattern (`GAP (gov-specific)` source system). | the invented standalone importer's undefined migration governance |
...
| **X.1 — Background Jobs runner** (idempotent per-period key; backoff ×3; `JOB-FAIL`→`MSG-SYS-JOBFAIL`; per-tenant isolation) | Effective-date apply (`JOB-M01-EFFDATE`), completeness recompute (`JOB-M01-COMPLETE`), org-view refresh (`JOB-M01-ORGVIEW`), contract/visa reminders (`JOB-M01-CONTRACT`/`JOB-M01-VISA-REMIND`), plus gov `JOB-G01-*` (certificate expiry, retention sweep, remediation, dedup, break-glass anomaly). | the invented ad-hoc schedulers |
| **X.2 — Notification Infrastructure** (`IN_APP`+`EMAIL` parallel; statutory email **mandatory/non-suppressible**; backoff ×5 + DLQ; templates by `MSG-*` id) | All G01 notices (§11) — re-keyed to `MSG-G01-*`; statutory ones (separation, governed-change, breach, death/handoff) are non-suppressible. | the invented per-module notification dispatch |
| **X.3 — Integration Framework** | Outbound calls to G12-SR / G11-PEN / G13 / portal integrations (retry, circuit-break, idempotency). | the invented direct integration code |
...
| **RBAC v1.7 model** | Role taxonomy, scoping dimensions, PII tiers, capability flags, individual entitlements (§3). Gov actors are **ADDITIONS**, never a parallel scheme. | the invented flat role list |
| **Foundation VAL-* / ERR-* / MSG-* / JOB-* catalogues** | Cited by id; only module-unique `VAL-G01-*`/`JOB-G01-*`/`MSG-G01-*`/`ERR-G01-*` authored and registered. | the invented per-module validators/codes |
| **G12-SR ledger** (net-new gov, on the P05 substrate) | **G01 is the identity/qualification SR writer**; it posts to the canonical write-port `POST /api/v1/sr/ingest` using the G12-published codes (`APPOINTMENT`, `NAME_CHANGE`, `DOB_CHANGE`, `GENDER_CHANGE`, `CATEGORY_CHANGE`, `DECEASED`, `QUALIFICATION_ADDED`, `DEPARTMENTAL_EXAM_PASSED`) and ingest payload contract in **§8.6** (the informal labels HIRE/PLACEMENT/ATTRIBUTE/MERGE/SEPARATION/DEATH/GOVERNED-CHANGE are superseded for the SR ledger). The `outbox_events` transactional backbone (E33) is G01's **emission/relay mechanism** that drains to the write-port (P05 ledger) and to consumers — it is not a second audit log. | the invented `service_register_events`-as-platform-entity; a direct ledger INSERT or `/sr/events` path |

> **Employee-master alignment.** G01's anchor entity `employees` (E1) is the **same physical PrimeSoft `M01` master**, not a fork: it reuses `employee_id`, `user_id` (login 1:1), `official_email` (tenant-unique, immutable), `lifecycle_state`, `date_of_joining`/`group_date_of_joining`, `previous_employee_id` (rehire link), `profile_completeness_pct`. Government-specific columns (`service_no`, `cadre`, `pay_scale_id`, `record_state`, `aadhaar_ref_key`, `category`, posting history) and the governance/compliance tables (E21–E34) are expressed as **EXTENSIONS / satellites** keyed on `employee_id` + `tenant_id`/`entity_id`. `statutory_identities` (PrimeSoft) is the platform home for PAN/PASSPORT/UAN etc.; G01's `employee_identity_documents` (E9) maps to it, and the Aadhaar **number** is isolated to the `aadhaar_vault` (E22) per the Aadhaar Act / UIDAI Data-Vault rule (a gov hardening of the platform's `id_value`-encrypted-at-rest + `masking_rule` pattern).

### A2. FR → platform service map

| FR | Consumes (platform services + VAL/JOB/MSG) | Gov net-new (`GAP`) authored on top |
|---|---|---|
| FR-001 Create on hire | P02 (authz/scope), P05 (DB-trigger audit), X.2 (`MSG-G01-PROFILE-CREATED`), `VAL-DOB/EMAIL/MOBILE/PAN/AADHAAR/EFFECTIVE/CONSENT`, `JOB-M01-EFFDATE` | `service_no` gen (`VAL-G01-SERVICENO`), PROVISIONAL state, Aadhaar vault, alias pre-check |
...
| FR-003 Contacts/addresses | P01 (self-service → G02 sensitive-change flow), P02, `VAL-EMAIL/MOBILE/PINCODE` | unique `official_email` (platform already enforces tenant-unique) |
| FR-004 Dependents/nominees/heirs | **P01 4-eyes** for PENSION/GRATUITY, X.2, `VAL-NOMINEE/DEPENDENT`, G13 doc refs | heir/succession-rank (feeds FR-024), SR event to G12 |
| FR-005 Emergency contacts | P02, P05 | — (mandatory-minimum gate already a PrimeSoft rule) |
| FR-006 Education/experience | P02, G13 doc refs | gov-service flag → G11 pensionable read |
| FR-007 Identity + Aadhaar vault | **P02 (reveal authz + PII ceiling)**, **P01 4-eyes**, P05, `VAL-PAN/AADHAAR` | Aadhaar Reference-Key vault, lawful-basis, `VAL-G01-AADHAAR-VAULT` |
| FR-008 Bank | **P01 4-eyes flow**, P02, X.2, `VAL-IFSC` | (G10 owns `NO_VERIFIED_BANK` gate) |
...
| FR-013 Field access + break-glass | **P02 (RBAC §7 `V/M/H/E/AR` tiers, PII Protection Ceiling)**, P05 | hardened break-glass caps/anomaly (`VAL-G01-BREAKGLASS`), `ERR-G01-CAP` |
| FR-014 Completeness/DQ (advisory) | `JOB-M01-COMPLETE`, P02 | advisory-only rule; `JOB-G01-CERT-EXPIRY` feed |
| FR-015 Alias dedup | **P01 4-eyes merge**, P05, outbox→G12 | `employee_id_aliases`, no cross-module re-point |
...
| FR-019 Consumption API + feed | P02 (masked projections), outbox→G12 + consumers | alias-transparent resolution; cursor feed |
| FR-020 DPDP rights/consent/breach | **`consent_records` (platform DPDPA), P05**, X.2, `VAL-CONSENT` | DPB breach workflow, rights SLAs |
| FR-021 Retention/legal-hold/erasure | P05 (redaction-marker erasure), `JOB-G01-RETENTION`, `VAL-HOLD` | statutory retention floor, legal holds |
| FR-022 Governed DOB/category/name | **P01 (named-authority 4-eyes) + RBAC §7 `E·AR`**, P05 | `VAL-G01-GOVCHANGE`, gazette ref |
| FR-023 Category/PwD certificates | P02, `JOB-G01-CERT-EXPIRY`, X.2, G13 | `VAL-G01-CATEGORY-CERT` |
| FR-024 Deceased succession | P01 (4-eyes), X.3 handoff to **G11-PEN**, P05 | family-pension linkage (G11 owns award) |
| FR-025 Phonetic/transliteration search | P02 (scope+mask on results) | Indic matcher (`VAL`-free; engineering) |

**Net-new (`GAP (gov-specific)`) authored by G01** (per `MODULE_RECONCILIATION.md` §D, all running on P05/P01/P06): the `service_no`/`cadre`/`pay_scale_id` + posting-history extension; the Aadhaar Reference-Key vault; the governed statutory-field-change machine; the retention/legal-hold/erasure reconciliation; and the alias identity-resolution map. Everything else is platform configuration.
...
| A5 | ~30 invented semantic error codes (`SOD_VIOLATION`, `DUPLICATE_CANDIDATE`, `STALE_VERSION`, …) | the **8 standard codes** + each former code now a **`details.reason` discriminator** surfaced as an `ERR-G01-*` message (§8.2) | §C rows 1–3; Foundation §1 |
...
| A9 | Invented role list (DPO, SR Custodian, Appointing Authority, Auditor, System Admin, …) | **RBAC v1.7 roles** + gov roles/flags as **ADDITIONS**; Auditor → Org-Admin read + entitlement; System Admin → Org/Platform Admin (§3) | §C "role list" row; `PLATFORM_FOUNDATION.md` §6.6 |
| A10 | `service_register_events` treated as a shared platform entity | **net-new G12-SR ledger** on the P05 substrate; G01 emits to it via the `outbox_events` backbone | §C "service_register_events" row; §D |
...
| A15 | Module code `M01-EPM`; ids `MSG-G01-*` | **`G01-EPM`**; jobs reuse `JOB-M01-*` (EXTEND of M01) + new `JOB-G01-*`; notices `MSG-G01-*`; errors `ERR-G01-*`; cite `VAL-*`, author `VAL-G01-*` | §B; §C; Foundation §2/§4/§5 |
...
| A18 | `employees` as a fresh gov table | **EXTEND of PrimeSoft `M01` `employees`** (reuse `employee_id`/`user_id`/`official_email`/`lifecycle_state`); gov fields + E21–E34 as EXTENSIONS | §A "M01-EPM" row; §D; BRD §4.4 |
...
| A20 | Consent as a module-owned ledger | **platform `consent_records`** (DPDPA, immutable/superseded, `VAL-CONSENT`); G01 references, does not redefine | §D footnote; Platform §P05, Vision §2.6 |

---

## Amendments (v3 → v3.1: cross-module remediation)

Surgical reconciliation against the frozen G12 SR-ingestion contract and shared-entity registry. Authority: `docs/review/REMEDIATION.md` (D1, D2, D5) consolidated from R1–R5.

| # | v3 (pre-remediation) | v3.1 (remediated) | Authority |
|---|---|---|---|
| R1 | SR events "emitted to G12 via outbox producer"; write path implicit | **§8.6 added** — G01 is the identity/qualification SR writer; posts to canonical write-port **`POST /api/v1/sr/ingest`** (reversals `…/ingest/reversal`); outbox drainer is a façade that **relays** there (never `/sr/events`, never a direct `service_register_events` INSERT). §8.4/§8.5/§5.2/§A1 rows re-pointed to the write-port. | REMEDIATION D1; G12 FR-01/FR-02 |
| R2 | Informal SR labels (HIRE/ATTRIBUTE/DEATH/GOVERNED_FIELD_CHANGED) | **Cite verbatim G12 `event_type` codes** for G01: `APPOINTMENT`, `NAME_CHANGE`, `DOB_CHANGE`, `GENDER_CHANGE`, `CATEGORY_CHANGE`, `DECEASED`, `QUALIFICATION_ADDED`, `DEPARTMENTAL_EXAM_PASSED` (§8.6 mapping table; supersede informal labels for the SR ledger). | REMEDIATION D1/D2 |
| R3 | No explicit ingest payload/dedup contract on G01's side | **Ingest payload contract (§8.6):** dedup tuple `(source_module="G01", source_reference_id, source_event_version)` (legacy `source_event_id`→`source_reference_id`); explicit `tenant_id`+`entity_id`; `fact_key` for qualifying-service-bearing types per `fact_correlation_rule`; corrections via `is_reversal=true` + `reverses_source_reference_id` (no invented correction verb). | REMEDIATION D1 |
| R4 | Effective-dating job cited as `JOB-M01-EFFDATE` | **`JOB-G01-EFFDATE`** registered/owned by G01 as the canonical effective-dating job (G02 and downstream cite it; `JOB-M01-EFFDATE` is its M01 substrate alias) — §2.2, §4.5. | REMEDIATION D5 |
| R5 | `employee_dependents` (E4) ownership implicit | **Confirmed G01-owned and fully defined here**; **G03 references it** (read-only, no re-declaration / divergent enum; G03 extras = satellite). No definition change. | REMEDIATION D5 |
...
| Feature area | Owned by G01? | Notes / hand-off |
|---|---|---|
...
| Identity & statutory documents | **Yes** | `employee_identity_documents` (refs G13 scans) |
...
| Bank & financial | **Yes** | `employee_bank_accounts` (consumed by G10/G11) |
| Photo & **isolated biometric** reference | **Yes** | `employee_photos` (binary in G13; biometric purpose-isolated) |
...
| Edit-request approval workflow | No → **G02** | G01 exposes fields + commit path only |
| Document binary storage | No → **G13** | G01 holds `document_id` references |
| Service-event ledger | No → **G12** | G01 writes events via shared producer |
| **Disbursement-readiness gating** | No → **G10** | `NO_VERIFIED_BANK` precondition + cheque fallback |
| **Family-pension award** | No → **G11** | G01 hands off heir/nominee linkage on DECEASED |
...
```
                    +---------------------------+
   Hire feed  --->  |        G01-EPM            |  ---> G12-SR (service events via outbox)
   (G05 joining)    |  Employee Master (SSOT)   |  ---> G10-PAY (bank attrs; G10 owns NO_VERIFIED_BANK gate)
                    |                           |  ---> G11-PEN (bank, pension, family-pension heir handoff)
   G02 edit-commit  |  - profile data           |  ---> G03/G04 (identity, org, manager)
   (approved) --->  |  - effective-dated jobs    |  ---> G06 (designation/position placement)
                    |  - effective-dated         |  ---> G09 (reporting line, identity)
   Bulk import ---> |    positions + attrs       |  ---> G14 (analytics reads)
   (PROVISIONAL)    |  - PII access + break-glass|  <--- G13 (document_id for scans/photos)
                    |  - consent/rights/retention|  <--> UIDAI vault basis (Aadhaar Data Vault, G01-internal)
   identity via --> |  - employee_id_aliases     |  ===> change-feed (outbox) consumed by G02-G14
   alias resolve    +---------------------------+
```
...
| **SR Custodian / Registrar** | new entity-scoped role + capability flag on the G12 ledger | **ADDED role + flag** (mirrors Document Admin; on P05 substrate) |
...
| Maintain contact/address | request→G02 | — | U | U | — | — | — | — | — |
| Maintain dependents/nominees/**heirs** | request→G02 | — | U | U | — | — | — | — | — |
| Maintain education/experience | request→G02 | — | U | U | — | — | — | — | — |
| Maintain identity/statutory IDs | request→G02 | — | U (masked, 4-eyes) | U (4-eyes) | — | — | R(masked) | R(masked) | — |
| **Read Aadhaar (vault reveal)** | own (masked) | — | — | A (4-eyes + reason) | — | — | R(audit) | R(masked) | — |
| Maintain bank/financial | request→G02 | — | U (4-eyes) | U (4-eyes) | A | — | — | R(masked) | — |
| Upload/replace photo | request→G02 | — | U | U | — | — | — | — | — |
...
| **Governed DOB/category/name change** | request→G02 | — | C (request) | A (approve) | A (appointing) | — | R | R | — |
...
| View audit log (G01) | own actions | — | scoped | scoped | scoped | scoped | privacy scope | full RO | full RO |
...
gov↔PrimeSoft mapping (`docs/brd/MODULE_RECONCILIATION.md`) wholesale; the legacy `SHARED_FOUNDATION.md`
is superseded where they conflict. The following is the G01-specific binding of those platform contracts.
...
- **Backend:** REST API at **`/api/v1`**; PostgreSQL primary store; binary objects in **G13 (PrimeSoft M11 Document vault)**, encrypted at rest; a CQRS read projection for the 360° view (engineering detail); a **DB-polled transactional outbox (`outbox_events`)** that drains G01's statutory events to the **G12-SR ledger** (P05 substrate) and to change-feed consumers.
- **Auth (Platform §0.2):** Bearer-token (JWT) session carrying resolved roles + tenant/entity scope; Google SSO / username-password (hashed); **MFA enforced by default for HR Admin / Org Admin** and high-privilege gov roles. Endpoints never re-implement permission logic — they call **P02 `Authorization.check`**.
...
> All of the above carry `tenant_id` (and `entity_id` where entity-scoped); G01 reads/writes them only through the platform's data-layer-scoped access.

### 4.3 Cross-agent build instructions

1. **G01 is foundational** — build before G02–G14 transactional logic; expose the consumption API
...
2. **Do not write a second employee master.** Any module needing person/job data calls the G01 API or
   reads the G01-owned tables under G01's access policy. No local caching of PII beyond TTL-bounded,
   non-persistent caches.
3. **Edit approvals route to G02.** When a self-service user requests a personal-detail change, G01
   surfaces the field set + validators; G02 runs maker-checker; on approval G02 calls G01's
   **commit endpoint** (`PATCH /employees/{id}:commit`) which performs the effective-dated write +
   audit + SR event emission via the outbox. G01 never invents an approval flow.
4. **Documents/photos** are stored in G13; G01 persists only `document_id`. Producers must obtain a
   `document_id` from G13 before saving a reference.
5. **Service events** (profile creation, separation, key job/position/attribute changes, merge,
   governed-field change, death) are emitted to G12 **through the transactional `outbox_events` table**
   and the shared `service_register_events` producer; G01 never writes the G12 ledger schema directly.
...
7. **Identity is resolved through `employee_id_aliases`.** A merge **never** re-points another module's
   rows. The merge consolidates only G01 satellites, soft-deletes the loser, writes
...
G01 **cites** the Foundation `VAL-*`/`JOB-*`/`MSG-*`/`ERR-*` catalogues by id and authors only module-unique `*-G01-*` ids, registered in the Foundation indexes.

- **Validation library (cite, never restate):** `VAL-EMAIL`, `VAL-MOBILE`, `VAL-PAN`, `VAL-AADHAAR` (12-digit Verhoeff, stored masked per Appendix B), `VAL-IFSC`, `VAL-DOB` (age ≥ 18 at DOJ), `VAL-EFFECTIVE`, `VAL-PINCODE`, `VAL-NOMINEE` (per-class shares sum to 100), `VAL-CONSENT`, `VAL-FILE`, `VAL-COMMENT`, `VAL-ENUM`, `VAL-MASTER-UNIQUE`. **Gov-authored:** `VAL-G01-SERVICENO`, `VAL-G01-AADHAAR-VAULT` (lawful-basis present + single-home), `VAL-G01-GOVCHANGE` (governed DOB/category/name alteration cap + gazette), `VAL-G01-CATEGORY-CERT` (validity/percentage), `VAL-G01-BREAKGLASS` (volume cap/anomaly), `VAL-G01-PROVISIONAL` (migration-relaxed profile), `VAL-HOLD` (legal-hold integrity).
- **Background jobs (X.1 runner; cite PrimeSoft M01 + author gov):** **`JOB-G01-EFFDATE`** is the canonical effective-dating job for the employee master, **registered and owned by G01** (it applies staged effective-dated job/org/position/core-attribute changes); **G02 and all downstream modules cite `JOB-G01-EFFDATE`** (the prior `JOB-M01-EFFDATE` reference is its M01-platform substrate alias — do not invent `JOB-M01-EFFDATE`-vs-`JOB-G01-EFFDATE` divergence or an unregistered `JOB-*-EFFDATE` variant). Also reuse `JOB-M01-COMPLETE` (completeness recompute), `JOB-M01-ORGVIEW` (org-chart refresh), `JOB-M01-CONTRACT`, `JOB-M01-LWD`, `JOB-M01-VISA-REMIND`; author `JOB-G01-CERT-EXPIRY`, `JOB-G01-RETENTION`, `JOB-G01-REMEDIATION`, `JOB-G01-DEDUP-SWEEP`, `JOB-G01-BREAKGLASS-ANOMALY`. Terminal failure → `JOB-FAIL` → `MSG-SYS-JOBFAIL`.
- **Notifications (X.2; templates by id):** all §11 notices are `MSG-G01-*`; statutory notices (separation, governed-change, breach, death/handoff) are **mandatory/non-suppressible**.
- **Errors (shared ERR-* + gov):** reuse `ERR-FORBIDDEN`, `ERR-PRECOND`, `ERR-REASON-REQ`, `ERR-DUP-INSTANCE`, `ERR-PAST-DATED`, `ERR-LOADFAIL`; author `ERR-G01-*` (e.g. `ERR-G01-CAP` break-glass cap, `ERR-G01-GOVLOCK` governed-field lock, `ERR-G01-HEIR` heir-required) as `details.reason` discriminators under the 8 standard codes (§8.2).
...
**Entity count, stated once cleanly (improvement #22, R18):** G01 owns **34 conceptual entities (E1–E34)
...
| Entity | Owner module | G01 action | Consumers |
|---|---|---|---|
| `employees` | **G01** | extend + own | G02–G14 (read, via alias), G02 (commit writes) |
| `employee_*` satellites (E2–E11, E13, E16, E18) | **G01** | own | G10/G11 (bank), G12 (events), G14 (read) |
| `positions` (E12), `position_history` (E24) | **G01** | own (with G06 input), eff-dated | G05/G06, G14 |
| `employee_id_aliases` (E21) | **G01** | own (identity resolution) | **all G02–G14** |
| `aadhaar_vault` (E22) | **G01** | own (vault) | none (reveal only, audited) |
| `employee_attribute_history` (E23) | **G01** | own | G11 (DOB/category as-of), G06 (seniority), G14 |
| `employee_certificates` (E25) | **G01** | own | G10/G11 (benefits), G14 |
| `privacy_notices`/`consent_records`/`data_principal_requests`/`breach_incidents` (E26–E29) | **G01** | own (DPDP governance) | DPO, Auditor, G14 |
| `retention_policies`/`legal_holds` (E30/E31) | **G01** | own | G09 (hold), G11 (pension hold), G13 (purge coordination) |
| `governed_field_change_requests` (E32) | **G01** | own | G02 (workflow), G11/G06 |
| `outbox_events` (E33) | **G01** | own (backbone) | G02–G14 consumers |
...
employees 1───∞ employee_identity_documents ─── scan document_id ──▶ documents (G13)
...
employees 1───∞ employee_photos ─── document_id ──▶ documents (G13); biometric_template_ref (opaque)
...
employee_dependents/nominees ──(on DECEASED)──▶ legal-heir linkage ──▶ G11 family-pension
field_access_policies (field_path, requires_four_eyes) ── governs ──▶ any employee_* column
outbox_events ──(drains to)──▶ service_register_events (G12) + change feed (G02–G14)
...
`audit_log`/`security_audit_log` are **platform-owned (P05)**, not G01 tables.

#### E1 — `employees` (extends canonical; G01-owned columns) — *v2 modified*
...
> **Canonical ownership (D5).** `employee_dependents` is **fully defined and owned here by G01**. **G03
> references this G01 entity** (read-only) and MUST NOT re-declare it with divergent field names or a
> divergent relationship enum; any G03-specific fields are a G03 satellite keyed to this entity. No
...
| `proof_document_id` | UUID | FK→documents(G13), NULL | birth/marriage cert |
...
| `is_family_pension_recipient` | BOOLEAN | default false | **v2: hand-off to G11 on DECEASED (FR-024)** |
| `effective_date` | DATE | NOT NULL | |
| `requires_four_eyes` | BOOLEAN | default true (PENSION/GRATUITY) | **v2: high-risk 4-eyes (improvement #20)** |
| `proof_document_id` | UUID | FK→documents(G13), NULL | nomination form |
...
| `scan_document_id` | UUID | FK→documents(G13), NULL | |
...
| `document_id` | UUID | FK→documents(G13), NULL | binary lives in G13 (NULL for pure biometric ref) |
...
| `source_module` | VARCHAR(10) | NULL | G05/G06 origin |
...
| `value_document_id` | UUID | FK→documents(G13), NULL | |
...
|---|---|---|---|
| `batch_id` | UUID | PK | |
| `file_document_id` | UUID | FK→documents(G13) | source file |
...
| `proof_document_id` | UUID | FK→documents(G13), NULL | |
...
| `source_module` | VARCHAR(10) | NULL | G09/G11 origin |
...
| `proof_document_id` | UUID | FK→documents(G13), NOT NULL | mandatory documentary proof |
...
10. **Photo binary externalised:** `employee_photos.document_id` must resolve to an existing G13 doc
...
11. **Merge via alias (v2, improvement #1/R2):** a merge writes exactly one `employee_id_aliases` row
    (`loser_id` UNIQUE), consolidates **only G01 satellites**, soft-deletes the loser, and emits
    `RECORDS_MERGED`. **No cross-module FK is ever re-pointed by G01.** All identity lookups resolve
...
|---|---|---|---|---|---|---|---|
| lh-0001 | 11111111-...-0002 | DISCIPLINARY | Pending inquiry CV-2026-12 | G09 | 2026-04-01T00:00:00Z | NULL | ACTIVE |
| lh-0002 | 11111111-...-0033 | PENSION | Pension dispute WP-441 | G11 | 2026-02-10T00:00:00Z | NULL | ACTIVE |
| lh-0003 | 11111111-...-0007 | LITIGATION | Service matter | G09 | 2025-11-01T00:00:00Z | 2026-05-01T00:00:00Z | RELEASED |
...
full Low-Level Design table). Roles use Section 3 keys. "→G02" denotes the edit-approval hand-off.
...
a `PROFILE_CREATED` event to G12 via the outbox. Multi-step wizard (Personal → Contact/Address →
...
6. The view is read-only; edit affordances appear only for permitted roles (self-service → G02).
...
| Dependencies & Reuse | Field-access engine (FR-013), completeness (FR-014), alias (FR-015), G13 photo URL, async audit sink, outbox consumer. |
...
- **Module:** G01-EPM
- **Primary Role(s):** HR Officer, HR Admin (Employee requests → G02)
...
official email**, and **optimistic concurrency** on every write. Employee-initiated changes are G02
...
6. Employee self-service edits create an G02 request, not a direct write.
...
| `workflow_instances` (G02) | INSERT | for self-service requests |
...
| Authorization Logic | HR write within org scope; self-service routes to G02; auditor read-only. |
| State Changes & Side Effects | Primary reassignment; verification; notification on verify; G02 request for self edits. |
...
| Dependencies & Reuse | Notification (OTP), G02 workflow, audit, advisory completeness recompute. |
| Test Guidance | Unit: format validators, primary invariant, unique email. Integration: address effective-date transition; row_version conflict. E2E: verify flow; self-service→G02. |
...
rank** (feeds FR-024 deceased handling), proof-document linkage to G13, and **4-eyes on
...
5. Proof documents linked via `document_id` (uploaded to G13 first).
...
7. **PENSION/GRATUITY nominee changes require 4-eyes** (`requires_four_eyes=true`) and emit
   `NOMINEE_UPDATED` to G12.
8. Self-service edits route to G02.
...
| `documents` (G13) | reference | proof docs |
...
| Data Operations | CRUD dependents/nominees; FK to G13 docs; workflow_task for 4-eyes; outbox; audit. |
...
| Authorization Logic | HR write in scope; self-service→G02; statutory nominee 4-eyes; proof doc required. |
...
| Dependencies & Reuse | G13 upload, outbox producer, G02 workflow, 4-eyes engine, FR-024 (succession), audit. |
...
- **Module:** G01-EPM
- **Primary Role(s):** HR Officer, HR Admin, Employee (own, → G02 for self edits)
...
for ACTIVE employees (advisory). Self-service edits may apply immediately or route to G02 per policy.
...
| Authorization Logic | HR write in scope; self-service per config (immediate or G02). |
...
3. `is_government_service=true` experience contributes to pensionable-service summary (read by G11).
...
| `documents` (G13) | reference | certificates |
...
| Data Operations | CRUD; G13 doc refs; audit; advisory completeness recompute. |
| Validation Logic | One-highest invariant; date bounds; overlap warning; min-qualification advisory DQ. |
| Authorization Logic | HR write in scope; verified-row edit only Admin; self-service→G02. |
...
| Dependencies & Reuse | G13, G11 (pension read), advisory DQ engine (FR-014), audit. |
...
- **Module:** G01-EPM
- **Primary Role(s):** HR Officer (masked), HR Admin (4-eyes for PAN/Aadhaar; Employee requests → G02)
...
vault row. Verification, expiry alerts, scan linkage to G13. **PAN and Aadhaar writes/reveals require
...
| Authorization Logic | Write HR Admin (4-eyes for PAN/Aadhaar); HR Officer masked; raw never returned except audited vault reveal; self-service→G02. |
...
| Dependencies & Reuse | KMS/crypto, `aadhaar_vault`, dedup engine, G13, FR-013 break-glass, notification (expiry), async audit. |
...
create/change, and **optimistic concurrency**. Consumed by G10/G11. **Note:** G10 — not G01 — gates
disbursement; G01 simply exposes the verified primary (improvement #6).
...
5. Verification (penny-drop or manual) sets `is_verified`; **G10 consumes only verified primary and owns
   the `NO_VERIFIED_BANK` disbursement precondition with a cheque fallback** (G01 never blocks pay).
...
change near payroll cut-off; unverified primary (G10 falls back to cheque, never withholds).
...
| State Changes & Side Effects | PENDING→ACTIVE on approval; primary reassignment; BANK_DETAIL_CHANGED notification; G10 reads verified primary. |
| Failure Handling | maker=checker→403 SOD_VIOLATION; invalid IFSC→422; duplicate→409; version mismatch→409 STALE_VERSION. |
| Dependencies & Reuse | Workflow engine (4-eyes), KMS, notification, G10/G11 consumers (disbursement gate owned by G10), audit. |
| Test Guidance | Unit: IFSC validator, one-primary invariant. Security: SOD, token secrecy. Integration: approval→effective; G10 reads only verified primary; **assert G01 never sets a payroll block**. E2E: 4-eyes flow. |
...
- **Module:** G01-EPM
- **Primary Role(s):** HR Officer, HR Admin (Employee upload → G02/approval); HR Admin (biometric)
...
**Description:** Upload/replace/approve profile photos (binary in G13, metadata in `employee_photos`,
...
biometric template is ever stored in G01** (improvement #3/R1).

**Acceptance Criteria:**
1. Photo binary uploaded to G13 first; G01 stores `document_id`, dimensions, type,
...
7. Replacing a photo retains prior versions (G13 versioning).
...
| `documents` (G13) | reference | binary + versions |
...
**Edge Cases:** oversized/wrong format; no face detected (warn); replacing primary while pending; G13
...
| Backend-Service Flow | Photo: upload→G13→insert PENDING→approve sets primary + updates employees.primary_photo_id. Biometric: check GRANTED consent → store opaque ref + retention_until; on withdrawal disable + schedule deletion. |
| Data Operations | INSERT/UPDATE employee_photos; UPDATE employees.primary_photo_id; consent lookup; G13 reference; audit. |
...
| Failure Handling | Bad file→422; G13 failure→rollback metadata; approve nonexistent→404; biometric w/o consent→403 CONSENT_REQUIRED; concurrent primary→409 STALE_VERSION. |
| Dependencies & Reuse | G13 storage/versioning, image-validation, consent (FR-020), retention (FR-021), audit. |
| Test Guidance | Unit: validation rules; consent gate. Integration: G13 upload+rollback; consent-withdrawal deletion. E2E: upload→approve→primary; biometric blocked without consent. |
...
logic inside the transaction (no DB trigger)** (improvement #16/R12). G05/G06 placements are recorded
...
| Dependencies & Reuse | G05/G06 callers, outbox producer, org_units/designations masters, audit; FR-011 as-of reads position_history. |
...
attributes were overwritten. Reads only; it does not bypass G02 or the governed-change workflow (FR-022).
...
- Core attribute changes are written **only** via FR-001 (hire seed), G02 commit, migration, or the
...
| Authorization Logic | System Admin config; HR Admin proposes; value writes follow normal employee edit auth (self→G02). |
...
owned by G10 as `NO_VERIFIED_BANK` with a cheque fallback. **Phasing note:** v1 ships a **single fixed
...
6. HR sees an org-level completeness/DQ dashboard feed (consumed by G14).
...
| Data Operations | UPSERT completeness; UPDATE employees cache; read config + certificates; emit G14 feed. |
...
| State Changes & Side Effects | Advisory DQ flag changes; nudge notifications; G14 metrics. **No downstream block.** |
| Failure Handling | Engine error→retain prior snapshot + alert; config invalid→reject. |
| Dependencies & Reuse | Triggered by all write FRs; FR-023 certificates; consumed by G14; notification. **G10 does NOT read this flag for gating.** |
...
only G01 satellites, soft-deletes the loser, writes an `employee_id_aliases(loser_id → survivor_id)`
row, and emits `RECORDS_MERGED{survivor_id, loser_id}` on the change feed.** **No foreign keys in
G10/G11/G12 or any other module are ever re-pointed by G01** (improvement #1/R2). Consumers resolve
...
3. **Merge consolidates all G01 satellites under the surviving `employee_id`, soft-deletes the loser,
...
7. A `RECORDS_MERGED` SR event is emitted to G12 via the outbox.
...
| `employees` + satellites | UPDATE/MOVE | consolidate to survivor; soft-delete loser **(G01-owned only)** |
...
| Backend-Service Flow | `DedupEngine.match()` (deterministic + FR-025 phonetic) → upsert candidates; `MergeService.merge(survivor, loser, picks)` in tx: move/keep **G01 satellites**, soft-delete loser, INSERT `employee_id_aliases` with snapshot, emit RECORDS_MERGED outbox. **No cross-module writes.** `undo` restores snapshot + flags alias reversed + re-emits. |
| Data Operations | CRUD candidates; UPDATE **G01-owned** satellites only; INSERT alias + snapshot; soft-delete; outbox; audit. |
...
| Dependencies & Reuse | FR-001/FR-007 (triggers), FR-025 (matcher), outbox/change-feed (FR-019), audit. **Explicitly NOT G10/G11/G12 schemas.** |
| Test Guidance | Unit: scoring, threshold gating, chained-alias collapse. Integration: transactional merge writes alias + tombstone; **assert zero writes to non-G01 tables**; undo restore + re-emit. E2E: review→4-eyes merge→consumer resolves survivor→undo. |
...
`is_self_visible` policies, with "request change" actions that create G02 edit requests (no direct
...
2. All edit affordances create an G02 request; the self-service view never writes master data directly.
...
6. Pending G02 requests and pending rights requests are visible with status.
...
| `workflow_instances` (G02) | SELECT/INSERT | pending / new requests |
...
| POST | `/api/v1/me/change-requests` | create G02 edit request |
...
| Backend-Service Flow | `SelfServiceController` binds to token employee_id; reads via FieldAccessResolver isSelf=true; change-request→G02; rights-request→FR-020; export builds portable JSON. |
| Data Operations | SELECT self projection; INSERT G02 request; INSERT data_principal_request; async audit. |
...
| State Changes & Side Effects | G02 + rights requests created; export logged; no master mutation. |
| Failure Handling | No login→403; cross-employee→403; export throttle→429. |
| Dependencies & Reuse | FR-013 (field access), FR-020 (rights), G02 workflow, FR-014 (advisory), async audit. |
| Test Guidance | Unit: self-scope binding. Security: cannot read others. Integration: change-request→G02; rights-request→FR-020. E2E: view→request→track; export; erasure-blocked-by-hold message. |
...
**Acceptance Criteria:**
1. Upload accepts the versioned template (CSV/XLSX); file stored in G13; batch created with a chosen
   `validation_profile`.
...
| Dependencies & Reuse | G13 (file), dedup engine (FR-015), FR-025 (phonetic), advisory completeness (FR-014), outbox, async job runner, audit. |
...
7. **DECEASED triggers FR-024** (heir linkage + family-pension handoff to G11).

**Business Rules:**
- DECEASED separations trigger nominee/pension workflows downstream (G11) and lock self-service.
...
| Data Operations | UPDATE employees + assignment; UPDATE users; SELECT legal_holds; outbox; audit; downstream triggers (G11 for pension/death). |
...
- **Module:** G01-EPM
- **Primary Role(s):** Service principals of G02–G14 (machine), Auditor (RO)
...
- **Consumers MUST resolve identity through the feed/alias; they must never re-point their own FKs from
  a G01 merge — they re-resolve to the survivor** (improvement #1).
...
| Dependencies & Reuse | FR-013 (masking), FR-011 (as-of), FR-015 (alias), FR-025 (phonetic), outbox backbone (§8.5); consumed by G02–G14. |
...
(§5.6 r18). Coordinates with G13 for document purge.
...
4. A legal hold can be placed by Dept Head/SR Custodian/DPO (often from G09/G11) and blocks
...
| Backend-Service Flow | `RetentionService` sweep flags records at horizon → ARCHIVE; `LegalHoldService` place/release; `ErasureEngine.evaluate(employee, request)` → check retention (overridable?) + active holds → decision + recorded precedence; purge path coordinates G13 + aadhaar_vault under sign-off. |
| Data Operations | CRUD policies/holds; UPDATE record_state; conditional vault/G13 purge; UPDATE data_principal_requests; audit; outbox SR event. |
...
| Dependencies & Reuse | FR-018 (lifecycle states), FR-020 (erasure requests), G09/G11 (holds), G13 (doc purge), Aadhaar vault (FR-007), audit. |
| Test Guidance | Unit: horizon + precedence logic. Integration: erasure refused under hold/retention; lawful purge cascades vault+G13. E2E: place hold→erasure refused→release→purge. |
...
**Business Rules:**
- DOB changes alter the retirement date (read by G11) — the new value propagates via the attribute
  history and feed; G11 reconciles superannuation.
...
| Data Operations | CRUD requests; INSERT attribute_history; UPDATE employees cache; outbox; audit; G13 proof ref. |
...
| State Changes & Side Effects | New effective-dated attribute version; cache update; SR event; G11/G06 reconcile via feed. |
...
| Dependencies & Reuse | FR-011 (attribute history + as-of), G13 (proof), outbox, G11/G06 consumers, audit. |
...
| `documents` (G13) | reference | certificate scan |
...
| Data Operations | CRUD certificates; G13 scan ref; status recompute; advisory completeness feed; audit. |
| Validation Logic | Validity dates; percentage range; UDID presence for PwD; creamy-layer for OBC. |
| Authorization Logic | HR write in scope; self-service→G02; verification HR. |
...
| Dependencies & Reuse | G13, FR-014 (advisory DQ), FR-022 (governed category value), notification (expiry), audit. |
...
- **User Story:** *As an HR Admin, when an employee dies in service, I want the system to capture/confirm
  legal heirs and nominees, hand the family-pension recipient off to G11, and transfer the data-principal
...
**`DEATH`** SR event, **hands the heir/nominee linkage off to G11** to create the family-pensioner award
(G11 owns the award; G01 provides the linkage), and **transfers data-principal rights** (access/erasure
exercised by the heir) under DPDP via FR-020. G01 does not compute the pension; it provides the
...
3. **A family-pension handoff is published to G11** (via the feed / a typed integration event) carrying
   the recipient nominee/heir reference; **G11 creates the family-pensioner record** (G01 does not).
...
5. Where a new family-pensioner *person record* is needed, G01 exposes a creation hook but the pensioner
   master/award is owned by G11 (bounded context preserved).
6. All succession actions are audited and 4-eyes approved.

**Business Rules:**
- Heir authorisation for data-rights requires recorded proof (succession/legal-heir certificate via G13).
...
| POST | `/api/v1/employees/{id}/family-pension:handoff` | publish recipient to G11 |
...
family-pension recipient selector; handoff status to G11; heir-rights panel; conflicting-claim warning.
...
| Backend-Service Flow | `SuccessionService` (invoked by FR-018 DECEASED): require heir + recipient → 4-eyes approve → tx{ UPDATE dependents/nominees, set DECEASED/record_state, outbox DEATH + family-pension handoff } → G11 consumes handoff to create family-pensioner award; heir-rights route to FR-020 with succession authorisation. |
| Data Operations | UPDATE dependents/nominees/employees; INSERT data_principal_requests (heir); outbox; audit; G13 proof refs. |
...
| State Changes & Side Effects | DECEASED; heir/recipient confirmed; DEATH SR event; G11 family-pension creation (downstream); self-service locked; heir rights enabled. |
...
| Dependencies & Reuse | FR-018 (DECEASED), FR-004 (heirs/nominees), FR-020 (rights succession), FR-021 (holds), G11 (family-pension award), outbox, G13, audit. |
| Test Guidance | Unit: heir/recipient required; conflict detection. Integration: DEATH event + G11 handoff; heir-rights authorisation; erasure refused under pension hold. E2E: record death→confirm heirs→handoff→G11 creates award. |
...
| Section editors | Cards/tables with add/edit modals; primary toggles; verification badges; effective-date pickers; **concurrency conflict prompt (row_version)**; self-service "request change" routing to G02. |
...
| Deceased/succession | Death-recording workflow, heir confirmation grid (rank, proof), family-pension recipient selector, G11 handoff status. |
...
| `INTERNAL` | 500 | unexpected server error (upstream/KMS/G13/G12 failures map here via X.3 + `ERR-LOADFAIL`; **no 503**) |

### 8.2.1 G01 semantic reasons (`details.reason` discriminators → standard code)

The v2 invented codes are **not** wire codes; each is now a `details.reason` value carried under a standard code (and surfaced as an `ERR-G01-*` / shared `ERR-*` message). In the FR failure-handling tables below, a token such as `SOD_VIOLATION` denotes *(standard code, reason)*.

| `details.reason` | Standard code (HTTP) | Message id | When |
|---|---|---|---|
| `INVALID_ID` | `VALIDATION_FAILED` (422) | `ERR-G01-IDFMT` | statutory-ID checksum/format failure |
| `REASON_REQUIRED` | `VALIDATION_FAILED` (422) | `ERR-REASON-REQ` | break-glass reveal without reason |
| `BATCH_TOO_LARGE` / `OUT_OF_RANGE` | `VALIDATION_FAILED` (422) | `ERR-G01-RANGE` | batch/as-of bounds violation |
...
| `GOVERNED_FIELD_LOCKED` | `FORBIDDEN` (403) | `ERR-G01-GOVLOCK` | raw update of DOB/category/name outside FR-022 |
| `CONSENT_REQUIRED` | `FORBIDDEN` (403) | `ERR-G01-CONSENT` | biometric/processing without valid `consent_records` |
| `DUPLICATE_CANDIDATE` | `CONFLICT` (409) | `ERR-DUP-INSTANCE` | dedup HIGH match blocks create |
| `PRIMARY_REQUIRED` / `SHARE_SUM_INVALID` | `CONFLICT` (409) | `ERR-G01-INVARIANT` | sole-primary removal; nominee shares ≠ 100 |
| `OVER_STRENGTH` / `POSITION_INACTIVE` / `INVALID_STATE` | `CONFLICT` (409) | `ERR-G01-STATE` | sanctioned-strength / position / lifecycle conflict |
| `MERGE_CONFLICT` / `UNDO_EXPIRED` | `CONFLICT` (409) | `ERR-G01-MERGE` | conflicting merge states; rollback past window |
| `STALE_VERSION` | `CONFLICT` (409) | `ERR-G01-STALE` | optimistic-lock `row_version` mismatch |
| `LEGAL_HOLD_ACTIVE` | `CONFLICT` (409) | `ERR-G01-HOLD` | purge/erasure blocked by active legal hold (FR-021) |
...
| `BREAK_GLASS_CAP` | `RATE_LIMITED` (429) | `ERR-G01-CAP` | break-glass volume cap reached (FR-013) |
| `KMS_UNAVAILABLE` | `INTERNAL` (500) | `ERR-LOADFAIL` | Aadhaar-vault KMS down → write fails closed |

> **Removed semantics:** the v1 `DQ_BLOCKED`/payroll-block path is **deleted**. G10 owns the
> `NO_VERIFIED_BANK` disbursement precondition; G01 emits no payroll-blocking error (improvement #6/R3).
...
|---|---|---|
| G13 Document Mgmt | G01 → G13 | upload binary, store `document_id`; resolve URLs; coordinated purge (FR-021) |
| G12 Digital SR | G01 → G12 | **G01 is the SR writer** for identity/qualification life events; posts to the canonical write-port **`POST /api/v1/sr/ingest`** (reversals `…/ingest/reversal`) via the transactional **`outbox_events`** drainer (façade relays only — never `/sr/events`, never a direct ledger INSERT). Codes/payload per **§8.6**. |
| G02 Edit Workflow | G02 → G01 | approved edits commit via `PATCH /employees/{id}:commit` |
| G10 Payroll | G01 → G10 | consumption API (verified primary bank); **G10 owns `NO_VERIFIED_BANK` gate + cheque fallback — G01 never blocks pay** |
| G11 Pension | G01 → G11 | consumption API; **family-pension heir/recipient handoff (FR-024); DOB/category via attribute history** |
| G05/G06 Transfer/Promotion | them → G01 | create assignments via assignment API |
| G09 Disciplinary | G09 → G01 | **legal holds (FR-021)**; reads reporting line/identity |
| G14 Analytics | G01 → G14 | consumption API + advisory completeness summary feed |
| Aadhaar Data Vault / KMS | G01-internal | Reference-Key vault; KMS envelope encryption (FR-007) |
| Notification platform | G01 → users/DPO | OTP, expiry alerts, change notices, **real-time break-glass/breach alerts** |
| OIDC/SSO + MFA | platform | authentication for all surfaces |
| Change-feed consumers | G01 → G02–G14 | **DB-polled outbox backbone (§8.5); alias-transparent identity** |
...
### 8.6 SR Event Posting Contract — G01 is the identity/qualification SR writer *(v3.1 — cross-module remediation D1/D2)*

G01 owns the employee master, so **G01 is the SR writer** for identity-, personal-data-, and
qualification-life events. **G02 is the approval workflow and is NOT an SR source**: an approved
G02 change commits to G01 via `PATCH /employees/{id}:commit`, and **G01 posts the resulting SR
event**. G03 does not post identity/qualification events either.

**Canonical write-port (G12-frozen, D1).** Every SR event is posted to the single ledger write-port
**`POST /api/v1/sr/ingest`** (reversals to **`POST /api/v1/sr/ingest/reversal`**). G01's transactional
...
direct `service_register_events` table INSERT. G01 never writes the G12 ledger schema directly.

**Event-type codes (cite G12-published `event_type_code` verbatim — D1/D2).** Each row below cites the
exact code G12 publishes for G01 (with its `allowed_source_modules` including `G01` and its
`payload_schema`); G01 emits only these codes for these life events:

| G01 life event | Trigger (FR) | G12 `event_type` (verbatim) | `fact_key` (per type `fact_correlation_rule`) |
|---|---|---|---|
...
> These G12 codes are the authoritative SR-ledger event types and **supersede** the informal emit
> labels (`HIRE`/`ATTRIBUTE`/`DEATH`/`GOVERNED_FIELD_CHANGED`) used elsewhere in this spec for the SR
> ledger. The `outbox_events.event_type` change-feed codes (§8.5, E33) remain G01's internal
> consumer-feed taxonomy for G02–G14; the drainer maps each to the canonical G12 code above when it
> relays to `POST /api/v1/sr/ingest`.

**Ingest payload contract (D1).** Every `POST /api/v1/sr/ingest` call from G01 populates:

| Field | Value / rule |
|---|---|
| `source_module` | **`"G01"`** (explicit, never inferred; validated against the type's `allowed_source_modules`) |
| `source_reference_id` | G01's stable writer-local reference (the `outbox_events.event_id`, or `governed_field_change_requests.id` for FR-022 changes) — renamed from any legacy `source_event_id` |
| `source_event_version` | monotonic version for the same `source_reference_id` (bumped on correction) |
| `tenant_id` | explicit, required (G12 hashes `tenant_id`+`employee_id` into `entry_hash`) |
| `entity_id` | explicit, required |
| `event_type` | one of the verbatim G12 codes above |
| `fact_key` | derived and sent for qualifying-service-bearing types per the type's `fact_correlation_rule` (G12 FR-01); missing on a type that requires it → `SR_FACT_KEY_REQUIRED` |
| `payload` | type-specific, conforming to the G12 `payload_schema` for that `event_type` |

The dedup tuple **`(source_module, source_reference_id, source_event_version)`** is the persisted
idempotency contract; the HTTP `Idempotency-Key` header value MAY be a G01-local hash of that tuple.

**Corrections / reversals (D1 — supersede-only).** A governed correction or erroneous post is reversed
through G12's **`is_reversal=true` + `reverses_source_reference_id`** envelope on
`POST /api/v1/sr/ingest/reversal` (G12 auto-spawns the published `*_REVERSAL` partner type). G01
...
| Availability | 99.5% uptime; graceful degradation when G13/G12/KMS down (queue events, defer doc ops; Aadhaar writes fail closed). |
...
| ACTIVE | leave sanctioned (G03) | ON_LEAVE | approved long leave |
| ON_LEAVE | rejoin | ACTIVE | leave ended |
| ACTIVE | suspension (G09) | SUSPENDED | disciplinary order |
| SUSPENDED | revoke / reinstate | ACTIVE | order revoked |
| ACTIVE/ON_LEAVE | transfer out (G05) | TRANSFERRED | relieving complete |
...
|---|---|---|---|
| Profile created on hire | New employee, HR, manager | Email + in-app | `MSG-G01-PROFILE_CREATED` |
| Contact/email OTP verification | Employee | SMS/Email | `MSG-G01-CONTACT_OTP` |
| Identity/certificate expiring (90/30/7d) | Employee, HR | Email + in-app | `MSG-G01-ID_EXPIRY` / `MSG-G01-CERT_EXPIRY` |
| Bank detail changed | Employee | Email + SMS | `MSG-G01-BANK_CHANGED` |
| Bank/nominee change pending approval | Approver | In-app | `MSG-G01-APPROVAL_PENDING` |
| Profile completeness nudge (advisory) | Employee | In-app + Email | `MSG-G01-COMPLETENESS_NUDGE` |
| Dedup candidate raised | HR Admin | In-app | `MSG-G01-DEDUP_CANDIDATE` |
| **Break-glass threshold/anomaly (real-time)** | **DPO** | In-app + Email (immediate) | `MSG-G01-BREAK_GLASS_ALERT` |
| **Personal-data breach** | DPO; affected principals; (DPB notification logged) | Email + in-app | `MSG-G01-BREACH_NOTIFY` |
| **Data-principal request status / SLA** | Employee/heir; DPO | Email + in-app | `MSG-G01-DP_REQUEST` |
| **Governed field change applied** | Employee, HR, G11/G06 | Email + in-app | `MSG-G01-GOVERNED_CHANGE` |
| **Consent granted/withdrawn** | Employee; DPO | In-app | `MSG-G01-CONSENT` |
| Separation approved | Employee, HR, payroll | Email + in-app | `MSG-G01-SEPARATION` |
| **Death recorded / family-pension handoff** | HR, G11, heirs | Email + in-app | `MSG-G01-DEATH_SUCCESSION` |
| Import completed/failed; remediation queued | Initiator, HR Admin | In-app + Email | `MSG-G01-IMPORT_RESULT` |
| **PROVISIONAL record needs remediation** | HR Officer | In-app | `MSG-G01-REMEDIATION_DUE` |

All notifications are dispatched through **X.2** (the platform notification infrastructure): `IN_APP` + `EMAIL` fire in parallel for approvals; **statutory notices (separation, governed-change, breach, death/handoff) are mandatory and non-suppressible** (Platform §X.2; BRD §9.9); retry backoff ×5 + dead-letter; every dispatch audit-logged. Template keys above are **`MSG-G01-*`** ids registered in the Foundation §5 index; copy is referenced by id, never inlined. **No PII values in bodies** (reference, not content); respect user locale and channel preferences. **Break-glass/breach alerts are real-time, not digests** (improvement #10).
...
|---|---|---|
| Headcount by org/cadre/designation/status/**record_state** | Live counts with drill-down | G14, HR |
| Profile completeness distribution (advisory) | % at score bands per org unit | G14, HR |
...
| **Position history / Pay-Commission impact** | Effective-dated position changes | HR Admin, G14 |
...
| Tenure & age profile | Service length, retirement-due pipeline (uses governed DOB) | G11, HR |
...
| 5 — Consumer integration | G02–G14 integrate against consumption API + **change-feed backbone**; **alias resolution** contract tests pass. | All consumers green on contract tests incl. alias. |
...
| FR-EPM-003 | contacts, addresses | /contacts, /addresses | HR (Self→G02) | notification, G02, row_version |
| FR-EPM-004 | dependents, nominees | /dependents, /nominees | HR (Self→G02) | G13, outbox, 4-eyes, FR-024 |
| FR-EPM-005 | emergency_contacts | /emergency-contacts | HR, Self | audit |
| FR-EPM-006 | education, experience | /education, /experience | HR (Self→G02) | G13, G11 |
| FR-EPM-007 | aadhaar_vault, identity_documents | /identity-docs, /aadhaar:reveal | HR Admin (4-eyes) | KMS, dedup, G13, FR-013 |
| FR-EPM-008 | bank_accounts | /bank-accounts | HR (4-eyes), Dept Head | workflow, G10/G11 (G10 owns gate) |
| FR-EPM-009 | photos, consent_records | /photos, /biometric-ref | HR (Self→approval), Admin | G13, FR-020 consent, FR-021 |
| FR-EPM-010 | positions, position_history, job_assignments | /positions, /assignments, /org-chart | HR Admin, Dept Head | G05/G06, outbox (service-layer sync) |
...
| FR-EPM-014 | profile_completeness, certificates | /completeness | HR, Self | G14 (**advisory; no G10 gate**) |
| FR-EPM-015 | dedup_candidates, employee_id_aliases | /dedup/*, /resolve | HR Admin (4-eyes) | outbox RECORDS_MERGED, FR-025 |
| FR-EPM-016 | read_model, data_principal_requests, G02 | /me/* | Employee | field-access, G02, FR-020 |
| FR-EPM-017 | import_batches, staging | /imports/*, /remediation-queue | HR Admin/Off | dedup, advisory completeness, outbox |
| FR-EPM-018 | employees, job_assignments, users, legal_holds | /:separate, /:reactivate, /:archive | HR Admin, Dept Head, SR Cust. | outbox, G11, FR-021, FR-024 |
...
| FR-EPM-021 | retention_policies, legal_holds | /retention/*, /legal-holds | Admin, Dept Head, DPO | FR-018, FR-020, G09/G11, G13, vault |
| FR-EPM-022 | governed_field_change_requests, attribute_history | /governed-changes | HR Off/Admin, Appointing Auth. | G13, outbox, G11/G06 |
| FR-EPM-023 | employee_certificates | /certificates | HR (Self→G02) | G13, FR-014 advisory, notification |
| FR-EPM-024 | dependents, nominees, data_principal_requests | /death:record, /family-pension:handoff | HR Admin, SR Cust., DPO | FR-018, FR-020, G11, outbox DEATH |
...
- **Internal deps:** FR-002 depends on FR-013 + CQRS projection; FR-011 depends on FR-010
  ...
  FR-025 + aliases; FR-016 depends on FR-013 + FR-020 + G02; FR-018 depends on §10 + FR-021 + FR-024;
  FR-020 depends on FR-021; FR-022 depends on FR-011; FR-024 depends on FR-004 + FR-018 + FR-020.
- **External deps:** G13 (documents/photos), G12 (SR events via outbox), G02 (edit approvals), KMS +
  Aadhaar vault, notification platform, OIDC/SSO, G10 (disbursement gate), G11 (family pension/DOB).
...
| C — Sensitive data + vault | FR-007, FR-008, FR-009 | A + KMS + vault + G13 ready |
...
| G — Self-service & view | FR-002 (CQRS), FR-016 | A + FR-013 + FR-020 ready (G02 stub ok) |
...
|---|---|---|---|
| `employees` extended fields (v2: Aadhaar via vault, mononym, record_state) | §5.4 E1 | all FRs, G02–G14 | ✅ Resolved |
| 35 G01-owned tables (E1–E34, import pair = 2) | §5.1 | FRs 001–025 | ✅ Resolved |
...
| Alias identity resolution + RECORDS_MERGED | FR-015, §5.4 E21 | FR-019, G02–G14 | ✅ Resolved |
| Aadhaar Data Vault (Reference-Key) | FR-007, §5.4 E22 | FR-001/007/021 | ✅ Resolved |
| Effective-dated core attributes + positions | FR-011/010, §5.4 E23/E24 | FR-011/022, G06/G11 | ✅ Resolved |
...
| Advisory completeness (no payroll block) | FR-014 | G14 | ✅ Resolved |
| DPDP consent/rights/breach + retention/legal-hold/erasure | FR-020/021, §5.4 E26–E31 | DPO, G09/G11/G13 | ✅ Resolved |
| Governed DOB/category/name change | FR-022, §5.4 E32 | FR-011, G11/G06 | ✅ Resolved |
| Category/PwD certificates | FR-023, §5.4 E25 | FR-014, G10/G11 | ✅ Resolved |
| Deceased succession + family-pension handoff | FR-024 | G11, FR-020 | ✅ Resolved (G11 owns award) |
...
| **RBAC role mapping (gov actors as ADDITIONS; Auditor/SysAdmin mapped)** | §3.0 | §3 matrix | ✅ Resolved (§C) |
| **VAL-*/JOB-M01-*/MSG-G01-*/ERR-G01-* cited & registered** | §4.5, §11, §8.2.1 | all FRs | ✅ Resolved (Foundation §2/§4/§5) |
...
| **G12-SR ledger destination (outbox drains to G12 on P05)** | §4.2, §8.5, Alignment §A1 | FR-019, all SR emits | ✅ Resolved (§C/§D) |
...
| O5 | Biometric vault API contract (opaque ref) | Track C + vendor | Finalised during track C build; G01 stores only opaque ref. |
| O6 | G11 family-pension award creation contract (handoff payload shape) | G11 team | Freeze handoff event with G11 before FR-024 GA. |
...
| `employee_id_aliases` | G01-owned map (`loser_id → survivor_id`) every consumer resolves identity through. |
...
| Outbox / change-feed backbone | The transactional `outbox_events` table draining ordered, replayable, tombstoned events to G12 and consumers. |
...
| Family pension | The pension payable to a deceased employee's eligible heir/recipient (award owned by G11). |
...
1. Master reference data (org_units, designations, cadres, pay_scales) is loaded/owned outside G01 and
   available at build time.
2. G13/G12/G02/notification/KMS service contracts are available as referenced; the **Aadhaar vault + KMS**
...
3. Biometric capture/matching engine integration details (vault API) are finalised during track C build —
   G01 stores only opaque references (O5).
...
6. **G11 owns the family-pension award**; G01 provides the heir/recipient handoff only (O6).
...
| R3 DQ blocks payroll | Critical | FR-014 advisory; G10 owns `NO_VERIFIED_BANK`; §1.1 |
...
*End of G01-EPM BRD v3.0 (platform-grounded) — 25 functional requirements; 34 entities (E1–E34, 35 tables,
...
error table + `X-Correlation-Id`, and the Foundation `VAL-*`/`JOB-M01-*`/`MSG-G01-*`/`ERR-G01-*` catalogues
