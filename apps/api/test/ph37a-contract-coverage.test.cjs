...
test("PH-37A ratchet floor: coverage does not regress below the recorded baseline", () => {
  const record = coverage();
  // Ratchet floor recorded in docs/reviews/contract-coverage-20260703.md (PH-43A raised it to 436 / 33%).
  assert.ok(record.implementedTotal >= 436, `implemented routes ${record.implementedTotal} < floor 436`);
  assert.ok(record.totalPct >= 33, `coverage ${record.totalPct}% < floor 33%`);
});
