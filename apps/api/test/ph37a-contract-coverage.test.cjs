...
test("PH-37A ratchet floor: coverage does not regress below the recorded baseline", () => {
  const record = coverage();
  // Ratchet floor recorded in docs/reviews/contract-coverage-20260703.md (PH-51A raised it to 489 / 37%).
  assert.ok(record.implementedTotal >= 489, `implemented routes ${record.implementedTotal} < floor 489`);
  assert.ok(record.totalPct >= 37, `coverage ${record.totalPct}% < floor 37%`);
});
