...
CREATE TYPE g07_master_status       AS ENUM ('DRAFT','PUBLISHED','RETIRED');
CREATE TYPE g07_model_scope_type    AS ENUM ('ROLE','DESIGNATION','CADRE','ORG_UNIT','GENERIC');
CREATE TYPE g07_competency_type     AS ENUM ('FUNCTIONAL','BEHAVIOURAL','LEADERSHIP','DIGITAL');
CREATE TYPE g07_skill_source        AS ENUM ('SELF','MANAGER','ASSESSMENT','TRAINING','CREDENTIAL');
CREATE TYPE g07_skill_status        AS ENUM ('DECLARED','VALIDATED','REJECTED');
CREATE TYPE g07_freshness_status    AS ENUM ('FRESH','STALE');
...
CREATE TYPE g07_gap_item_source     AS ENUM ('MODEL','APPRAISAL','MANDATORY');
CREATE TYPE g07_gap_contract_status AS ENUM ('CURRENT','SUPERSEDED');
CREATE TYPE g07_certification_status AS ENUM ('ACTIVE','EXPIRED','REVOKED');
CREATE TYPE g07_campaign_status     AS ENUM ('DRAFT','LAUNCHED','COMPLETED','CANCELLED');
CREATE TYPE g07_target_status       AS ENUM ('PENDING','NOMINATED','COMPLETED','EXEMPTED','OVERDUE');
...
CREATE TABLE g07_skill_categories (
    ...
    parent_category_id uuid REFERENCES g07_skill_categories(id) ON DELETE RESTRICT,
...
CREATE TABLE g07_skills (
    ...
    tenant_id               uuid NOT NULL REFERENCES tenants(id) ON DELETE RESTRICT,
    entity_id               uuid REFERENCES entities(id) ON DELETE RESTRICT,
    skill_category_id       uuid NOT NULL REFERENCES g07_skill_categories(id) ON DELETE RESTRICT,
    code                    varchar(40) NOT NULL,
    name                    varchar(150) NOT NULL,
...
CREATE TABLE g07_competency_model_items (
    id                       uuid PRIMARY KEY DEFAULT gen_random_uuid(),
    tenant_id                uuid NOT NULL REFERENCES tenants(id) ON DELETE RESTRICT,
    competency_model_id      uuid NOT NULL REFERENCES g07_competency_models(id) ON DELETE RESTRICT,
    competency_id            uuid NOT NULL REFERENCES g07_competencies(id) ON DELETE RESTRICT,
...
CREATE TABLE g07_employee_skills (
    ...
    skill_id                  uuid NOT NULL REFERENCES g07_skills(id) ON DELETE RESTRICT,
    ...
    freshness_status          g07_freshness_status NOT NULL DEFAULT 'FRESH',
...
CREATE TABLE g07_skill_gap_items (
    id                        uuid PRIMARY KEY DEFAULT gen_random_uuid(),
    tenant_id                 uuid NOT NULL REFERENCES tenants(id) ON DELETE RESTRICT,
    skill_gap_analysis_id     uuid NOT NULL REFERENCES g07_skill_gap_analyses(id) ON DELETE RESTRICT,
    competency_id             uuid NOT NULL REFERENCES g07_competencies(id) ON DELETE RESTRICT,
    target_proficiency_level  integer NOT NULL,
    ...
    source                    g07_gap_item_source NOT NULL,
    created_at                timestamptz NOT NULL DEFAULT now(),
    updated_at                timestamptz NOT NULL DEFAULT now(),
    created_by                uuid,
    updated_by                uuid,
...
CREATE TABLE g07_gap_contracts (
    ...
    status                g07_gap_contract_status NOT NULL DEFAULT 'CURRENT',
    created_at            timestamptz NOT NULL DEFAULT now(),
    updated_at            timestamptz NOT NULL DEFAULT now(),
    created_by            uuid,
    updated_by            uuid,
...
CREATE TABLE g07_certifications (
    ...
    renewed_by_certification_id uuid REFERENCES g07_certifications(id) ON DELETE SET NULL,
    renewal_of_certification_id uuid REFERENCES g07_certifications(id) ON DELETE SET NULL,
...
CREATE TABLE g07_campaign_targets (
    id                      uuid PRIMARY KEY DEFAULT gen_random_uuid(),
    tenant_id               uuid NOT NULL REFERENCES tenants(id) ON DELETE RESTRICT,
    training_campaign_id    uuid NOT NULL REFERENCES g07_training_campaigns(id) ON DELETE RESTRICT,
    employee_id             uuid NOT NULL REFERENCES employees(id) ON DELETE RESTRICT,
    wave_no                 integer,
