#!/usr/bin/env bash
# PH-27A oracle: G13 DPDP DSR console UI — a real controlled web surface using the injected client
# with canonical loading/error/empty states; the client carries the adjudicateDsr method; web suites green.
set -uo pipefail
cd "$(git -C "$(dirname "$0")" rev-parse --show-toplevel 2>/dev/null || echo /Users/n15318/hrms)"
fail=0; red(){ echo "  RED  $*"; fail=1; }; grn(){ echo "  ok   $*"; }
have(){ grep -qE "$2" "$1" 2>/dev/null && grn "$3" || red "$3"; }
COMP="apps/web/src/modules/g13/DataSubjectRequestConsole.tsx"; CLIENT="apps/web/src/api/hrmsClient.ts"; FIX="apps/web/src/api/fixtureHrmsClient.ts"; APP="apps/web/src/App.tsx"
echo "== PH-27A exit-criteria (G13 DPDP DSR console UI) =="
[ -d node_modules ] || red "node_modules absent"
[ -f "$COMP" ] && grn "component present: $COMP" || red "missing component: $COMP"
have "$COMP" 'onSubmit=|onClick=' "component has a submit/click handler"
have "$COMP" 'useState' "component uses controlled state"
have "$COMP" 'adjudicateDsr' "component calls client.adjudicateDsr"
have "$COMP" 'OperationalState|"loading"|"error"|"empty"' "component renders canonical states"
have "$CLIENT" 'adjudicateDsr' "client interface/impl exposes adjudicateDsr"
have "$FIX" 'adjudicateDsr' "fixture client implements adjudicateDsr"
have "$APP" 'g13/DataSubjectRequestConsole.tsx' "surface mounted in App"
if [ -d node_modules ]; then
  npm run -s typecheck >/dev/null 2>&1 && grn "api typecheck green" || red "api typecheck failed"
  npm run -s web:typecheck >/dev/null 2>&1 && grn "web typecheck green" || red "web typecheck failed"
  npm test >/dev/null 2>&1 && grn "npm test green" || red "npm test failed"
  npm run -s web:test >/dev/null 2>&1 && grn "web:test green" || red "web:test failed"
fi
echo "== $([ "$fail" -eq 0 ] && echo 'GREEN - PH-27A met' || echo 'RED - PH-27A not complete') =="; exit "$fail"
