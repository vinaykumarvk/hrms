# Employee Disciplinary Cases and Punishment Management — Government HRMS Module BRD (G09, v3.0 · platform-grounded)

**Module code:** G09 (Government HRMS · `SHARED_FOUNDATION` M09-DCP; re-keyed per `MODULE_RECONCILIATION.md` §B)
**Program:** Government HRMS — a public-sector configuration and extension of the **PrimeSoft HRMS platform** (Master BRD v2.1 · Product Vision v2.6 · Platform Spec v1.6 · RBAC Design v1.7 · Foundation FS v1.6).
**Document version:** v3.0 (platform-re-grounded; supersedes v2.0 / v1.0).
**Relationship:** **NET-NEW (gov-specific)** statutory due-process engine — PrimeSoft has no disciplinary module (`MODULE_RECONCILIATION.md` §A, row G09; §D). The due-process business logic is authored here but **runs on platform engines**: P01 (workflow/SLA), P05 (audit/immutability), P06 (migration). It posts penalty/exoneration to the **G12 SR ledger** (P05 substrate) and flows effects by reference to **G06** (seniority/promotion), **G10** (recovery/subsistence) and **G11** (pension).
...
The Employee Disciplinary Cases and Punishment Management module (**G09-DCP**) digitises the **end-to-end disciplinary lifecycle** of a public-sector / government employer, from the first receipt of a complaint or detection of misconduct through preliminary inquiry, optional suspension, framing of articles of charge, departmental inquiry, penalty imposition, and statutory appeal / revision / review — concluding with an immutable posting of the outcome (punishment **or** exoneration) into the Digital Service Register (G12).
...
| O5 | Propagate outcomes to dependent modules accurately and idempotently | Penalty/exoneration posted exactly-once to G12 SR; effects flowed to G06/G10/G11; 0 duplicate downstream effects |
...
Out of scope (owned elsewhere, integrated here): the SR ledger itself (G12), payroll execution of recoveries/subsistence (G10), pension re-computation engine (G11), seniority list recomputation (G06), the physical document store (G13), identity/SSO and the DSC/eSign trust service (platform), and external court/police case systems (reference-linked only).
...
|---|---|---|---|
| AI-1 / R1 | `authority_competence` reference entity + DI-13 penalty-competence guard; error `ERR-G09-AUTHORITY-NOT-COMPETENT` (Art. 311(1)) | Critical | New entity **E23 `authority_competence`** (§5.2); **DI-13** (§5.6); **FR-G09-018** (competence matrix + finalise-time guard); error code §10.3; state guard §12.4 |
| AI-2 / R2 | `case_consultations` entity + DI-14; finalise blocked until required consultations closed; error `ERR-G09-CONSULTATION-PENDING` (UPSC/CVC) | Critical | New entity **E24 `case_consultations`** (§5.2); **DI-14** (§5.6); **FR-G09-019**; error §10.3; gate in FR-011 finalise |
| AI-3 / R3 | Lawful "dispense-with-inquiry" path (`inquiry_dispensations`, reason-coded) — Art. 311(2) provisos | High | New entity **E25 `inquiry_dispensations`** (§5.2); **DI-23** (§5.6); **FR-G09-020**; satisfies DI-3 major branch via documented exception |
| AI-4 / R4 | Document/witness list supplied + inspection afforded right; inquiry cannot proceed to evidence without it | High | Fields on `inquiry_proceedings` (`list_supplied_date`, `inspection_afforded_date`); **DI-24** (§5.6); amended **FR-G09-007**; error `ERR-G09-INSPECTION-NOT-AFFORDED` |
| AI-5 / R5 | Separate `legal_service` record from informational `notifications`; valid `service_mode` per template; EMAIL non-statutory by default | High | New entity **E26 `legal_service_records`** (§5.2); **DI-15** (§5.6); **FR-G09-021**; valid modes carried on `procedure_templates` |
| AI-6 / R6 | Subsistence: `non_employment_certificate_received` gate; deemed-review / auto-flag on 90-day charge-memo lapse (Ajay Kumar Choudhary) | High | Fields on `suspensions` (`non_employment_certificate_received`, `nec_received_date`, `charge_memo_due_date`, `deemed_review_flag`); **DI-16** (§5.6); amended **FR-G09-003** |
| AI-7 / R7 | Common/joint proceedings — `case_respondents` (1..N) sharing inquiry/evidence; per-respondent article findings | High | New entity **E27 `case_respondents`** (§5.2); **DI-25** (§5.6); **FR-G09-022**; `charge_articles` finding moved to per-respondent grain |
| AI-8 / R8 | Two-layer model: `procedure_template` overlay (consultations, competence ref, timelines, service modes, limitation, dispensation); kernel invariant | High | New entity **E22 `procedure_templates`** (§5.2); **DI-17** (§5.6); **FR-G09-017**; `procedure_template_id` on `disciplinary_cases` |
...
| AI-10 / R10 | `sla_pause_events` ledger + recompute `expected_closure_date`/`sla_target_at` on stay/remit/condonation/consultation/criminal-stay | Medium | New entity **E28 `sla_pause_events`** (§5.2); **DI-18** (§5.6); **FR-G09-024**; amended FR-016 |
| AI-11 / R11 | Idempotency-key lifecycle (mint, dedup store, TTL); double-finalise / double post-to-SR ⇒ exactly one effect | Medium | New entity **E30 `idempotency_keys`** (§5.2); **DI-19** (§5.6); §10.1 lifecycle; amended **FR-G09-013**; error `ERR-G09-IDEMPOTENCY-CONFLICT` |
| AI-12 / R12 | Mandatory `proportionality_reasoning` on `penalty_orders` + analytics outlier flag | Medium | Field on **E16** (§5.2); **DI-20** (§5.6); amended **FR-G09-011** + **FR-G09-016**; error `ERR-G09-PROPORTIONALITY-REASONING-REQUIRED` |
| AI-13 / R13 | Mid-proceeding jurisdiction transfer (re-resolve DA), sealed-cover promotion freeze, Rule 9 four-year-bar + sanction for retirees | Medium | `case_jurisdiction_transfer` timeline event + fields; **DI-27** (§5.6); **FR-G09-026**; sealed-cover flag feeds G06 |
| AI-14 / R14 | Abatement terminal handling: `order_type=ABATED`, `case_status=ABATED`; auto-stop SLA + suppress effects on death | Low/Med | Enum extensions (§5.5); **DI-26** (§5.6); **FR-G09-028**; state §12.1 |
| AI-15 / R15 | Court-grade hash-chained `audit_log`/`case_timeline_events` (prev-hash linkage) + verify endpoint | Medium | `row_hash`/`prev_hash` on **E19** (§5.2); **DI-21** (§5.6); **FR-G09-027**; verify API §10 |
| AI-16 / R16 | POSH/ICC procedure template for HARASSMENT — Internal Committee composition/timelines feeding penalty stage | High | `procedure_templates` ICC variant; `inquiry_appointments` ICC roles; **FR-G09-023**; error `ERR-G09-ICC-PROCEDURE-REQUIRED` |
| AI-17 / R17 | DSC/eSign with trusted timestamp + bound signatory identity on charge-sheets, reports, notices, orders | Medium | Signature fields on `penalty_orders`/`legal_service_records`/`case_documents`; **DI-28** (§5.6); **FR-G09-021**; error `ERR-G09-SIGNATURE-REQUIRED` |
| AI-18 / R18 | Inquiry `STAYED` sub-state + auto SLA pause when parallel criminal trial active | Medium | `inquiry_status=STAYED` (§5.5); fields on `inquiry_proceedings`; amended **FR-G09-007**; pause reason `CRIMINAL_STAY` (FR-024) |
| AI-19 / R19 | Charged-Officer Portal → rights-and-deadlines surface (entitlements + limitation countdown) | Low | Amended **FR-G09-005** UI; §7.1 portal redesign; §7.3 rights panel |
| AI-20 / R20 | Recovery-penalty caps (≤1/3 pay, instalments, not beyond retirement except DCRG) validated pre-emit | Medium | **DI-22** (§5.6); amended **FR-G09-011**; error `ERR-G09-RECOVERY-CAP-EXCEEDED` |
| AI-21 | Personal-hearing record for show-cause and appeal stages | — | New entity **E29 `personal_hearings`** (§5.2); **DI-29** (§5.6); **FR-G09-025**; amended FR-010/FR-012 |
...
| AI-24 | Disclose adverse PI material when relied upon as evidence; served/relied-upon flag | — | **DI-9** extended (§5.6); amended **FR-G09-002**/**FR-G09-007**; `relied_upon`/`disclosed` flags on `inquiry_exhibits` |
...
| Area | Feature | FR(s) |
|------|---------|-------|
| Intake | Complaint / source-of-misconduct registration & triage | FR-G09-001 |
| Fact-finding | Preliminary (fact-finding) inquiry (+ adverse-material disclosure) | FR-G09-002 |
| Interim action | Suspension (parallel status), revocation, subsistence + NEC gate, deemed review | FR-G09-003 |
| Charge | Charge-sheet / articles of charge / statement of imputations | FR-G09-004 |
| Defence | Employee's written statement of defence; admission/denial; rights surface | FR-G09-005 |
| Inquiry setup | Appointment of Inquiry Officer & Presenting Officer | FR-G09-006 |
| Inquiry conduct | Hearings, witnesses, exhibits, daily order sheets, ex-parte, inspection right, STAY | FR-G09-007 |
| Inquiry close | Inquiry report & findings (proved / not proved) | FR-G09-008 |
| DA consideration | Disagreement memo & disciplinary authority decision | FR-G09-009 |
| Natural justice | Show-cause on proposed penalty (major penalty / enhancement) | FR-G09-010 |
| Outcome | Penalty order (minor/major) & exoneration; proportionality; recovery caps | FR-G09-011 |
| Remedies | Appeal / revision / review | FR-G09-012 |
| Propagation | SR posting & downstream effects (G06/G10/G11/G12) — idempotent | FR-G09-013 |
| Records | Document evidence vault (link to G13) | FR-G09-014 |
| Oversight | Integrity register & vigilance clearance status | FR-G09-015 |
| Governance | SLA / statutory-timeline tracking & case analytics | FR-G09-016 |
| Configuration | Procedure-template & jurisdiction overlay (two-layer model) | FR-G09-017 |
| Legality | Authority-competence matrix & finalise-time competence guard | FR-G09-018 |
| Legality | Statutory consultation management (UPSC/CVC/ICC/legal) | FR-G09-019 |
| Legality | Dispense-with-inquiry (Article 311(2) provisos) | FR-G09-020 |
| Legality | Legal service of documents & digital signing (DSC/eSign) | FR-G09-021 |
| Scale | Common/joint (multi-respondent) proceedings | FR-G09-022 |
| Compliance | POSH / Internal Committee procedure | FR-G09-023 |
| Governance | SLA pause/resume & clock management | FR-G09-024 |
| Natural justice | Personal hearing record (show-cause / appeal) | FR-G09-025 |
| Lifecycle | Mid-proceeding jurisdiction transfer, sealed cover & retiree proceedings | FR-G09-026 |
| Integrity | Court-grade hash-chained audit & verification | FR-G09-027 |
| Lifecycle | Abatement on death / terminal handling | FR-G09-028 |

### 2.2 Common Capabilities (inherited, applied module-wide)

- **Maker-checker / workflow** runs on the **platform P01 WorkflowEngine** (`workflows` / `workflow_instances` / `workflow_actions`; **not** an invented `workflow_actions` table) for every stage transition that changes statutory state. The disciplinary stages (preliminary inquiry → charge-sheet → inquiry → DA consideration/consultation → show-cause → penalty → appeal) are **configured P01/W.1 flow definitions** whose subjects are the G09 case/charge/inquiry/penalty/appeal entities; G09 calls `WorkflowEngine.startInstance/advance/approve/reject/sendBack/delegate/cancel` (each idempotent) and does **not** build a parallel engine.
...
- **Two-layer procedure model:** an invariant natural-justice **kernel** plus a configurable **`procedure_template`** overlay; the kernel is never parameterised away (DI-17). The overlay is realised as **W.1 flow + W.2 form** configuration on P01; G09 builds no generic BPM/rules designer.
- **SLA & statutory-timeline runtime** is the **P01 per-stage SLA timer + escalation** runtime (breach → escalation event → X.2 notification + P05 audit); lawful clock pause/resume is driven by **`JOB-G09-SLA`** / **`JOB-G09-TIMELINE`** background jobs registered against the **X.1** runner and indexed in Foundation §4 (pause/resume recompute, DI-18).
...
- **Document handling** via **G13** document vault (the gov document module running on PrimeSoft M11 Document Management) — versioned, encrypted, access-controlled `documents` objects referenced by metadata.
- **Notifications** via **X.2** infrastructure: `IN_APP` + `EMAIL` fire in parallel; **EMAIL for statutory notices is mandatory and non-suppressible** (Platform §X.2; BRD §9.9). Templates referenced by `MSG-G09-*` id (Foundation §5), never inlined.
...
- **Multi-tenancy:** every G09 entity carries **`tenant_id`** (and `entity_id` where entity-scoped); scoping is enforced at the **data layer** and a query without a resolvable tenant scope is rejected, not defaulted to "all" (Platform §0.1).
...
|----------|-----------|----------|
| G01-EPM (employees) | read | Charged employee(s), complainant, IO/PO identity, designation, cadre, org_unit, employment_status |
| G05-TRJ (transfers) | read (event) | Mid-proceeding transfer triggers jurisdiction re-resolution (FR-026) |
| G06-PPP | write (event) + read | Reduction in rank, withholding of promotion, seniority re-fixation; **sealed-cover** promotion freeze |
| G10-PAY | write (event) | Suspension → subsistence (NEC-gated); recovery (capped); pay reduction; promotion/increment withholding |
| G11-PEN | write (event) | Pension cut/withholding; effect of removal/dismissal/CR; retiree-proceeding flags; Rule 9 |
| G12-SR (service_register_events) | write (append-only, idempotent) | Suspension, penalty, exoneration, appeal outcome, abatement events |
| G13-DMS | read/write | Evidence vault objects, charge-sheet/order PDFs (DSC-signed) |
| G14-DAS | read | KPI feed incl. proportionality-outlier dashboard |
...
| Platform **P05** Audit & G12 SR substrate | write/read | DB-trigger dual-log audit; G12 SR ledger append (P05 immutability substrate) |
| Platform **X.1 / X.2 / P04 / P06** | use | `JOB-G09-*` jobs; X.2 notifications; integration credentials; P06 legacy migration |

### 2.4 Explicit non-goals

- G09 does **not** compute payroll, pension, or seniority figures; it emits **events** that the owning module executes.
- G09 does **not** store document binaries; it stores references to G13 objects.
- G09 does **not** operate the DSC/eSign Certifying Authority; it calls the platform trust service and stores returned signature metadata.
...
- G09 is **not** a generic BPM designer; configurability is **capped** at the named overlay items in `procedure_templates` (DI-17).
...
> | **Disciplinary Case Manager / HR-DCP Admin** | map to **HR Admin** (RBAC §2.2) + G09 module-admin entitlement | operational, non-deciding |
...
| Vigilance Officer | Screens complaints, maintains integrity register & vigilance clearance | G09 |
...
| Inquiry Officer (IO) | Conducts the departmental inquiry impartially | Shared (G09-specific) |
| Presenting Officer (PO) | Presents the case on behalf of the department before the IO | G09 |
| Internal Committee (ICC) Member / Presiding Officer / External Member | POSH Internal Committee for HARASSMENT cases (replaces IO route) | G09 (POSH) |
| Defence Assistant (DA-Asst) | Person assisting the charged officer | G09 |
| Disciplinary Case Manager / HR-DCP Admin | Operates the case workbench, drafts artefacts, manages SLAs | Shared (HR Admin) |
| Consulting Authority Liaison | Records UPSC/CVC/legal consultation references and advice | G09 |
| Appellate Authority | Decides appeals | G09 |
| Reviewing / Revising Authority | Exercises suo-motu review/revision | G09 |
...
- **Architecture & deployment:** logical architecture and NFR are owned by the platform (Vision Part 2 / Platform Spec); the physical stack is an engineering choice within that logical architecture, and the deployment target (gov cloud / CGG Data Centre) is a Standalone/Group-Company deployment model — **not** re-architected here (`MODULE_RECONCILIATION.md` §C). REST under **`/api/v1`**; PostgreSQL with **data-layer tenant/entity scoping**; encrypted object storage via **G13** for binaries.
- **Multi-tenancy (was omitted):** every G09 table carries **`tenant_id`** (non-nullable) and, where entity-scoped, **`entity_id`**; cross-entity reach is an Org-Admin widened scope filter, never a bypass; an unscoped query is rejected, not defaulted to "all" (Platform §0.1).
...
- **Validation:** cite the 36-entry `VAL-*` catalogue by id (Foundation §2); author only module-unique `VAL-G09-*` (e.g. `VAL-G09-DUEPROCESS` charge→reply→inquiry→penalty sequencing, `VAL-G09-COMPETENCE`, `VAL-G09-SERVICE`) and register them in the Foundation index.
- **Audit (P05, not a custom log):** the **dual log** (`audit_log` + `security_audit_log`) is captured by **DB trigger** on every mutation, immutable, PII-masked, ≥ 7-yr retention; reading an audit row is itself audited. Tamper-evidence tracks **OPEN-PLAT-03** hash-chaining (chain head → WORM). G09 defines **no** `audit_log` of its own.
- **Notifications (X.2):** `IN_APP` + `EMAIL` in parallel; **EMAIL for statutory notices mandatory/non-suppressible** (BRD §9.9); templates by `MSG-G09-*` id.
- **Background jobs (X.1):** `JOB-G09-SLA` (stage SLA pre-breach/breach escalation), `JOB-G09-TIMELINE` (statutory-timeline + pause/resume recompute), `JOB-G09-EFFDATE` (effective-dated order/penalty effects) registered against the runner and indexed in Foundation §4; runner guarantees idempotency, backoff ×3, `JOB-FAIL`→`MSG-SYS-JOBFAIL`, per-tenant isolation, run audit row.
- **Migration (P06):** legacy disciplinary registers migrate through ETL+V (3 staging dry runs, waves, `migration_runs`, `<gov>_source_id` traceability).
...
**Reused platform-provided entities (referenced, never redefined):** `employees`, `users`, `org_units`, `designations`/`cadres`/`pay_scales` (G01 master on PrimeSoft M01), `documents` (G13 / PrimeSoft M11), `notifications`, `consent_records`, `integration_credentials`, `migration_runs`, the **P05** `audit_log` / `security_audit_log`, and the **P01** `workflows` / `workflow_instances` / `workflow_actions` (the invented `workflow_actions` is replaced by `workflow_actions`). `service_register_events` is **not** a platform primitive — it is the **net-new G12 SR ledger** running on the P05 substrate, which G09 **appends to** (DI-10/DI-19).
...
|---|--------|------|-------|---------|:--:|
| E1 | `disciplinary_cases` | Module | G09 | Master case record (the file) | — |
| E2 | `case_complaints` | Module | G09 | Source-of-misconduct / complaint intake records | — |
| E3 | `preliminary_inquiries` | Module | G09 | Fact-finding inquiry before formal charges | — |
| E4 | `suspensions` | Module | G09 | Suspension / revocation / subsistence allowance (NEC + deemed review) | — |
| E5 | `charge_sheets` | Module | G09 | Memorandum of charges (articles of charge container) | — |
| E6 | `charge_articles` | Module | G09 | Individual article of charge + statement of imputation | — |
| E7 | `defence_statements` | Module | G09 | Charged officer's written statement of defence | — |
| E8 | `inquiry_proceedings` | Module | G09 | The departmental inquiry instance (+ STAY, inspection) | — |
| E9 | `inquiry_appointments` | Module | G09 | IO / PO / Defence Assistant / ICC appointments | — |
| E10 | `inquiry_hearings` | Module | G09 | Daily order sheets / hearing log | — |
| E11 | `inquiry_witnesses` | Module | G09 | Listed/examined witnesses (prosecution & defence) | — |
| E12 | `inquiry_exhibits` | Module | G09 | Documentary/material evidence (vault items; relied-upon/disclosed) | — |
| E13 | `inquiry_reports` | Module | G09 | IO/ICC findings (article-wise per respondent) | — |
| E14 | `disagreement_memos` | Module | G09 | DA disagreement with IO findings | — |
| E15 | `show_cause_notices` | Module | G09 | Notice on proposed penalty | — |
| E16 | `penalty_orders` | Module | G09 | Final order (penalty/exoneration/drop/abated) + proportionality + signature | — |
| E17 | `penalty_items` | Module | G09 | Individual penalty(ies) imposed (recovery caps) | — |
| E18 | `appeals` | Module | G09 | Appeal / revision / review applications & decisions | — |
| E19 | `case_timeline_events` | Module | G09 | SLA-tracked stage events (hash-chained ledger) | — |
| E20 | `vigilance_records` | Module | G09 | Integrity register & vigilance clearance status | — |
| E21 | `case_documents` | Module | G09 (link) | Join between case artefacts and G13 `documents` (+ signature) | — |
| **E22** | `procedure_templates` | Module | G09 | Jurisdiction overlay: consultations, competence ref, timelines, service modes, limitation, dispensation conditions | ✔ |
| **E23** | `authority_competence` | Module | G09 | (cadre/level × penalty_class) → empowered authority level matrix | ✔ |
| **E24** | `case_consultations` | Module | G09 | UPSC/CVC/ICC/legal consultation records gating finalise | ✔ |
| **E25** | `inquiry_dispensations` | Module | G09 | Lawful dispense-with-inquiry (Art. 311(2) provisos) | ✔ |
| **E26** | `legal_service_records` | Module | G09 | Statutory legal service (mode, proof, served_by) distinct from notifications | ✔ |
| **E27** | `case_respondents` | Module | G09 | 1..N charged officers per proceeding (common/joint proceedings) | ✔ |
| **E28** | `sla_pause_events` | Module | G09 | Clock pause/resume ledger (stay/remit/condonation/consultation/criminal) | ✔ |
| **E29** | `personal_hearings` | Module | G09 | Personal-hearing record (show-cause / appeal stages) | ✔ |
| **E30** | `idempotency_keys` | Module | G09 | Dedup store for propagating posts (mint, TTL) | ✔ |
| — | `employees`, `org_units`, `designations`/`cadres`, `service_register_events`, `documents`, `notifications`, `audit_log`, `workflow_*` | Shared | G01/G12/G13/platform | Referenced, not redefined | — |
...
| `non_employment_certificate_received` | BOOLEAN | N | **(v2)** Precondition for subsistence payment to G10 (AI-6) |
...
| `payroll_event_id` | UUID | Y | Correlation to G10 |
...
| `side` | ENUM `witness_side` | N | PROSECUTION / DEFENCE |
| `description` | TEXT | N | |
| `document_id` | UUID FK→documents | N | Vault item (G13) |
...
| `sr_event_id` | UUID FK→service_register_events | Y | G12 correlation |
...
| `downstream_event_id` | UUID | Y | Correlation to G06/G10/G11 |
| audit fields | | | |
...
#### E21 — `case_documents` (link to G13)
...
| `document_id` | UUID FK→documents | N | G13 object |
...
```
employees (G01) 1──* disciplinary_cases *──1 org_units (G01)
...
inquiry_proceedings 1──* inquiry_exhibits *──1 documents (G13)
...
penalty_orders 1──0..1 service_register_events (G12)
disciplinary_cases 1──* case_timeline_events (hash-chained)          (v2)
disciplinary_cases 1──* sla_pause_events                            (v2)
disciplinary_cases 1──* case_documents *──1 documents (G13)
...
|--------|----------|---------|-----------|
| E1–E30 (case entities) | G09 | G09, G14 (KPI), Auditor | G09 roles per §3 |
| `procedure_templates`, `authority_competence` (E22/E23) | G09 | G09 decision guards, Auditor | SysAdmin only (reference data) |
| `employees` | G01 | G09 | G01 only |
| `org_units` / `designations` / `cadres` | G01 | G09 | G01 only |
| `service_register_events` | G12 | G09 | G09 (append SR event, idempotent), G12 |
| `documents` | G13 | G09 | G09 (via G13 API), G13 |
| `notifications` | platform | G09 | G09 (informational only — never legal service) |
| `audit_log` | platform | Auditor, G09 | G09 (append on every action). Statutory tamper-evidence = **P05 dual-log (DB-trigger) + OPEN-PLAT-03 hash-chaining**, not a G09-owned chain; E19 `case_timeline_events` is a per-case SLA/stage ledger that rides on the P05 substrate |
| `workflow_*` | platform | G09 | G09 |
| Payroll/pension/seniority effects | G10/G11/G06 | G09 (status echo) | G09 emits event; owning module executes |
| DSC/eSign trust service | platform | G09 | G09 calls; stores signature metadata |
...
| Warning / Fine | MINOR | SR entry; fine → G10 recovery (capped, DI-22) |
| Withholding of increment(s) | MINOR (MAJOR if cumulative effect) | G10 pay, G06 progression |
| Withholding of promotion | MAJOR | G06 |
| Recovery from pay of loss caused | MINOR/MAJOR | G10 recovery (≤1/3 pay; instalments; DCRG-only beyond retirement, DI-22) |
| Reduction to lower stage/rank | MAJOR | G06 seniority, G10 pay |
| Compulsory retirement | MAJOR (Art. 311(1) competence) | G11 pension (reduced) |
| Removal from service | MAJOR (Art. 311(1) competence) | G11 |
| Dismissal from service | MAJOR (Art. 311(1) competence) | G11 (disqualifies future govt employment) |
...
7. **DI-7 Evidence seal:** every `inquiry_exhibits`/`case_documents` row stores `content_hash`; mismatch on read raises `ERR-G09-EVIDENCE-TAMPERED`.
...
13. **DI-13 Penalty competence (v2, AI-1, R1):** finalising any `penalty_items` row requires `passed_by` to be competent for that penalty class/type per `authority_competence` for the respondent's cadre. For DISMISSAL/REMOVAL/CR, the authority must **not be subordinate to the appointing authority** (Art. 311(1)). Otherwise return `ERR-G09-AUTHORITY-NOT-COMPETENT` (409) and refuse persistence. `competence_verified` must be true to finalise.
14. **DI-14 Mandatory consultation (v2, AI-2, R2):** finalise is blocked until every `case_consultations` row that the case's `procedure_template` marks mandatory is `CLOSED` (or `WAIVED` with reasons). Otherwise `ERR-G09-CONSULTATION-PENDING` (409).
...
16. **DI-16 Subsistence correctness (v2, AI-6, R6):** a subsistence payment event to G10 requires `non_employment_certificate_received=true`. If a charge-memo is not validly served by `charge_memo_due_date` (90-day window), set `deemed_review_flag=true` and escalate for review/possible revocation.
...
21. **DI-21 Hash-chained audit (v2, AI-15, R15):** each `case_timeline_events` row stores `prev_hash` (prior row's `row_hash`) and a computed `row_hash`; the verify endpoint recomputes the chain and raises `ERR-G09-AUDIT-CHAIN-BROKEN` on any mismatch.
22. **DI-22 Recovery caps (v2, AI-20, R20):** for RECOVERY/FINE items, monthly deduction ≤ `recovery_monthly_cap_pct` (default 1/3 of pay), instalments bounded, and no recovery beyond retirement except from DCRG (`recovery_beyond_retirement` ⇒ DCRG source); violation `ERR-G09-RECOVERY-CAP-EXCEEDED` (422).
23. **DI-23 Dispensation guard (v2, AI-3, R3):** a penalty without an inquiry is lawful only if an `inquiry_dispensations` row is `APPROVED` with a valid `reason_code` permitted by the template and recorded speaking reasons; else `ERR-G09-DUE-PROCESS-INCOMPLETE`.
24. **DI-24 Inspection afforded (v2, AI-4, R4):** an inquiry cannot record substantive evidence (witness examination/exhibit admission) unless `list_supplied_date` and `inspection_afforded_date` are set; else `ERR-G09-INSPECTION-NOT-AFFORDED` (409).
...
27. **DI-27 Retiree proceedings (v2, AI-13, R13):** proceedings against a retired employee require `is_retiree_case=true`, a recorded `retiree_sanction_ref`, and must respect the Rule 9 **four-year** event bar; failing the bar/sanction returns `ERR-G09-RETIREE-PROCEEDING-BARRED` (409).
28. **DI-28 Digital signing (v2, AI-17, R17):** charge-sheets, inquiry reports, show-cause notices, and orders cannot reach a served/finalised state without `signature_type` ∈ {DSC, ESIGN}, a bound `signatory_id`, and `signed_at`; else `ERR-G09-SIGNATURE-REQUIRED` (422).
...
### FR-G09-001 — Complaint / Source-of-Misconduct Registration & Triage
...
- BR-4 **(v2):** Template resolution is mandatory; if no template matches jurisdiction+misconduct, `ERR-G09-PROCEDURE-TEMPLATE-INVALID` (422).
...
**Edge Cases:** Subject is a non-employee/contractor (external subject ref); subject already retired (route to retiree-proceeding path FR-026, G11 advisory); same allegation re-submitted (merge); **(v2)** DA resolves to the complainant ⇒ block with `ERR-G09-DA-BIAS-CONFLICT`; HARASSMENT routes to ICC template.
...
| Failure Handling | Dup ⇒ 409 `ERR-G09-DUPLICATE-COMPLAINT`; missing DA ⇒ 422 `ERR-G09-DA-NOT-RESOLVED`; no template ⇒ 422 `ERR-G09-PROCEDURE-TEMPLATE-INVALID`; DA is complainant ⇒ 409 `ERR-G09-DA-BIAS-CONFLICT` |
| Dependencies | G01 (employees), `procedure_templates`, `authority_competence`, notifications, audit |
| Test Guidance | Dedup; anonymity masking; template resolution; DA-bias block; triage→case+respondent txn |

---

### FR-G09-002 — Preliminary (Fact-Finding) Inquiry
...
- BR-4 **(v2):** PI material relied upon but not disclosed blocks finding (`ERR-G09-NATURAL-JUSTICE-VIOLATION`).
...
| Failure Handling | Invalid transition ⇒ 409 `ERR-G09-INVALID-STATE-TRANSITION`; missing doc ⇒ 422; relied-not-disclosed ⇒ 409 `ERR-G09-NATURAL-JUSTICE-VIOLATION` |
| Dependencies | G13 (sealed doc), notifications, SLA engine (FR-016) |
| Test Guidance | Confidentiality (charged officer cannot fetch PI report); relied-upon disclosure gate; SLA breach |

---

### FR-G09-003 — Suspension (Parallel Status), Subsistence Allowance & Review
...
**Description:** **(v2)** Suspension is modelled as a **parallel interim status** on the case (`is_under_suspension`/`suspension_status`), **not a linear stage** — the charge/inquiry track proceeds concurrently. Issues suspension orders (ordered/deemed/continued), sets subsistence within statutory bounds, **gates the subsistence payment event on a non-employment certificate (DI-16)**, schedules mandatory periodic review, **auto-flags a deemed review when the charge-memo is not served within the 90-day window (Ajay Kumar Choudhary, DI-16)**, emits a payroll event to G10, and supports revocation.

**Acceptance Criteria:**
1. A suspension order sets `effective_from`, `subsistence_rate_pct` ∈ [template floor, ceiling], sets case `is_under_suspension=true` (parallel) **(v2)**, and updates `employees.employment_status=SUSPENDED` via G01 event.
2. Deemed suspension auto-creates a record when detention > 48h is recorded.
3. **(v2)** The subsistence payment event to G10 is emitted **only** when `non_employment_certificate_received=true`; otherwise the payment is held and flagged.
...
6. Revocation sets `status=REVOKED`, `effective_to`, and emits G10 + G01 events; an SR event is appended for suspension and revocation (FR-013).
...
| Backend Flow | Validate bounds → persist suspension in txn → set parallel case flag → emit G01 status event + G12 SR event → schedule review tasks → emit G10 subsistence event **only if NEC received** → scheduled job sets `deemed_review_flag` on 90-day lapse |
| Data Operations | INSERT/UPDATE `suspensions`; UPDATE `disciplinary_cases.is_under_suspension/suspension_status`; append via the G12 ingestion contract `POST /api/v1/sr/ingest` (FR-02); INSERT `workflow_actions` |
...
| Failure Handling | Out of bounds ⇒ 422 `ERR-G09-SUBSISTENCE-OUT-OF-BOUNDS`; payment without NEC ⇒ 422 `ERR-G09-NON-EMPLOYMENT-CERT-REQUIRED`; G10/G01 down ⇒ outbox + X.3 retry (backoff ×3); on exhaustion ⇒ 500 `ERR-LOADFAIL` |
| Dependencies | G01, G10, G12, workflow engine, idempotency store |
| Test Guidance | Bound checks; NEC gate; deemed-review on lapse; parallel-flag (not stage); outbox retry; SR idempotency |

---

### FR-G09-004 — Charge-Sheet / Articles of Charge (Statement of Imputations)
...
3. **(v2)** On issue, a **DSC/eSign-signed** PDF is produced (`signature_type`, `signatory_id`, `signed_at`); without a signature the sheet cannot be served (`ERR-G09-SIGNATURE-REQUIRED`).
4. **(v2)** Service is recorded as a `legal_service_records` row; `defence_due_date` computes **only** from a `is_statutorily_valid=true` service (EMAIL non-statutory by default); invalid mode ⇒ `ERR-G09-INVALID-SERVICE-MODE`.
...
| Backend Flow | Validate articles → persist draft → issue: render PDF via G13, call DSC/eSign trust service, lock articles → serve: create `legal_service_records`, compute validity + due-date, mark served, notify |
...
| Failure Handling | Unsigned serve ⇒ 422 `ERR-G09-SIGNATURE-REQUIRED`; invalid mode ⇒ 409 `ERR-G09-INVALID-SERVICE-MODE`; PDF render fail ⇒ retain DRAFT + 500 `ERR-LOADFAIL` (X.3 retry) |
| Dependencies | G13, DSC/eSign trust service, notifications, `procedure_templates` |
| Test Guidance | Article immutability; signature gate; valid-service computation; EMAIL non-statutory block |

---

### FR-G09-005 — Employee's Written Statement of Defence (+ Rights & Deadlines Surface)
...
| Failure Handling | Submit after due-date ⇒ 409 `ERR-G09-DEFENCE-WINDOW-CLOSED` (unless extension); not validly served ⇒ 403 |
...
### FR-G09-006 — Appointment of Inquiry Officer & Presenting Officer (or ICC Constitution)
...
**Acceptance Criteria:**
1. IO, PO, DA, complainant, witnesses, and the charged officer must be mutually distinct (DI-2); violations blocked with `ERR-G09-ACTOR-CONFLICT`.
...
5. **(v2)** For `is_posh_case`, the system enforces ICC composition (≥1 `ICC_EXTERNAL_MEMBER`, a presiding officer); missing composition ⇒ `ERR-G09-ICC-PROCEDURE-REQUIRED`.
...
| Failure Handling | Conflict ⇒ 409 `ERR-G09-ACTOR-CONFLICT`; POSH missing composition ⇒ 409 `ERR-G09-ICC-PROCEDURE-REQUIRED` |
| Dependencies | G01, notifications, `procedure_templates` |
| Test Guidance | Distinct-actor incl. DA; ICC composition; objection→replacement audit chain |

---

### FR-G09-007 — Conduct of Departmental Inquiry (Hearings, Witnesses, Exhibits, Inspection, Ex-Parte, Stay)
...
**Acceptance Criteria:**
1. **(v2)** The IO records `list_supplied_date` and `inspection_afforded_date`; witness examination/exhibit admission is blocked until both are set (`ERR-G09-INSPECTION-NOT-AFFORDED`).
...
**Edge Cases:** Witness hostile; exhibit authenticity challenged (objection recorded); charged officer attends late after ex-parte (IO may recall); hash mismatch ⇒ `ERR-G09-EVIDENCE-TAMPERED`; **(v2)** evidence attempted before inspection ⇒ blocked; stay with no criminal ref ⇒ rejected.
...
| Failure Handling | Inspection missing ⇒ 409 `ERR-G09-INSPECTION-NOT-AFFORDED`; hash mismatch ⇒ 409 `ERR-G09-EVIDENCE-TAMPERED`; ex-parte without threshold ⇒ 422 |
| Dependencies | G13 (sealed vault), SLA engine, notifications |
| Test Guidance | Inspection gate; ex-parte threshold; exhibit hash + relied/disclosed; criminal stay pause |

---

### FR-G09-008 — Inquiry Report & Findings
...
| Failure Handling | Missing findings ⇒ 422 `ERR-G09-INCOMPLETE-FINDINGS`; unsigned ⇒ 422 `ERR-G09-SIGNATURE-REQUIRED`; wrong state ⇒ 409 |
| Dependencies | G13, DSC/eSign, notifications |
| Test Guidance | All-articles/all-respondents guard; signature; finding write-back; remit |

---

### FR-G09-009 — Disagreement Memo & Disciplinary Authority Consideration
...
| Failure Handling | Penalty on not-proved without memo ⇒ 409 `ERR-G09-NATURAL-JUSTICE-VIOLATION` |
| Dependencies | notifications, G13 |
| Test Guidance | Upgrade-without-memo block; representation window |

---

### FR-G09-010 — Show-Cause Notice on Proposed Penalty
...
| Failure Handling | Major order without closed show-cause ⇒ 409; enhancement ⇒ 409 `ERR-G09-PENALTY-EXCEEDS-PROPOSED`; hearing denied without reason ⇒ 422 `ERR-G09-PERSONAL-HEARING-DENIED` |
| Dependencies | notifications, personal hearings |
| Test Guidance | Subset enforcement; lapse handling; personal-hearing record |

---

### FR-G09-011 — Penalty Order & Exoneration (Competence, Consultation, Proportionality, Recovery Caps, Signing)
...
4. **(v2)** The order is signed via DSC/eSign (`signature_type`, `signatory_id`, `signed_at`, `signature_ref`) before finalise; unsigned ⇒ `ERR-G09-SIGNATURE-REQUIRED`.
...
**Edge Cases:** Multiple penalties of different classes (competence resolved to the **highest** class); recovery exceeding cap (validation); exoneration after long suspension (back-pay settlement to G10); **(v2)** employee deceased before order ⇒ abate (FR-028); incompetent DA ⇒ block.
...
| Data Operations | INSERT `penalty_orders`,`penalty_items`; UPDATE case status; append via the G12 ingestion contract `POST /api/v1/sr/ingest` (FR-02); emit G06/G10/G11 events |
...
| Failure Handling | Incomplete chain ⇒ 409 `ERR-G09-DUE-PROCESS-INCOMPLETE`; incompetent ⇒ 409 `ERR-G09-AUTHORITY-NOT-COMPETENT`; consultation pending ⇒ 409 `ERR-G09-CONSULTATION-PENDING`; no proportionality ⇒ 422 `ERR-G09-PROPORTIONALITY-REASONING-REQUIRED`; recovery cap ⇒ 422 `ERR-G09-RECOVERY-CAP-EXCEEDED`; unsigned ⇒ 422 `ERR-G09-SIGNATURE-REQUIRED`; edit after finalise ⇒ 409 `ERR-G09-ORDER-IMMUTABLE` |
| Dependencies | G06, G10, G11, G12, G13, DSC/eSign, MFA, `authority_competence`, `case_consultations` |
...
### FR-G09-012 — Appeal / Revision / Review
...
5. Decision propagates: SR supersession (FR-013) and downstream reversal/adjustment (G06/G10/G11).
...
**Edge Cases:** Time-barred without condonation (reject); enhancement without show-cause (block); multiple remedies in sequence; appeal pending while recovery deducted (hold/refund with G10).
...
| Data Operations | INSERT/UPDATE `appeals`; conditional INSERT revised `penalty_orders`; append a superseding event via the G12 ingestion contract `POST /api/v1/sr/ingest` (FR-02); INSERT `sla_pause_events`/`personal_hearings` |
...
| Failure Handling | Time-barred admit ⇒ 409 `ERR-G09-APPEAL-TIME-BARRED`; incompetent authority ⇒ 409 `ERR-G09-AUTHORITY-NOT-COMPETENT`; enhance without show-cause ⇒ 409 |
| Dependencies | G06, G10, G11, G12, MFA, personal hearings, SLA pause |
| Test Guidance | Limitation/condonation pause; set-aside reversal; enhancement guard; competence |

---

### FR-G09-013 — Service Register Posting & Downstream Effects (Idempotent)

- **Module:** G09-DCP
- **Primary Role(s):** Disciplinary Authority, DCP Admin, SR Custodian (G12)
...
**Description:** Appends an immutable SR event and emits idempotent effect events to G06/G10/G11. **(v3 re-grounding)** The SR ledger is the **net-new G12 SR engine running on the P05 audit/immutability substrate** (DB-trigger, immutable, ≥ 7-yr, OPEN-PLAT-03 hash-chain) — G09 **appends** to it via the G12 contract, it does not own the ledger. Downstream effects flow **by reference** to G06 (seniority/promotion), G10 (recovery/subsistence) and G11 (pension); the owning module executes — G09 emits events only. **(v2)** Uses the defined **`idempotency_keys` lifecycle (DI-19, AI-11)** plus the platform **`Idempotency-Key`** header — keys minted per scope, deduplicated with a TTL, guaranteeing exactly-once effects even on double-finalise / double post-to-SR; reinforced by idempotent P01 `workflow_actions`. Outbox/retry on X.3 for reliability; supersession on appeal.

**Acceptance Criteria:**
1. On order finalise (and appeal decision), an SR event is appended with order reference (DI-10).
2. Penalty-type-specific downstream events are emitted (WITHHOLD_INCREMENT/REDUCTION → G06+G10; RECOVERY → G10; CR/REMOVAL/DISMISSAL → G11; WITHHOLD_PROMOTION → G06).
3. **(v2)** Emission is idempotent via `idempotency_keys`; a replayed key returns the stored result and produces no second effect; mismatched fingerprint on a seen key ⇒ `ERR-G09-IDEMPOTENCY-CONFLICT`.
...
| Entity | Use |
|--------|-----|
| `service_register_events` | append (G12) |
...
**Edge Cases:** G12 unavailable (outbox retains, retries); duplicate trigger (key dedup); partial downstream success (per-effect status); reduction target designation invalid (validation before emit).
...
| Backend Flow | On finalise → check/mint idempotency key (TTL) → if seen+match return snapshot; else write outbox entries (SR + per-effect) → dispatcher posts to G12/G06/G10/G11 → record correlation ids → persist response snapshot |
| Data Operations | append via the G12 ingestion contract `POST /api/v1/sr/ingest` (FR-02); INSERT/UPDATE `idempotency_keys`; INSERT outbox rows; UPDATE correlations |
...
| Failure Handling | Upstream down ⇒ outbox + backoff + alert; duplicate key match ⇒ replay snapshot; key mismatch ⇒ 409 `ERR-G09-IDEMPOTENCY-CONFLICT` |
| Dependencies | G12, G06, G10, G11, idempotency store |
| Test Guidance | Double-finalise ⇒ one SR event; double post-to-SR ⇒ one effect; outbox retry; supersession; per-effect status |

---

### FR-G09-014 — Document Evidence Vault (Link to G13)
...
**Description:** Centralises all case artefacts as references to encrypted G13 objects, each with SHA-256 `content_hash`, served/sealed flags, **(v2)** signature metadata, and full read-audit. Served items are visible to the charged officer; sealed items restricted; relied-upon evidence must be disclosed (DI-9).
...
3. On retrieval, the stored `content_hash` is re-verified; mismatch raises `ERR-G09-EVIDENCE-TAMPERED` and alerts.
...
**Business Rules:**
- BR-1: Vault items never hard-deleted; supersession/versioning via G13.
...
**Edge Cases:** Large files (chunked upload via G13); hash mismatch (block + alert); cross-case shared exhibit (separate links).
...
| Backend Flow | Upload via G13 → compute hash → persist link (+ signature meta) → on download verify hash + RBAC + write view-audit |
...
| Failure Handling | G13 down ⇒ 500 `ERR-LOADFAIL` (X.3 retry); hash mismatch ⇒ 409 `ERR-G09-EVIDENCE-TAMPERED` |
| Dependencies | G13, audit |
| Test Guidance | Hash verify; served+disclosed visibility; read-audit on sealed |

---

### FR-G09-015 — Integrity Register & Vigilance Clearance (+ Sealed Cover)
...
**Description:** Maintains a per-employee vigilance record (clearance status, integrity grade, **(v2)** sealed-cover flag) driven by case lifecycle, and exposes a clearance API consumed by G06/G11. **(v2)** When proceedings are pending, G06 promotion recommendations are held in **sealed cover** (AI-13).
...
5. **(v2)** `sealed_cover_flag` is set when proceedings pending and surfaced to G06.
...
- BR-2: `NOT_CLEAR`/`UNDER_PROCEEDINGS`/sealed-cover blocks promotion in G06.
...
**Edge Cases:** Multiple concurrent cases (most-restrictive wins); clearance during pending appeal (UNDER_PROCEEDINGS retained); retiring employee clearance for pension (G11 consumes).
...
| Authorization | Read: RBAC (G06/G11 service + Vigilance/DA); write: Vigilance Officer |
| State Changes & Side Effects | Status + sealed-cover drive G06 promotion gate, G11 pension clearance |
| Failure Handling | Concurrent updates ⇒ most-restrictive resolver; 409 on stale write |
| Dependencies | G06, G11, audit |
| Test Guidance | Event-driven derivation; most-restrictive; sealed-cover; override audit |

---

### FR-G09-016 — SLA / Statutory-Timeline Tracking, Proportionality Analytics & Case Analytics
...
| Authorization | Detail: DA/Admin/Vigilance; aggregates: Auditor; G14 service-account |
| State Changes & Side Effects | sla_status updates; escalation notifications; G14 feed |
| Failure Handling | Evaluator failure ⇒ retry + alert; stale read-model ⇒ refresh |
| Dependencies | scheduler, notifications, G14, `sla_pause_events`, `procedure_templates` |
| Test Guidance | Target computation; at-risk/breach with pause skip; ageing; proportionality outlier correctness |

---

### FR-G09-017 — Procedure Template & Jurisdiction Overlay (Two-Layer Model) **(v2, AI-8, R8)**
...
**Edge Cases:** No matching template at case creation (`ERR-G09-PROCEDURE-TEMPLATE-INVALID`); template edited mid-case (new cases only; open cases keep pinned version); conflicting active selectors (validation blocks activation).
...
| Failure Handling | No match ⇒ 422 `ERR-G09-PROCEDURE-TEMPLATE-INVALID`; kernel-disable attempt ⇒ 422 `VALIDATION_FAILED` |
...
### FR-G09-018 — Authority Competence Matrix & Finalise-Time Competence Guard **(v2, AI-1, R1)**
...
3. **A `DISMISSAL` finalised by an authority subordinate to the appointing authority returns `ERR-G09-AUTHORITY-NOT-COMPETENT` (409) and cannot persist** (the canonical failing-then-passing test).
...
**Edge Cases:** Cadre changes by promotion mid-case (snapshot used); no matrix entry for cadre/class (`ERR-G09-AUTHORITY-NOT-COMPETENT`, fail-safe deny); delegated authority (modelled as authority level).
...
| Failure Handling | 409 `ERR-G09-AUTHORITY-NOT-COMPETENT`; fail-safe deny on missing entry |
| Dependencies | G01 (appointing authority resolution), `procedure_templates` |
| Test Guidance | **Canonical:** DISMISSAL by subordinate authority blocked; highest-class resolution; missing-entry deny |

---

### FR-G09-019 — Statutory Consultation Management (UPSC / CVC / ICC / Legal) **(v2, AI-2, R2)**
...
3. Finalise is blocked with `ERR-G09-CONSULTATION-PENDING` until all mandatory consultations are CLOSED/WAIVED.
...
| Failure Handling | Finalise with pending ⇒ 409 `ERR-G09-CONSULTATION-PENDING` |
| Dependencies | `procedure_templates`, FR-024, G13 |
| Test Guidance | Required-set derivation; finalise gate; pause; relied-advice disclosure |

---

### FR-G09-020 — Dispense-with-Inquiry (Article 311(2) Provisos) **(v2, AI-3, R3)**
...
| Failure Handling | Invalid reason ⇒ 422 `ERR-G09-INQUIRY-DISPENSATION-INVALID`; penalty without approved dispensation/inquiry ⇒ 409 `ERR-G09-DUE-PROCESS-INCOMPLETE` |
...
### FR-G09-021 — Legal Service of Documents & Digital Signing (DSC/eSign) **(v2, AI-5, AI-17, R5, R17)**
...
- BR-3: Service proof is stored as an G13 object with hash.
...
| Failure Handling | Invalid mode ⇒ 409 `ERR-G09-INVALID-SERVICE-MODE`; unsigned ⇒ 422 `ERR-G09-SIGNATURE-REQUIRED`; verify fail ⇒ 409 `ERR-G09-SIGNATURE-INVALID` |
| Dependencies | DSC/eSign trust service, G13, `procedure_templates` |
| Test Guidance | Valid-service window computation; EMAIL non-statutory; signing gate; verify |

---

### FR-G09-022 — Common / Joint (Multi-Respondent) Proceedings **(v2, AI-7, R7)**
...
| Failure Handling | Close with unresolved respondent ⇒ 409 `ERR-G09-RESPONDENT-UNRESOLVED`; conflicting actor ⇒ 409 `ERR-G09-ACTOR-CONFLICT` |
...
### FR-G09-023 — POSH / Internal Committee Procedure **(v2, AI-16, R16)**
...
2. ICC constitution enforces composition (presiding officer + ≥1 external member); missing ⇒ `ERR-G09-ICC-PROCEDURE-REQUIRED`.
...
| Failure Handling | Missing composition ⇒ 409 `ERR-G09-ICC-PROCEDURE-REQUIRED` |
...
### FR-G09-024 — SLA Pause/Resume & Clock Management **(v2, AI-10, AI-18, R10, R18)**
...
| Failure Handling | Resume without pause ⇒ 409 `ERR-G09-SLA-PAUSE-INVALID` |
| Dependencies | FR-016, FR-007/008/012/019 |
| Test Guidance | No false breach while paused; recompute on resume; coalesced overlaps |

---

### FR-G09-025 — Personal Hearing Record (Show-Cause / Appeal) **(v2, AI-21)**
...
| Failure Handling | Deny without reason ⇒ 422 `ERR-G09-PERSONAL-HEARING-DENIED` |
| Dependencies | FR-010, FR-012 |
| Test Guidance | Grant/deny reasons; minutes; linkage |

---

### FR-G09-026 — Mid-Proceeding Jurisdiction Transfer, Sealed Cover & Retiree Proceedings **(v2, AI-13, R13)**
...
**Description:** Handles employee-lifecycle races against long proceedings: a **jurisdiction-transfer** event re-resolves the competent DA (from G05/G06 changes) and records `JURISDICTION_TRANSFER` in the timeline; **sealed cover** freezes G06 promotion recommendations while proceedings are pending; **retiree proceedings** enforce the CCS (Pension) Rule 9 **four-year event bar** plus required government/President sanction (DI-27).
...
2. While proceedings are pending, G06 promotion is held in **sealed cover** (`vigilance_records.sealed_cover_flag`, `disciplinary_cases.is_sealed_cover`); exoneration lifts it, penalty resolves it per rules.
3. Retiree proceedings require `is_retiree_case=true`, a recorded `retiree_sanction_ref`, and that the misconduct event be within the Rule 9 four-year bar; failing ⇒ `ERR-G09-RETIREE-PROCEEDING-BARRED`.
...
| `vigilance_records` | sealed-cover flag (G06) |
...
**UI Behavior Notes:** Jurisdiction-transfer banner with old/new DA; sealed-cover badge feeding G06; retiree-proceeding panel with sanction ref and four-year-bar check.
...
| Backend Flow | On G05/G06 event → re-resolve DA → record event; set sealed cover while pending; on retiree mark, enforce Rule 9 + sanction |
...
| Failure Handling | Barred retiree ⇒ 409 `ERR-G09-RETIREE-PROCEEDING-BARRED`; missing sanction ⇒ 422 |
| Dependencies | G05, G06, G11, FR-015, FR-018 |
| Test Guidance | DA re-resolution; sealed-cover gate; Rule 9 bar + sanction |

---

### FR-G09-027 — Court-Grade Hash-Chained Audit & Verification **(v2, AI-15, R15)**
...
**Description (v3 re-grounded):** Tamper-evidence is **not** an invented parallel audit engine. The authoritative immutable record is the **platform P05 dual log** (`audit_log` + `security_audit_log`) captured by **DB trigger** on every mutation (immutable, PII-masked, ≥ 7-yr, 100% capture); **statutory-grade hash-chaining is the platform's OPEN-PLAT-03 mechanism** (periodic hash-chaining of audit partitions, chain head exported to WORM) which G09 **tracks and consumes rather than re-implements**. The G09 `case_timeline_events` ledger is a **domain SLA/stage ledger** layered on P05: each row stores `seq_no`, `prev_hash` (prior row's `row_hash`) and a computed `row_hash` (DI-21) to give per-case verifiability for court production, and the **verify endpoint** recomputes the chain and cross-checks against P05/OPEN-PLAT-03. Where v2 implied G09 owned the cryptographic chain, v3 re-anchors it onto the platform substrate.
...
4. Any DB-layer mutation of a row is detected on verify (`ERR-G09-AUDIT-CHAIN-BROKEN`).
...
| Failure Handling | Mismatch ⇒ 409 `ERR-G09-AUDIT-CHAIN-BROKEN` with first broken seq |
| Dependencies | platform audit |
| Test Guidance | Chain continuity; tamper detection; concurrent insert ordering |

---

### FR-G09-028 — Abatement on Death / Terminal Handling **(v2, AI-14, R14)**
...
- BR-3: Abatement does not by itself decide pension — G11 applies its own rules.
...
| Data Operations | UPDATE `disciplinary_cases`/`case_respondents`; INSERT `penalty_orders`(ABATED); append the abatement event via the G12 ingestion contract `POST /api/v1/sr/ingest` (FR-02) |
...
| Failure Handling | Penalty finalise on abated ⇒ 409 `ERR-G09-CASE-ABATED` |
| Dependencies | G01, G11, FR-013, FR-016 |
...
{
  "error": { "code": "ERR-G09-AUTHORITY-NOT-COMPETENT", "message": "Signing authority is subordinate to the appointing authority for a DISMISSAL penalty (Article 311(1)).", "field": "passed_by", "details": { "penaltyType": "DISMISSAL", "requiredLevel": "APPOINTING_AUTHORITY" } }
}
```
...
Module error codes adopt the **`ERR-G09-*` namespace** (Foundation §8.3 `ERR-<gov>-*` convention; registered in the Foundation §5 catalogue). Each `ERR-G09-*` id **maps onto exactly one of the platform 8 standard HTTP codes** (`VALIDATION_FAILED 422`, `UNAUTHENTICATED 401`, `FORBIDDEN 403`, `NOT_FOUND 404`, `CONFLICT 409`, `PRECONDITION_FAILED 412`, `RATE_LIMITED 429`, `INTERNAL 500`) and carries a `MSG-G09-*` user-message id. No bare `UPPER_SNAKE` module codes, and no invented `VALIDATION_ERROR`/`UPSTREAM_UNAVAILABLE`/`503`, appear in live sections. `ERR-G09-SIGNATURE-INVALID` is namespaced to resolve the legacy cross-module bare `SIGNATURE_INVALID` collision (R4 Finding 3); `ERR-G09-IDEMPOTENCY-CONFLICT` specialises the platform `CONFLICT 409` / `ERR-DUP-INSTANCE` semantics (R4 Finding 6).

| `ERR-G09-*` code | HTTP (standard) | User message id | Meaning | New v2 |
| --- | --- | --- | --- | :--: |
| `ERR-G09-DUPLICATE-COMPLAINT` | 409 CONFLICT | `MSG-G09-DUPLICATE-COMPLAINT` | Open complaint/case already exists for subject | |
| `ERR-G09-DA-NOT-RESOLVED` | 422 VALIDATION_FAILED | `MSG-G09-DA-NOT-RESOLVED` | Competent disciplinary authority not resolvable | |
| `ERR-G09-DA-BIAS-CONFLICT` | 409 CONFLICT | `MSG-G09-DA-BIAS-CONFLICT` | DA is also complainant/witness (DI-2) | ✔ |
| `ERR-G09-INVALID-STATE-TRANSITION` | 409 CONFLICT | `MSG-G09-INVALID-STATE-TRANSITION` | Stage/status transition not allowed | |
| `ERR-G09-ACTOR-CONFLICT` | 409 CONFLICT | `MSG-G09-ACTOR-CONFLICT` | DA/IO/PO/witness not mutually distinct | |
| `ERR-G09-DEFENCE-WINDOW-CLOSED` | 409 CONFLICT | `MSG-G09-DEFENCE-WINDOW-CLOSED` | Defence submitted after due date without extension | |
| `ERR-G09-INSPECTION-NOT-AFFORDED` | 409 CONFLICT | `MSG-G09-INSPECTION-NOT-AFFORDED` | Evidence recorded before document/witness inspection (DI-24) | ✔ |
| `ERR-G09-INCOMPLETE-FINDINGS` | 422 VALIDATION_FAILED | `MSG-G09-INCOMPLETE-FINDINGS` | Inquiry report missing an article/respondent finding | |
| `ERR-G09-NATURAL-JUSTICE-VIOLATION` | 409 CONFLICT | `MSG-G09-NATURAL-JUSTICE-VIOLATION` | Penalty on not-proved without memo; or relied-not-disclosed material (DI-9) | |
| `ERR-G09-PENALTY-EXCEEDS-PROPOSED` | 409 CONFLICT | `MSG-G09-PENALTY-EXCEEDS-PROPOSED` | Order penalty not subset of show-cause (DI-4) | |
| `ERR-G09-DUE-PROCESS-INCOMPLETE` | 409 CONFLICT | `MSG-G09-DUE-PROCESS-INCOMPLETE` | Required prior stage missing / no lawful dispensation (DI-3/DI-23) | |
| `ERR-G09-AUTHORITY-NOT-COMPETENT` | 409 CONFLICT | `MSG-G09-AUTHORITY-NOT-COMPETENT` | Signing authority not competent for penalty class/type (DI-13, Art. 311(1)) | ✔ |
| `ERR-G09-CONSULTATION-PENDING` | 409 CONFLICT | `MSG-G09-CONSULTATION-PENDING` | Mandatory UPSC/CVC/ICC consultation not closed (DI-14) | ✔ |
| `ERR-G09-INQUIRY-DISPENSATION-INVALID` | 422 VALIDATION_FAILED | `MSG-G09-INQUIRY-DISPENSATION-INVALID` | Dispensation reason not permitted by template (DI-23) | ✔ |
| `ERR-G09-INVALID-SERVICE-MODE` | 409 CONFLICT | `MSG-G09-INVALID-SERVICE-MODE` | Service mode not statutorily valid per template (DI-15) | ✔ |
| `ERR-G09-SIGNATURE-REQUIRED` | 422 VALIDATION_FAILED | `MSG-G09-SIGNATURE-REQUIRED` | Artefact lacks DSC/eSign (DI-28) | ✔ |
| `ERR-G09-SIGNATURE-INVALID` | 409 CONFLICT | `MSG-G09-SIGNATURE-INVALID` | Signature verification failed (namespaced; resolves collision, R4 Finding 3) | ✔ |
| `ERR-G09-NON-EMPLOYMENT-CERT-REQUIRED` | 422 VALIDATION_FAILED | `MSG-G09-NON-EMPLOYMENT-CERT-REQUIRED` | Subsistence payment without NEC (DI-16) | ✔ |
| `ERR-G09-PROPORTIONALITY-REASONING-REQUIRED` | 422 VALIDATION_FAILED | `MSG-G09-PROPORTIONALITY-REASONING-REQUIRED` | Order finalise without proportionality reasoning (DI-20) | ✔ |
| `ERR-G09-RECOVERY-CAP-EXCEEDED` | 422 VALIDATION_FAILED | `MSG-G09-RECOVERY-CAP-EXCEEDED` | Recovery beyond cap/instalment/retirement rule (DI-22) | ✔ |
| `ERR-G09-ICC-PROCEDURE-REQUIRED` | 409 CONFLICT | `MSG-G09-ICC-PROCEDURE-REQUIRED` | POSH case missing ICC composition (AI-16) | ✔ |
| `ERR-G09-RESPONDENT-UNRESOLVED` | 409 CONFLICT | `MSG-G09-RESPONDENT-UNRESOLVED` | Case close with an unresolved active respondent (DI-25) | ✔ |
| `ERR-G09-RETIREE-PROCEEDING-BARRED` | 409 CONFLICT | `MSG-G09-RETIREE-PROCEEDING-BARRED` | Retiree proceeding beyond four-year bar or without sanction (DI-27) | ✔ |
| `ERR-G09-CASE-ABATED` | 409 CONFLICT | `MSG-G09-CASE-ABATED` | Penalty finalise on an abated case/respondent (DI-26) | ✔ |
| `ERR-G09-SLA-PAUSE-INVALID` | 409 CONFLICT | `MSG-G09-SLA-PAUSE-INVALID` | Resume without an open pause (DI-18) | ✔ |
| `ERR-G09-IDEMPOTENCY-CONFLICT` | 409 CONFLICT | `MSG-G09-IDEMPOTENCY-CONFLICT` | Known idempotency key with mismatched fingerprint — specialises `ERR-DUP-INSTANCE` (DI-19) | ✔ |
| `ERR-G09-AUDIT-CHAIN-BROKEN` | 409 CONFLICT | `MSG-G09-AUDIT-CHAIN-BROKEN` | Per-case timeline-chain verification failed against P05/OPEN-PLAT-03 (DI-21) | ✔ |
| `ERR-G09-PROCEDURE-TEMPLATE-INVALID` | 422 VALIDATION_FAILED | `MSG-G09-PROCEDURE-TEMPLATE-INVALID` | No/ambiguous procedure template (DI-17) | ✔ |
| `ERR-G09-ORDER-IMMUTABLE` | 409 CONFLICT | `MSG-G09-ORDER-IMMUTABLE` | Edit attempted on finalised order (DI-6) | |
| `ERR-G09-APPEAL-TIME-BARRED` | 409 CONFLICT | `MSG-G09-APPEAL-TIME-BARRED` | Appeal beyond limitation without condonation (DI-5) | |
| `ERR-G09-EVIDENCE-TAMPERED` | 409 CONFLICT | `MSG-G09-EVIDENCE-TAMPERED` | content_hash mismatch (DI-7) | |
| `ERR-G09-SUBSISTENCE-OUT-OF-BOUNDS` | 422 VALIDATION_FAILED | `MSG-G09-SUBSISTENCE-OUT-OF-BOUNDS` | Subsistence rate outside template bounds (DI-8) | |
| `ERR-G09-PERSONAL-HEARING-DENIED` | 422 VALIDATION_FAILED | `MSG-G09-PERSONAL-HEARING-DENIED` | Requested personal hearing denied without reasons (DI-29) | ✔ |
| `ERR-G09-CONFIDENTIALITY-DENIED` | 403 FORBIDDEN | `MSG-G09-CONFIDENTIALITY-DENIED` | Attempt to access sealed/confidential artefact | |
...
// 409 Conflict
{
  "error": { "code": "ERR-G09-AUTHORITY-NOT-COMPETENT", "message": "A DISMISSAL must be passed by an authority not subordinate to the appointing authority (Article 311(1)).", "field": "passed_by" }
}
```
...
// 409 Conflict
{ "error": { "code": "ERR-G09-INVALID-SERVICE-MODE", "message": "EMAIL is not a statutorily valid service mode for template CCS_CCA_2026.", "field": "serviceMode" } }
```
...
```json
// 409 Conflict
{ "error": { "code": "ERR-G09-CONSULTATION-PENDING", "message": "UPSC consultation must be CLOSED before finalising a major penalty.", "field": "consultations" } }
```
...
|--------|---------|-----------------|-------------|
| G12 SR | order finalise, suspension, appeal, **abatement** | event_type, employee_id, order_ref, effective_date | append-only, **idempotent (key)** |
| G10 PAY | suspension (**NEC-gated**), recovery (**capped**), reduction, withhold-increment | employee_id, effect_type, amount/%/effective_date | outbox + retry, idempotent |
| G11 PEN | CR/removal/dismissal, pension cut, **retiree** | employee_id, effect_type, value | outbox + retry |
| G06 PPP | reduction in rank, withhold promotion, **sealed cover** | employee_id, designation/seniority effect, sealed_cover_flag | outbox + retry |
| G05 TRJ | transfer (inbound) | employee_id, new org_unit ⇒ **jurisdiction transfer** | event consume |
| G13 DMS | all artefacts (**signed**) | object upload, hash, signature metadata | synchronous + retry |
| DSC/eSign trust service | sign/verify | artefact hash, signatory, timestamp | synchronous |
| G14 DAS | analytics incl. **proportionality outliers** | KPI read feed | read-only |
...
| Observability | Structured logs (no PII values, per §0.5), metrics on SLA breaches/pauses, competence/consultation blocks, propagation failures; traces correlated by **`X-Correlation-Id`**; `JOB-G09-*` run audit rows |
...
| **Sealed cover set/lifted** | DA, Vigilance, G06 | in-app | Promotion sealed cover {status} |
...
|--------------|-------------|-----------|
| Caseload & status mix | Open/closed by status (incl. ABATED), case_type, org_unit | DA, Admin, G14 |
...
| Penalty mix | Distribution by penalty_type/class | DA, G14 |
| **Penalty proportionality outliers (v2)** | Orders flagged where penalty severity is unusual vs misconduct/precedent (AI-12) | DA, Auditor, G14 |
| Exoneration rate | Exonerated ÷ concluded | Auditor, G14 |
| Appeal overturn rate | Set-aside+modified ÷ appeals decided | Auditor, G14 |
| Average cycle time | Initiation → closure (pause-adjusted) | DA, G14 |
...
| Vigilance clearance dashboard | Clearance status distribution; doubtful-integrity & **sealed-cover** lists | Vigilance, G06/G11 |
...
| M-3 | Migrate open cases with pinned template + current stage + key artefacts (scanned to G13 with hashes **and signature metadata where available**); closed cases as historical |
...
| M-5 | Link suspensions/penalties to existing G10/G11/G06 effects without re-triggering (migration flag suppresses re-emit; idempotency keys seeded) |
...
- **Build order honours the Council "One Thing To Do First":** seed `authority_competence` + `procedure_templates` and prove the canonical failing-then-passing test (`DISMISSAL` by subordinate authority ⇒ `ERR-G09-AUTHORITY-NOT-COMPETENT`) before any decision logic ships.
- Dual-run period: new intake on G09; legacy frozen read-only.
...
- Integration smoke tests with G05/G06/G10/G11/G12/G13 + DSC/eSign trust service green.
...
|----|----------|----------|--------------|--------------|
| FR-G09-001 | case_complaints, disciplinary_cases, case_respondents, procedure_templates | /complaints, /triage | G01, templates, competence, audit | §12.1 |
| FR-G09-002 | preliminary_inquiries, inquiry_exhibits | /preliminary-inquiries | G13, SLA | §12.1 |
| FR-G09-003 | suspensions | /suspensions, /non-employment-certificate | G01, G10, G12 | §12.6/12.7 |
| FR-G09-004 | charge_sheets, charge_articles, legal_service_records | /charge-sheets | G13, DSC/eSign, templates | §12.2 |
| FR-G09-005 | defence_statements | /defence, /my-rights | scheduler | §12.1 |
| FR-G09-006 | inquiry_appointments, inquiry_proceedings | /appointments, /icc | G01, templates | §12.3 |
| FR-G09-007 | inquiry_hearings, inquiry_witnesses, inquiry_exhibits, sla_pause_events | /hearings,/witnesses,/exhibits,/supply-list,/stay | G13, SLA | §12.3 |
| FR-G09-008 | inquiry_reports, case_respondents | /report | G13, DSC/eSign | §12.3 |
| FR-G09-009 | disagreement_memos, legal_service_records | /disagreement-memo | notifications | §12.1 |
| FR-G09-010 | show_cause_notices, personal_hearings | /show-cause | notifications | §12.1 |
| FR-G09-011 | penalty_orders, penalty_items, authority_competence, case_consultations | /orders,/sign,/finalise | G06/G10/G11/G12/G13, MFA, DSC/eSign | §12.4 |
| FR-G09-012 | appeals, personal_hearings, sla_pause_events | /appeals | G06/G10/G11/G12 | §12.5 |
| FR-G09-013 | service_register_events, idempotency_keys, penalty_orders | /post-to-sr | G12/G06/G10/G11 | §12.4 |
| FR-G09-014 | case_documents, inquiry_exhibits | /documents | G13 | — |
| FR-G09-015 | vigilance_records | /vigilance | G06/G11 | — |
| FR-G09-016 | case_timeline_events, sla_pause_events | /timeline,/analytics | scheduler, G14 | all |
| FR-G09-017 | procedure_templates, authority_competence | /admin/procedure-templates | — | §12.1 |
| FR-G09-018 | authority_competence, penalty_orders, case_respondents | /admin/authority-competence,/competence-check | G01 | §12.4 |
| FR-G09-019 | case_consultations, sla_pause_events | /consultations | templates, G13 | §12.9 |
| FR-G09-020 | inquiry_dispensations, inquiry_proceedings | /dispensation | templates | §12.1 |
| FR-G09-021 | legal_service_records, case_documents, penalty_orders | /legal-service,/sign,/verify | DSC/eSign, G13, templates | §12.2/12.4 |
| FR-G09-022 | case_respondents, inquiry_reports, penalty_orders | /respondents | FR-007/008/011 | §12.1 |
| FR-G09-023 | inquiry_appointments, inquiry_proceedings, procedure_templates | /icc,/conciliation | FR-006/007/008/011 | §12.3 |
| FR-G09-024 | sla_pause_events, case_timeline_events | /sla/pause,/sla/resume | FR-016 | §12.8 |
| FR-G09-025 | personal_hearings, show_cause_notices, appeals | /personal-hearings | FR-010/012 | — |
| FR-G09-026 | disciplinary_cases, case_respondents, vigilance_records | /jurisdiction-transfer,/sealed-cover,/retiree-proceeding | G05/G06/G11 | §12.1 |
| FR-G09-027 | case_timeline_events, audit_log | /audit/verify,/audit/chain | platform audit | — |
| FR-G09-028 | disciplinary_cases, case_respondents, penalty_orders, service_register_events | /abate | G01/G11/G12 | §12.1/12.4 |
...
| Downstream integration (G05/G06/G10/G11/G12/G13/G14 + DSC/eSign) | mapped | yes | 0 |
...
| **Platform: SR ledger = G12 on P05 substrate** (append-only, idempotent) | required | FR-013 / §10.5 | 0 |
| **Platform: notifications on X.2** (EMAIL mandatory for statutory notices) | required | §13 / §2.2 / `MSG-G09-*` | 0 |
| **Platform: background jobs on X.1** (`JOB-G09-SLA/TIMELINE/EFFDATE` for SLA pause/resume) | required | §4 / §2.2; Foundation §4 index | 0 |
...
| **Platform: migration on P06** (ETL+V, 3 dry runs, `<gov>_source_id`) | required | §4 / §15 | 0 |
| **Platform: G-code scheme (G09) + cross-module references re-keyed** | required | applied throughout | 0 |
...
This module is **NET-NEW (gov-specific)** statutory due-process (`MODULE_RECONCILIATION.md` §A row G09, §D): the case/charge/inquiry/penalty/appeal **business logic** is authored here, but it **runs on platform engines** and consumes platform contracts by id — it is not a parallel platform. Per `PLATFORM_FOUNDATION.md` §9, government BRDs author only the net-new statutory engine and run it on P01/P05/P06.

### FR → platform-service map

| FR | What it authors (gov-specific) | Runs on / consumes (platform) |
|----|--------------------------------|-------------------------------|
| FR-G09-001 Complaint & triage | complaint intake, template/DA resolution | **P02** (auth/scope), **P05** (audit), `VAL-G09-*`, X.2 notify |
| FR-G09-002 Preliminary inquiry | PI workflow + relied-material disclosure | **P01** (PI flow + SLA), **P05**, **G13** (sealed report) |
| FR-G09-003 Suspension & subsistence | suspension status, NEC gate, deemed review | **P01** (order flow), **G10** subsistence event (NEC-gated), **G12** SR event, **X.1** review jobs |
| FR-G09-004 Charge-sheet | articles, track, valid service | **P01**, **G13** (signed PDF), DSC/eSign via **X.3**, `legal_service_records` |
| FR-G09-005 Defence & rights surface | statement of defence, rights/deadlines | **P02**, X.2, **X.1** deadline jobs |
| FR-G09-006 IO/PO / ICC appointment | appointments with SoD | **P01** (DYNAMIC_APPROVER), **P02** (distinct-actor enforcement) |
| FR-G09-007 Inquiry conduct | hearings, witnesses, exhibits, stay, inspection | **P01** (inquiry stage + STAYED + SLA pause via `JOB-G09-SLA/TIMELINE`), **G13** vault, **P05** |
| FR-G09-008 Inquiry report | per-respondent findings, signing | **P01**, **G13**, DSC/eSign (**X.3**) |
| FR-G09-009 Disagreement memo | DA disagreement + representation | **P01**, X.2, `legal_service_records` |
| FR-G09-010 Show-cause + personal hearing | proposed-penalty notice, hearing record | **P01**, X.2 |
| FR-G09-011 Penalty order & exoneration | competence/consultation/proportionality/recovery-cap guards, signing | **P01** (stage-entry guards + finalise + MFA), **P05**, **G12** SR append, effects to **G06/G10/G11**, DSC/eSign (**X.3**) |
| FR-G09-012 Appeal / revision / review | remedy flow, limitation, competence | **P01** (appeal flow; authority ≠ DA), **P02**, **G12** supersession |
| FR-G09-013 SR posting & downstream effects | exactly-once propagation | **G12 SR ledger on P05 substrate**; effects by reference to **G06/G10/G11**; `Idempotency-Key` + idempotent `workflow_actions`; X.3 outbox |
| FR-G09-014 Evidence vault | artefact ↔ document links, signatures | **G13** document vault, **P05** read-audit |
| FR-G09-015 Vigilance & sealed cover | integrity register, clearance, sealed cover | **P02** (confidential roles), feeds **G06** promotion freeze |
| FR-G09-016 SLA / timeline / analytics | stage SLA, pause-aware analytics | **P01** SLA runtime + **X.1** `JOB-G09-SLA/TIMELINE`, **G14** KPI feed |
| FR-G09-017 Procedure-template overlay | two-layer overlay config | **W.1/W.2** flow/form definitions; SysAdmin = Org Admin |
| FR-G09-018 Authority-competence guard | competence matrix + finalise guard | **P01** stage guard, **P02** |
| FR-G09-019 Statutory consultation | UPSC/CVC/ICC/legal gating | **P01** (CONDITIONAL stage), **X.1** pause job, **G13** advice |
| FR-G09-020 Dispense-with-inquiry | Art. 311(2) provisos | **P01** (approval), **P05** speaking reasons |
| FR-G09-021 Legal service & digital signing | valid service vs notification; DSC/eSign | **X.3** trust service, **G13**, `legal_service_records`; X.2 informational only |
| FR-G09-022 Multi-respondent proceedings | joint proceedings, per-respondent findings | **P01** (PARALLEL patterns), **P05** |
| FR-G09-023 POSH / ICC procedure | ICC composition, conciliation | **P01** ICC flow, **P02** heightened confidentiality |
| FR-G09-024 SLA pause/resume | clock management ledger | **X.1** `JOB-G09-TIMELINE` recompute, **P01** SLA runtime |
| FR-G09-025 Personal-hearing record | grant/denial record | **P01**, **P05** |
| FR-G09-026 Jurisdiction transfer / sealed cover / retiree | DA re-resolution, Rule 9 bar | consumes **G05** transfer event, feeds **G06/G11**, **P01** |
| FR-G09-027 Tamper-evident audit & verify | court-grade verifiability | **P05 dual log** (DB-trigger, immutable, 7-yr) + **OPEN-PLAT-03** hash-chain (consumed, not re-built) |
| FR-G09-028 Abatement on death | terminal handling, suppress effects | **P01** (terminal), **G12** ABATED SR event, **P05** |

### `GAP (gov-specific)` engine authored

G09 authors **one** net-new statutory engine — the **disciplinary due-process machine** (case → charge → inquiry → penalty → appeal with natural-justice kernel, Art. 311 competence, mandatory consultation, lawful service, DSC/eSign) — plus module-unique `VAL-G09-*` rules, `JOB-G09-*` jobs, `MSG-G09-*` notifications and `ERR-G09-*` error copy, all registered against the Foundation indexes. It authors **no** platform plumbing: workflow, RBAC, audit, notifications, jobs, migration and configured-content engines are consumed by id. The SR ledger it writes to is owned by **G12**; payroll/pension/seniority effects are owned by **G10/G11/G06**.

### New / renamed `VAL-`, `JOB-`, `MSG-`, `ERR-` ids (registered in Foundation indexes)

| Family | Ids (illustrative) | Purpose |
|--------|--------------------|---------|
| `VAL-G09-*` | `VAL-G09-DUEPROCESS`, `VAL-G09-COMPETENCE`, `VAL-G09-SERVICE`, `VAL-G09-RECOVERYCAP` | charge→reply→inquiry→penalty sequencing; competence; valid service; recovery caps |
| `JOB-G09-*` | `JOB-G09-SLA`, `JOB-G09-TIMELINE`, `JOB-G09-EFFDATE` | stage SLA escalation; statutory-timeline pause/resume recompute; effective-dated penalty effects |
| `MSG-G09-*` | `MSG-G09-CHARGE-ISSUED`, `MSG-G09-SHOWCAUSE`, `MSG-G09-ORDER-SERVED`, `MSG-G09-SLA-BREACH` | statutory notices (EMAIL mandatory per X.2/§9.9) |
| `ERR-G09-*` | the §10.3 module codes (e.g. `ERR-G09-AUTHORITY-NOT-COMPETENT`, `ERR-G09-CONSULTATION-PENDING`) | domain refinements returned alongside the platform 8-code table |
...
|---|------|-----------------------------------|------------------------|-----------|
| A-1 | Module code | `M09-DCP` | **`G09`** (cross-module refs re-keyed `M01/05/06/10/11/12/13/14 → G0x`) | `MODULE_RECONCILIATION.md` §B |
...
| A-7 | SR ledger | `service_register_events` as "shared platform entity" | **net-new G12 SR ledger on the P05 substrate**; G09 appends, effects by reference to G06/G10/G11 | §C; §D |
...
| A-12 | Notifications | shared `notifications` (in-app/email/SMS) | **X.2** infra; EMAIL for statutory notices **mandatory/non-suppressible**; `MSG-G09-*` templates | Platform §X.2; BRD §9.9 |
| A-13 | Background jobs / SLA | implicit SLA engine | **X.1** runner + `JOB-G09-SLA/TIMELINE/EFFDATE`; P01 SLA runtime for stages | Platform §X.1; Foundation §4 |
| A-14 | Validation | inline rules | cite `VAL-*` by id; author `VAL-G09-*` only | Foundation §2 |
| A-15 | NFR | 99.9% uptime, RPO ≤ 15 min | **99.5%/month, RPO < 1 h, RTO < 4 h**, p95 < 500 ms, WCAG 2.1 AA, soft-delete only | Vision §2.9; §C |
| A-16 | Migration | undefined | **P06** ETL+V, 3 dry runs, waves, `<gov>_source_id` | Platform §P06 |
...
> Cross-module remediation per `docs/review/REMEDIATION.md` (D3/D4) and findings R4/R5. v3.1 preserves all v3 content, entities, FRs, integrity rules and rigor; it only aligns identifier hygiene, removes invented-convention leaks from live sections, and re-attributes the statutory tamper-evidence mechanism owner. **G09 SR event types/envelope are unchanged** (already conformant per D2).

| # | Decision | v3 (before) | v3.1 (after) | Authority |
| --- | --- | --- | --- | --- |
| B-1 | Error-code namespace (D3) | ~33 bare `UPPER_SNAKE` module codes in live §10.3 catalog + FR failure-handling tables (e.g. `AUTHORITY_NOT_COMPETENT`, `SIGNATURE_INVALID`, `IDEMPOTENCY_CONFLICT`) | **`ERR-G09-*` namespace** for every module code, each mapped onto one of the 8 standard HTTP codes with a `MSG-G09-*` message id (§10.3 mapping table) | REMEDIATION §D3; R4 Finding 1 |
| B-2 | `SIGNATURE_INVALID` collision (D3) | bare `SIGNATURE_INVALID` shared with G12/G13 | namespaced **`ERR-G09-SIGNATURE-INVALID`** (collision resolved) | R4 Finding 3 |
| B-3 | `IDEMPOTENCY_CONFLICT` redefinition (D3) | bare 409 `IDEMPOTENCY_CONFLICT` duplicating platform `CONFLICT`/`ERR-DUP-INSTANCE` | **`ERR-G09-IDEMPOTENCY-CONFLICT`** documented as a specialisation of `CONFLICT 409` / `ERR-DUP-INSTANCE` | R4 Finding 6 |
...
| B-5 | Invented `503`/`UPSTREAM_UNAVAILABLE` leaks (D4) | live FR tables returned `503` (PDF render, G13 down) and `UPSTREAM_UNAVAILABLE` (G10/G01 down) | mapped to **`INTERNAL 500` / `ERR-LOADFAIL`** with X.3 outbox retry; `503`/`UPSTREAM_UNAVAILABLE` retained only in the v2→v3 override row A-10 | REMEDIATION §D4; R5 Finding 1 |
| B-6 | Parallel module hash-chain mirror (D4) | ownership table implied a G09-owned "module mirror hash-chained in E19" as the audit record | statutory immutability/tamper-evidence is the **P05 dual-log (DB-trigger) + OPEN-PLAT-03 hash-chaining**, not a G09-owned chain; E19 `case_timeline_events` is a per-case SLA/stage ledger riding on the P05 substrate (requirement kept, mechanism owner changed) | REMEDIATION §D4; R5 Finding 5 |
| B-7 | SR write data-op wording (F-12) | FR data-ops wrote `INSERT service_register_events` directly | reworded to **append via the G12 ingestion contract `POST /api/v1/sr/ingest` (FR-02)** in FR-003/011/012/013/028; event types/envelope unchanged | REMEDIATION §D1/D4 (G09 row) |
...
| Penalty | G06 | G10 | G11 | G12 | Competence (Art. 311(1)) |
|---------|-----|-----|-----|-----|--------------------------|
...
|------|----------|-------------------|
| R1 Authority competence | Critical | FR-018 + DI-13 + `authority_competence` + `ERR-G09-AUTHORITY-NOT-COMPETENT` |
| R2 Mandatory consultation | Critical | FR-019 + DI-14 + `case_consultations` + `ERR-G09-CONSULTATION-PENDING` |
| R3 Dispense-with-inquiry | High | FR-020 + DI-23 + `inquiry_dispensations` |
| R4 Document/witness inspection | High | FR-007 + DI-24 + `ERR-G09-INSPECTION-NOT-AFFORDED` |
...
- The penalty taxonomy, authority hierarchy, and competence matrix are reference data resolved from org configuration (G01/org_units), `authority_competence`, and the conduct-rules master.
- DSC/eSign is provided by a platform trust service; G09 stores returned signature metadata and verifies on demand.
