...
async function call(api, actorCtx, request) {
  return await api.dispatch({ ...request, headers: { "X-Correlation-Id": "corr-g13-documents-self-service", ...(request.headers ?? {}) }, actor: actorCtx });
}

test("G13 documents: the seed produces a real CONFIDENTIAL certificate owned by Rohan", async () => {
  const { api, rohan } = boot();
  const result = await call(api, actor(rohan.id, ["g13.document.read"]), { method: "GET", path: `/api/v1/documents/employees/${rohan.id}` });
...
test("G13 documents: wire responses never leak internal tenantId/entityId fields", async () => {
  const { api, rohan } = boot();
  const documents = (await call(api, actor(rohan.id, ["g13.document.read"]), { method: "GET", path: `/api/v1/documents/employees/${rohan.id}` })).body.items;
...
test("G13 documents: an employee can list their own documents, but not another employee's", async () => {
  const { api, rohan, sunita } = boot();
  const own = await call(api, actor(rohan.id, ["g13.document.read"]), { method: "GET", path: `/api/v1/documents/employees/${rohan.id}` });
  assert.equal(own.status, 200);

  const strangerReads = await call(api, actor(sunita.id, ["g13.document.read"]), { method: "GET", path: `/api/v1/documents/employees/${rohan.id}` });
  assert.equal(strangerReads.status, 403);
});

test("G13 documents: an hr_admin override role may list any employee's documents", async () => {
  const { api, rohan } = boot();
  const hrAdminReads = await call(api, actor("hr-admin-probe", ["g13.document.read"], { roles: ["hr_admin"] }), {
    method: "GET",
...
test("G13 documents: the seeded clearance lets Rohan view and download his own CONFIDENTIAL certificate", async () => {
  const { api, rohan } = boot();
  const documents = (await call(api, actor(rohan.id, ["g13.document.read"]), { method: "GET", path: `/api/v1/documents/employees/${rohan.id}` })).body.items;
  const documentId = documents[0].id;

  const view = await call(api, actor(rohan.id, ["g13.document.read"]), {
    method: "GET",
  ...
  const download = await call(api, actor(rohan.id, ["g13.document.read", "g13.document.download"]), {
    method: "GET",
    path: `/api/v1/documents/${documentId}:fetch`,
...
test("G13 documents: the general document list never surfaces another employee's owned documents to a plain employee (post-full-review CRITICAL fix)", async () => {
  const { api, rohan, sunita } = boot();
  const rohanGeneralList = await call(api, actor(rohan.id, ["g13.document.read"]), { method: "GET", path: "/api/v1/documents" });
  ...
  const hrAdminGeneralList = await call(api, actor("hr-admin-general-list-probe", ["g13.document.read"], { roles: ["hr_admin"] }), {
    method: "GET",
...
test("G13 documents: a CONFIDENTIAL clearance grant unlocks only the grantee's own documents, never another employee's at the same classification (post-full-review CRITICAL fix)", async () => {
  const { api, rohan, sunita } = boot();
  const sunitaDocuments = (await call(api, actor(sunita.id, ["g13.document.read"]), {
    method: "GET",
  ...
  const rohanFetchesSunitasDoc = await call(api, actor(rohan.id, ["g13.document.read"]), {
    method: "GET",
  ...
  const rohanGetsSunitasDoc = await call(api, actor(rohan.id, ["g13.document.read"]), {
    method: "GET",
...
test("G13 documents: without a clearance grant, a CONFIDENTIAL document is denied even to a plain employee holding read/download permission", async () => {
  const { api, sunita } = boot();
  const documents = (await call(api, actor(sunita.id, ["g13.document.read"]), { method: "GET", path: `/api/v1/documents/employees/${sunita.id}` })).body.items;
  const documentId = documents[0].id;

  const view = await call(api, actor(sunita.id, ["g13.document.read"]), {
    method: "GET",
    path: `/api/v1/documents/${documentId}:fetch`,
    query: { intent: "VIEW" },
  });
