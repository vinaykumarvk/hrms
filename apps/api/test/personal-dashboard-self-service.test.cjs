...
function call(api, actorCtx, request) {
  return api.dispatch({ ...request, headers: { "X-Correlation-Id": "corr-g14-personal-dashboard", ...(request.headers ?? {}) }, actor: actorCtx });
}

test("G14 personal dashboard: an employee sees their own real leave balance and attendance summary", () => {
  const { api, sunita } = boot();
  const result = call(api, actor(sunita.id, ["g14.analytics.read.self"]), { method: "GET", path: `/api/v1/analytics/employees/${sunita.id}/dashboard` });
...
test("G14 personal dashboard: post-full-review-goal fix — an employee cannot view another employee's dashboard", () => {
  const { api, sunita, meera } = boot();
  const stranger = call(api, actor(meera.id, ["g14.analytics.read.self"]), { method: "GET", path: `/api/v1/analytics/employees/${sunita.id}/dashboard` });
  assert.equal(stranger.status, 403);
});

test("G14 personal dashboard: post-full-review fix (F3) — holding only the self-service permission does not reach the org-wide executive dashboard", () => {
  const { api, sunita } = boot();
  ...
  const executiveDashboard = call(api, selfOnlyActor, { method: "GET", path: "/api/v1/analytics/dashboards/executive-readiness" });
  assert.equal(executiveDashboard.status, 403);

  const myDashboard = call(api, selfOnlyActor, { method: "GET", path: `/api/v1/analytics/employees/${sunita.id}/dashboard` });
  assert.equal(myDashboard.status, 200);
});

test("G14 personal dashboard: post-full-review-goal fix — the leave-balance and attendance reads underneath are ownership-gated even when called directly", () => {
...
test("G14 personal dashboard: an hr_admin override role may view any employee's dashboard", () => {
  const { api, sunita } = boot();
  const hrAdmin = call(api, actor("hr-admin-dashboard-probe", ["g14.analytics.read.self"], { roles: ["hr_admin"] }), {
    method: "GET",
