// PH-16A — G01 alias-based dedup/merge (FR-EPM-015), PROVISIONAL bulk import (FR-EPM-017),
...
async function createEmployee(api, body) {
...
async function mergePair(api, survivorId, loserId) {
  const scan = await call(api, { method: "POST", path: "/api/v1/dedup/scan", headers: { "Idempotency-Key": idem() } });
  assert.equal(scan.status, 201);
  const candidates = await call(api, { method: "GET", path: "/api/v1/dedup/candidates", query: { status: "OPEN" } });
...
test("PH-16A dedup: exact statutory-ID match queues a HIGH (>=90) dedup candidate with matched attributes", async () => {
  const api = createFoundationApi(createFoundationServices());
  const original = await createEmployee(api, { firstName: "Rahul", lastName: "Verma", pan: "PQRST1111Z", dob: "1985-03-10" });
  const duplicate = await createEmployee(api, { firstName: "Rahul", lastName: "Varma", pan: "PQRST1111Z", dob: "1985-03-10" });
...
test("PH-16A merge: 4-eyes merge writes employee_id_aliases with merge_snapshot, soft-deletes the loser, emits RECORDS_MERGED", async () => {
  const api = createFoundationApi(createFoundationServices());
  const survivor = await createEmployee(api, { firstName: "Sunita", lastName: "Devi", pan: "LMNOP2222Q" });
  const loser = await createEmployee(api, { firstName: "Sunitha", lastName: "Devi", pan: "LMNOP2222Q" });
  // The loser owns a satellite that must be consolidated under the survivor (G01-owned only).
  ...
  const merged = await mergePair(api, survivor.id, loser.id);
  ...
  const feed = await call(api, { method: "GET", path: "/api/v1/employees/changes" });
  assert.ok(
  ...
  const survivorContacts = await call(api, { method: "GET", path: `/api/v1/employees/${survivor.id}/contacts` });
  assert.ok(survivorContacts.body.items.some((item) => item.contactValue === "+919811100011"));
});

test("PH-16A alias resolution: a merged loser id resolves to the survivor via employee_id_aliases (chained collapse)", async () => {
  const api = createFoundationApi(createFoundationServices());
  const a = await createEmployee(api, { firstName: "Deepak", lastName: "Kumar", pan: "AAAAB1111A" });
  const b = await createEmployee(api, { firstName: "Dipak", lastName: "Kumar", pan: "AAAAB1111A" });
  await mergePair(api, a.id, b.id); // b -> a

  const resolved = await call(api, { method: "GET", path: `/api/v1/employees/${b.id}/resolve` });
  ...
  const read = await call(api, { method: "GET", path: `/api/v1/employees/${b.id}` });
  ...
  const c = await createEmployee(api, { firstName: "Deepak", lastName: "Kumaar", pan: "AAAAB1111A" });
  await mergePair(api, c.id, a.id); // a -> c
  const chained = await call(api, { method: "GET", path: `/api/v1/employees/${b.id}/resolve` });
...
test("PH-16A NEGATIVE merge SoD: maker = checker approval fails closed with SOD_VIOLATION (403)", async () => {
  const api = createFoundationApi(createFoundationServices());
  const survivor = await createEmployee(api, { firstName: "Asha", lastName: "Rani", pan: "CCCCD3333C" });
  const loser = await createEmployee(api, { firstName: "Aasha", lastName: "Rani", pan: "CCCCD3333C" });
  await call(api, { method: "POST", path: "/api/v1/dedup/scan", headers: { "Idempotency-Key": idem() } });
  const candidates = await call(api, { method: "GET", path: "/api/v1/dedup/candidates", query: { status: "OPEN" } });
  const candidate = candidates.body.items.find(
  ...
  const stillThere = await call(api, { method: "GET", path: `/api/v1/employees/${loser.id}/resolve` });
  assert.equal(stillThere.body.resolvedVia, "IDENTITY");
});

test("PH-16A NEGATIVE merge conflict: conflicting ACTIVE statutory states without override fail with MERGE_CONFLICT (409)", async () => {
  const api = createFoundationApi(createFoundationServices());
  // Same phonetic name + dob queue a fuzzy candidate, but the two live PANs conflict.
  const survivor = await createEmployee(api, { firstName: "Mohan", lastName: "Sharma", pan: "EEEEF4444E", dob: "1980-01-01" });
  const loser = await createEmployee(api, { firstName: "Mohun", lastName: "Sharma", pan: "GGGGH5555G", dob: "1980-01-01" });
  await call(api, { method: "POST", path: "/api/v1/dedup/scan", headers: { "Idempotency-Key": idem() } });
  const candidates = await call(api, { method: "GET", path: "/api/v1/dedup/candidates", query: { status: "OPEN" } });
  const candidate = candidates.body.items.find(
    (item) => [item.employeeAId, item.employeeBId].includes(survivor.id) && [item.employeeAId, item.employeeBId].includes(loser.id)
...
test("PH-16A merge undo: within the window the snapshot restores the loser and flips is_reversed", async () => {
  const api = createFoundationApi(createFoundationServices());
  const survivor = await createEmployee(api, { firstName: "Farida", lastName: "Begum", pan: "IIIIJ6666I" });
  const loser = await createEmployee(api, { firstName: "Fareeda", lastName: "Begum", pan: "IIIIJ6666I" });
  const merged = await mergePair(api, survivor.id, loser.id);

  const undo = await call(api, {
    method: "POST",
  ...
  const resolved = await call(api, { method: "GET", path: `/api/v1/employees/${loser.id}/resolve` });
...
test("PH-16A NEGATIVE undo window: undo past the configured window fails closed with UNDO_EXPIRED (409)", async () => {
  ...
  const api = createFoundationApi(createFoundationServices({ g01MergeUndoWindowDays: 0 }));
  const survivor = await createEmployee(api, { firstName: "Gopal", lastName: "Nair", pan: "KKKKL7777K" });
  const loser = await createEmployee(api, { firstName: "Gopaal", lastName: "Nair", pan: "KKKKL7777K" });
  const merged = await mergePair(api, survivor.id, loser.id);
...
test("PH-16A import: MIGRATION profile commits PROVISIONAL rows login-disabled and QUEUED; promote-active re-validates under STRICT", async () => {
  const api = createFoundationApi(createFoundationServices());
  // validation_profile=MIGRATION: row 1 passes STRICT, row 2 misses dob/doj (PROVISIONAL), row 3 is unrecoverable.
  const createBatch = await call(api, {
    method: "POST",
    path: "/api/v1/imports",
    headers: { "Idempotency-Key": idem() },
    body: {
      templateVersion: "G01-IMPORT-V1",
  ...
  const validate = await call(api, { method: "POST", path: `/api/v1/imports/${batchId}:validate`, headers: { "Idempotency-Key": idem() } });
  ...
  const report = await call(api, { method: "GET", path: `/api/v1/imports/${batchId}/report` });
  ...
  const commit = await call(api, { method: "POST", path: `/api/v1/imports/${batchId}:commit`, headers: { "Idempotency-Key": idem() } });
  ...
  const replay = await call(api, { method: "POST", path: `/api/v1/imports/${batchId}:commit`, headers: { "Idempotency-Key": idem() } });
  ...
  const committedRows = (await call(api, { method: "GET", path: `/api/v1/imports/${batchId}/report` })).body.rows;
  ...
  const provisionalEmployee = (await call(api, { method: "GET", path: `/api/v1/employees/${provisionalEmployeeId}` })).body.employee;
  assert.equal(provisionalEmployee.recordState, "PROVISIONAL");
  assert.equal(provisionalEmployee.loginDisabled, true);
  const queue = await call(api, { method: "GET", path: "/api/v1/remediation-queue", query: { state: "QUEUED" } });
  ...
  const blockedPromote = await call(api, {
    method: "POST",
  ...
  const resolvedQueue = await call(api, { method: "GET", path: "/api/v1/remediation-queue", query: { state: "RESOLVED" } });
  assert.ok(resolvedQueue.body.items.some((row) => row.resolvedEmployeeId === provisionalEmployeeId));
});

test("PH-16A import guards: STRICT profile hard-fails gap rows; template version mismatch blocks the upload", async () => {
  const api = createFoundationApi(createFoundationServices());
  const wrongTemplate = await call(api, {
    method: "POST",
    path: "/api/v1/imports",
    headers: { "Idempotency-Key": idem() },
    body: { templateVersion: "G01-IMPORT-V0", validationProfile: "STRICT", rows: [{ firstName: "X", orgUnitId: ph03Ids.orgRevenue }] },
  });
  assert.equal(wrongTemplate.status, 400);

  const strictBatch = await call(api, {
    method: "POST",
    path: "/api/v1/imports",
    headers: { "Idempotency-Key": idem() },
    body: {
...
test("PH-16A lifecycle: maker!=checker separation applies status + separation_reason, disables login, emits SEPARATION", async () => {
  const api = createFoundationApi(createFoundationServices());
  const employee = await createEmployee(api, { firstName: "Prakash", lastName: "Iyer" });
  const initiate = await call(api, {
    method: "POST",
  ...
  const reactivate = await call(api, {
    method: "POST",
...
test("PH-16A NEGATIVE lifecycle transitions: §10.1 guards fail closed with INVALID_STATE (409)", async () => {
  const api = createFoundationApi(createFoundationServices());
  const employee = await createEmployee(api, { firstName: "Nisha", lastName: "Kapoor" });
  // Reactivating an ACTIVE employee is not a §10.1 transition.
  const badReactivate = await call(api, {
    method: "POST",
    path: `/api/v1/employees/${employee.id}:reactivate`,
    headers: { "Idempotency-Key": idem() },
  ...
  const badArchive = await call(api, {
    method: "POST",
  ...
  const doubleSeparation = await call(api, {
    method: "POST",
    path: `/api/v1/employees/${employee.id}:separate`,
    headers: { "Idempotency-Key": idem() },
...
test("PH-16A NEGATIVE archive under hold: an ACTIVE legal hold blocks archival with LEGAL_HOLD_ACTIVE (409)", async () => {
  const services = createFoundationServices();
  const api = createFoundationApi(services);
  const employee = await createEmployee(api, { firstName: "Vikram", lastName: "Singh" });
...
test("PH-16A NEGATIVE blocking obligations: open obligations without override fail separation with BLOCKING_OBLIGATIONS (409)", async () => {
  const services = createFoundationServices();
  const api = createFoundationApi(services);
  const employee = await createEmployee(api, { firstName: "Ramesh", lastName: "Gupta" });
  services.employeeIdentityOps.registerBlockingObligation(actor(), {
    employeeId: employee.id,
    description: "Unreturned government quarters",
  });
  const blocked = await call(api, {
    method: "POST",
    path: `/api/v1/employees/${employee.id}:separate`,
  ...
  const overridden = await call(api, {
    method: "POST",
    path: `/api/v1/employees/${employee.id}:separate`,
    headers: { "Idempotency-Key": idem() },
...
test("PH-16A lifecycle DEATH: a DECEASED separation emits DEATH with the FR-024/G11 succession handoff", async () => {
  const api = createFoundationApi(createFoundationServices());
  const employee = await createEmployee(api, { firstName: "Shanta", lastName: "Bai" });
  await call(api, {
    method: "POST",
    path: `/api/v1/employees/${employee.id}:separate`,
    headers: { "Idempotency-Key": idem() },
  ...
  assert.equal(approve.body.outboxEvent.payload.succession_handoff, "FR-024/G11_FAMILY_PENSION");
});
