...
async function call(api, actorCtx, request) {
  return await api.dispatch({ ...request, headers: { "X-Correlation-Id": "corr-g10-payslip-self-service", ...(request.headers ?? {}) }, actor: actorCtx });
}

test("G10 payslips: the seed produces a real PUBLISHED payslip for Arjun (2026-06) with a full earnings/deductions breakdown", async () => {
  const { api, arjun } = boot();
  const result = await call(api, actor(arjun.id, ["g10.payroll.read"]), { method: "GET", path: `/api/v1/payroll/employees/${arjun.id}/payslips` });
...
test("G10 payslips: the wire response never leaks internal tenantId/entityId/runId/calcTrace fields", async () => {
  const { api, arjun } = boot();
  const result = await call(api, actor(arjun.id, ["g10.payroll.read"]), { method: "GET", path: `/api/v1/payroll/employees/${arjun.id}/payslips` });
  assert.equal(result.status, 200);
  const record = result.body.items[0];
...
test("G10 payslips: an employee can view their own YTD statement over HTTP", async () => {
  const { api, arjun } = boot();
  const result = await call(api, actor(arjun.id, ["g10.payroll.read"]), { method: "GET", path: `/api/v1/payroll/employees/${arjun.id}/ytd` });
  assert.equal(result.status, 200);
...
test("G10 payslips: an employee cannot view another employee's payslips or YTD (P02 own-record scope)", async () => {
  const { api, arjun, rohan } = boot();
  const forbiddenPayslips = await call(api, actor(rohan.id, ["g10.payroll.read"]), {
    method: "GET",
    path: `/api/v1/payroll/employees/${arjun.id}/payslips`,
  });
  assert.equal(forbiddenPayslips.status, 403);

  const forbiddenYtd = await call(api, actor(rohan.id, ["g10.payroll.read"]), {
    method: "GET",
...
test("G10 payslips: the enrolment route persists componentAmountsCents that a subsequent engine run picks up", async () => {
  ...
  const enrolled = await call(api, payrollOfficer, {
    method: "POST",
  ...
  const run = await call(api, payrollOfficer, { method: "POST", path: "/api/v1/payroll/engine-runs", headers: { "Idempotency-Key": "idem-enrol-run" }, body: { period: "2026-04", runMode: "FINAL" } });
  await call(api, payrollOfficer, { method: "POST", path: `/api/v1/payroll/engine-runs/${run.body.run.id}:snapshot`, headers: { "Idempotency-Key": "idem-enrol-snap" }, body: {} });
  const computed = await call(api, payrollOfficer, { method: "POST", path: `/api/v1/payroll/engine-runs/${run.body.run.id}:compute`, headers: { "Idempotency-Key": "idem-enrol-compute" }, body: {} });
  ...
  await call(api, approver, { method: "POST", path: `/api/v1/payroll/engine-runs/${run.body.run.id}:approve`, headers: { "Idempotency-Key": "idem-enrol-approve" }, body: {} });
  await call(api, payrollOfficer, { method: "POST", path: `/api/v1/payroll/engine-runs/${run.body.run.id}:lock`, headers: { "Idempotency-Key": "idem-enrol-lock" }, body: {} });

  const payslips = await call(api, actor(rohan.id, ["g10.payroll.read"]), { method: "GET", path: `/api/v1/payroll/employees/${rohan.id}/payslips` });
...
test("G10 payslips: a payroll officer (override role) can view any employee's payslips and YTD", async () => {
  const { api, arjun } = boot();
  const payrollOfficer = actor("payroll-officer-probe", ["g10.payroll.read"], { roles: ["payroll_officer"] });
  const payslips = await call(api, payrollOfficer, { method: "GET", path: `/api/v1/payroll/employees/${arjun.id}/payslips` });
  assert.equal(payslips.status, 200);
  assert.equal(payslips.body.items.length, 1);

  const ytd = await call(api, payrollOfficer, { method: "GET", path: `/api/v1/payroll/employees/${arjun.id}/ytd` });
