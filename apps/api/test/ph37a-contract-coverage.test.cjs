...
test("PH-37A ratchet floor: coverage does not regress below the recorded baseline", () => {
  const record = coverage();
  // Ratchet floor recorded in docs/reviews/contract-coverage-20260703.md (PH-40A raised it to 411 / 31.1%).
  assert.ok(record.implementedTotal >= 411, `implemented routes ${record.implementedTotal} < floor 411`);
  assert.ok(record.totalPct >= 31.1, `coverage ${record.totalPct}% < floor 31.1%`);
});
