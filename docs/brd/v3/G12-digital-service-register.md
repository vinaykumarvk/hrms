# Digital Employee Service Register (Digital SR) — Government HRMS Module BRD (G12, v3.0 · platform-grounded)

**Module code:** G12 (alias `G12-SR`; SHARED_FOUNDATION id `M12-SR` — re-keyed to the canonical `G01..G14` scheme per `MODULE_RECONCILIATION.md` §B)
**Program:** Government-HRMS programme — a **public-sector configuration & extension of the existing PrimeSoft HRMS platform** (Product Vision v2.6, Platform Spec v1.6, Foundation FS v1.6, RBAC Design v1.7, Document Management FS v1.3); deployed at the CGG Data Centre (Standalone / Group-Company tenancy model).
...
**Author persona:** Platform architect (government-HRMS) honouring the public-sector statutory context on the delivered PrimeSoft platform.
...
> **Re-grounding note (v3).** The **SR ledger (`service_register_events`) is a NET-NEW, government-specific statutory system-of-record owned by G12** (confirmed in `MODULE_RECONCILIATION.md` §A/§C/§D — it is **not** a PrimeSoft platform primitive). It is **built ON the PrimeSoft P05 audit/immutability substrate** (DB-trigger capture, immutable — no UPDATE/DELETE, ≥ 7-yr retention, erasure-by-redaction-marker only) and is **owned by G12 while the canonical writer set — G01, G04, G05, G06, G08, G09, G10 and G11 (see FR-01.B; G03 is NOT a writer) — write to it** through G12's governed ingestion contract (the single write-port `POST /api/v1/sr/ingest`); **G11 Pension (also a writer for the separation event) and G14 Analytics consume it**. Its per-`(tenant_id, employee_id)` hash-chaining + status sub-ledger + mandatory external anchor **realise and extend the platform's tamper-evidence proposal `OPEN-PLAT-03` at the statutory-record level** — it is **neither a standalone "blockchain" nor a generic platform service**, but the gov ledger's concrete realisation of OPEN-PLAT-03. Corrigendum/annotation/appeal maker-checker runs on the **P01 WorkflowEngine**; certified-extract rendering, signing and storage use the **G13 document vault** (the gov extension of PrimeSoft M11 Documents) + DocumentGen; authorization, field-masking and PII ceilings are enforced by **P02**; statutory notifications fire through **X.2**; legacy service-book digitisation runs on **P06 ETL+V**. See **`## Alignment with PrimeSoft Platform`** (FR→service map) and **`## Amendments (v2 → v3: platform re-grounding)`**.
...
G12-SR delivers, across four engineered guarantees:
...
**Out of scope (owned elsewhere):** employee master data and DOB/DOJ golden source (G01); the business workflows that *generate* events — leave (G03/G04), transfer (G05), promotion (G06), appraisal (G08), disciplinary (G09), payroll (G10), pension computation (G11); document byte storage (G13); cross-module dashboards (G14). G12 **records the statutory fact**; it does not run the originating business processes. **The per-cadre service rules consumed by the completeness model (FR-17) are referenced from G06/G10/service-rule masters; G12 evaluates expected-vs-recorded, it does not own the substantive promotion/increment rules.**
...
|---|---|---|---|---|
| 1 | Completeness assurance & gap register; retract "100% complete" | R1 (Critical) | **FR-17 (new)**; **E21 `sr_expected_event_rule`**, **E22 `sr_gap_register`**; §1.4 metric "gap-closure rate"; §12 | Expected-event model per cadre/service rules; reconciliation job `JOB-G12-GAPSCAN`; `GAP_FLAGGED` findings + corroboration workflow; metric corrected |
| 2 | Mandatory external anchoring | R2 (Critical), R12 | **FR-04 (amended)**; **E20 `sr_anchors`**; §4 (trust layers), §9 NFR-Integrity, §13.2 | Periodic Merkle root over chain heads, RFC 3161-timestamped to independent WORM; head-vs-anchor mismatch = non-suppressible FAIL; `JOB-G12-ANCHOR` |
...
| 6 | Long-term validation (LTV) & evidence-record renewal | R5 (High) | **FR-19 (new)**; **E25 `sr_ltv_renewals`**; §16.1, §9 NFR-Retention | PAdES-LTV envelopes; RFC 4998 evidence-record renewal job `JOB-G12-LTV`; crypto-migration re-anchor procedure |
...
| 18 | Simplify subscriptions for launch | R17 (Low) | **FR-13 (amended)**; §5.5 enum note | Ship single authenticated pull-feed for G11/G06; defer WEBHOOK + MESSAGE_BUS + DLQ behind a documented real-time requirement |
...
| 20 | Plain-language operator/citizen layer | (adoption) | §7 (amended), §11 (amended), §15 | Plain-language copy + grievance route for every `ERR-G12-*` / `MSG-G12-*`; operator playbook translating crypto/legal jargon; load-tested nightly-sweep SLA before NFR commitment |
...
| Concern | Owner | G12 relationship |
|---|---|---|
| Employee master, DOB/DOJ golden source, employment status | G01-EPM | References `employees`; records *change events*; golden value lives in G01 |
| Substantive promotion/increment/service rules consumed by completeness model | G06 / G10 / service-rule master | G12 references the rules to compute **expected** events; it does not own them |
| Leave application/approval & leave→SR posting | G03 (approval) / **G04 (SR writer)** | **G04 is the leave→SR writer**; G03 feeds G04 (exposes `leave_spell_lineage_id`) and does **NOT** post to SR (FR-01.B) |
| Transfer / relieving / joining orders | G05-TRJ | Receives posting/transfer events (incl. cancellations) |
| Promotion / seniority computation | G06-PPP | Receives promotion events; emits seniority-relevant facts via pull-feed |
| Appraisal / APAR | G08-PAM | Receives appraisal-recorded events |
| Disciplinary charges, penalties, suspension | G09-DCP | Receives punishment/suspension events; **and reversal events when a penalty is quashed** |
| Payroll, pay-fixation computation | G10-PAY | Receives pay-fixation/increment events |
| Pension & qualifying-service computation; separation/superannuation life event | G11-PEN | **G11 IS an SR writer** for the SEPARATION/SUPERANNUATION/RETIREMENT/family-pension event (FR-01.B) — it does **not** write *computation* (qualifying-service totals) back, but it **does** post the separation life-event fact; it also **consumes** SR (pull-feed/extract) and runs the pre-retirement verification gate |
| Document byte storage / object store | G13-DMS | References `documents` for scans, orders, signed extracts |
| Cross-module analytics dashboards | G14-DAS | Exposes SR metrics & pull-feed |
| Auth / RBAC platform | Shared | Inherits OIDC/SSO + RBAC + row-level scoping |
| Licensed CA / HSM / RFC 3161 TSA / WORM substrate | Platform / external procurement | G12 **consumes**; availability is a gated pre-FR-03 dependency (§13.1) |
...
| Governed ingestion contract (idempotent, semantic-dedup, reversal) | FR-02 | **G01/G04/G05/G06/G08/G09/G10/G11** (canonical writer set — FR-01.B; G03 is NOT a writer, G11 IS) |
...
| Certified true copy + qualified signing + redaction | FR-10 | SR Custodian, G13 |
| §65B / BSA certificate-of-authenticity | FR-18 | Courts, Auditor |
| LTV / evidence-record renewal | FR-19 | Auditor, G11 |
| Offline / QR verification of extracts | FR-11 | External verifiers, banks, courts |
| Custody, access control & access log | FR-12 | Auditor |
| Event subscriptions (single pull-feed) | FR-13 | G11, G06, G14 |
| Bulk legacy digitisation (confidence-flagged) | FR-14 | G13, HR data-entry, SR Custodian |
| Bulk corrigendum (cadre-wide) | FR-20 | SR Custodian, Appointing Authority |
| Completeness assurance & gap register | FR-17 | G06/G10 rules, Employee, Custodian |
| Retention, archival, legal hold, cross-tenant continuity, DR anchor | FR-15 | Auditor, G11 |
| Audit / forensics view & SR analytics | FR-16 | Auditor, G14 |
...
- G01 is the authoritative source for `employee_id` ↔ `service_no`; G12 denormalises `service_no` but treats G01 as golden.
- G13 provides durable, encrypted, **WORM/object-lock-capable** storage for scans, signed PDFs, order copies, and the anchor export.
...
- The completeness model (FR-17) can read per-cadre service rules (increment cadence, confirmation window) from G06/G10 / a service-rule master.

---

## Section 3 — Roles & Permissions

### 3.1 Roles relevant to G12 (extends Shared Foundation §4; no contradictions)
...
- **Source Module (service principal)** — machine identity for the canonical writer set **G01/G04/G05/G06/G08/G09/G10/G11** (FR-01.B) posting events (and reversals) via the ingestion contract; `sr.ingest.write` scope only. **G03 is not a writer** (it feeds G04).
- **Pension Officer** — read SR (full) + consume pull-feed/extract for terminal-benefit computation (G11); no write.
...
- **Canonical error envelope (platform — Foundation FS §1; `MODULE_RECONCILIATION.md` §C override):** `{ "error": { "code": "...", "message": "...", "field": "...", "details": {} } }`; the correlation id is carried in the **`X-Correlation-Id` response header**, not a body `requestId`; user-facing copy resolves via plain-language `ERR-G12-*` ids (§11/§15).
- **Inherited error codes (Foundation FS §1 — platform 8-code table):** VALIDATION_FAILED(422), UNAUTHENTICATED(401), FORBIDDEN(403), NOT_FOUND(404), CONFLICT(409, incl. idempotency replays & state conflicts), PRECONDITION_FAILED(412), RATE_LIMITED(429), INTERNAL(500). Source-module/upstream unavailability is handled by the X.3 integration framework and surfaced as INTERNAL(500)/`ERR-LOADFAIL` — the non-standard 503 is dropped per `MODULE_RECONCILIATION.md` §C. G12-specific codes are cataloged in Section 8.
...
- **Observability:** structured logs keyed by `X-Correlation-Id`; metrics (ingestion rate, integrity status, **anchor freshness & head-match rate**, **gap-closure rate**, attestation backlog, verification completion, **LTV renewal backlog**, extract issuance, appeal backlog) exposed to G14.
...
**Workflow engine (P01).** Manual maker-checker SR writes start a platform workflow. SR workflow codes (v2): `WF-G12-CORRIGENDUM`, `WF-G12-BULK-CORRIGENDUM`, `WF-G12-ANNOTATION`, `WF-G12-IDENTITY-CHANGE` (DOB/name — dual sign-off), `WF-G12-LEGACY-PROMOTE`, `WF-G12-MANUAL-EVENT`, `WF-G12-EXTRACT-DUAL` (FULL_SR extract dual-custodian), `WF-G12-APPEAL` (grievance escalation), `WF-G12-GAP-RESOLVE` (completeness corroboration).
...
**Background jobs (X.1).** SR schedulers (v2): `JOB-G12-INTEGRITY` (rolling chain + status-chain verification, FR-04), `JOB-G12-ANCHOR` (periodic Merkle-root anchor + RFC 3161 timestamp + WORM write, FR-04), `JOB-G12-VERIFY-GEN` (cycle generation, FR-08), `JOB-G12-GAPSCAN` (expected-vs-recorded reconciliation, FR-17), `JOB-G12-LTV` (PAdES-LTV / RFC 4998 evidence-record renewal, FR-19), `JOB-G12-FEED` (pull-feed materialisation, FR-13), `JOB-G12-RETENTION` (archival/legal-hold eligibility, FR-15). Each is idempotent (per-period run key), retries with exponential backoff, emits `JOB-FAIL → MSG-SYS-JOBFAIL` on terminal failure, runs per-tenant, and writes one audit row per run.
...
**Notifications (X.2).** SR notifications use `MSG-G12-*`; IN_APP + EMAIL fire in parallel for approvals; **statutory notifications are mandatory and not user-suppressible**; retries with backoff up to 5 attempts + DLQ; **all user-facing copy is plain-language** (§11, council Improvement 20).
...
> This section satisfies the platform authoring rule (`PLATFORM_FOUNDATION.md` §9.6) that every government module BRD map each FR to the platform service(s) it runs on and name the `GAP (gov-specific)` engine it authors. **G12 authors exactly one net-new statutory engine — the SR ledger (`service_register_events`) and its sub-ledgers — and runs everything else on the existing platform.** The SR ledger is the flagship net-new entity in `MODULE_RECONCILIATION.md` §D.

### A1. What G12 authors net-new vs. consumes

| Concern | Owner | G12 posture |
|---|---|---|
| **SR ledger + status/anchor/gap/extract/appeal sub-ledgers (E8, E19–E26 …)** | **G12 (NET-NEW gov engine)** | Authored from scratch as an append-only statutory ledger **on the P05 substrate**; its hash-chain + status-chain + anchor **realise `OPEN-PLAT-03`** at the statutory-record level. Not a platform primitive. |
| Audit / immutability substrate | **P05** | DB-trigger capture, immutable `audit_log` (+ `security_audit_log`), 7-yr+ retention, erasure-by-redaction-marker only. G12 does **not** define its own `audit_log`. |
| Maker-checker / corrigendum / appeal / legacy-promote / identity-change flows | **P01 WorkflowEngine** | `WF-G12-*` are **configured W.1 flow definitions** executed by P01 (`startInstance/advance/approve/reject/sendBack/delegate/cancel`); SoD (maker ≠ checker ≠ second-custodian) enforced by P01/P02, not re-coded. |
...
| Background jobs (integrity, anchor, gap-scan, LTV, feed, verify-gen, retention) | **X.1** | `JOB-G12-*` register `{job_id, schedule, tenant_scope}`; runner gives idempotency, backoff, `JOB-FAIL → MSG-SYS-JOBFAIL`, per-tenant isolation, run audit row. |
| Statutory notifications | **X.2 / W.3** | `MSG-G12-*` by id; IN_APP + EMAIL in parallel; statutory notices **mandatory / non-suppressible**; 5-retry + DLQ. |
| Legacy service-book digitisation | **P06 ETL+V** | 3 staging dry runs, waves, `migration_runs`, permanent `legacy_source_id` (the gov `<gov>_source_id` pattern), legacy source read-only ≥ 4 weeks. |
| Certified-extract rendering, signing, vault storage, retention class | **G13 (on PrimeSoft M11 Documents) + DocumentGen** | Serial numbering, `VAL-M11-SIGNER`/`VAL-M11-RETENTION`, employee-vault, WORM/object-lock anchor export, signed-PDF storage. |
| Validation library | **Foundation FS §2** | Cite `VAL-PAN/AADHAAR/IFSC/DOB/NOMINEE/CONSENT/EFFECTIVE/DATE/FILE/COMMENT/ENUM/MASTER-UNIQUE/FLOW-NOCYCLE`; author only `VAL-G12-*` (e.g. `VAL-G12-SREVENT` ledger-append integrity). |
...
|---|---|---|
| FR-01 Taxonomy (+cadence/correlation) | P02 (authz), W.2 forms (taxonomy editor), P05 (audit) | `sr_event_type` master; `VAL-G12-SREVENT` |
| FR-02 Ingestion contract (idempotent, semantic-dedup, reversal) | **Platform Idempotency-Key + `X-Correlation-Id`**; P02 (`sr.ingest.write` principal); P05 | **SR write-port** — the single frozen contract the canonical writer set **G01/G04/G05/G06/G08/G09/G10/G11** call (FR-01.B; G03 NOT a writer) |
...
| FR-04 Integrity + status + mandatory anchor | X.1 (`JOB-G12-INTEGRITY`, `JOB-G12-ANCHOR`); P05 | Anchor/Merkle verification at statutory-record level |
| FR-05 Corrigendum / supersession (+co-sign/reversal) | **P01** (`WF-G12-CORRIGENDUM`/`WF-G12-IDENTITY-CHANGE`); P02 SoD; P05 | Supersession ledger semantics |
| FR-06 Annotation / dispute / appeal-hook | P01 (`WF-G12-ANNOTATION`); X.2; P05 | Dispute sub-ledger |
| FR-07 Custodian attestation (qualified signature) | P04 (CA/HSM/TSA); P02; P05 | Qualified-signature attestation on ledger |
| FR-08 Verification (+bulk/assisted/heir) | X.1 (`JOB-G12-VERIFY-GEN`); P01; X.2; G11 nominee records | Verification cycle + gap gate |
| FR-09 SR timeline view | P02 (scope/mask); five canonical UI states (Foundation §3) | Timeline read model |
| FR-10 Certified extract (+redaction/co-sign/§65B/LTV) | **P01** (`WF-G12-EXTRACT-DUAL`); **G13** vault + DocumentGen; P02 redaction; P04 CA/TSA | Statutory extract generator |
| FR-11 Offline / QR verification | G13 (offline bundle); published CA chain | Independent verification bundle |
| FR-12 Custody, access control & access log | P02; P05 | `sr_access_log` (fail-closed) |
| FR-13 Event subscriptions (single pull-feed) | X.1 (`JOB-G12-FEED`); P02 | Pull-feed for G11/G06/G14 |
| FR-14 Legacy digitisation (confidence-flagged) | **P06 ETL+V** (`migration_runs`, `legacy_source_id`); P01 (`WF-G12-LEGACY-PROMOTE`); G13 scans | Confidence-flagged digitisation lane |
| FR-15 Retention / hold / continuity / DR anchor | P05 (permanent retention); G13 archival tier; X.1 (`JOB-G12-RETENTION`) | Cross-tenant chain continuity + DR anchor reconciliation |
| FR-16 Forensics / analytics | P05; G14 (analytics surface) | Forensics reconstruction |
| FR-17 Completeness & gap register | X.1 (`JOB-G12-GAPSCAN`); P01 (`WF-G12-GAP-RESOLVE`); G06/G10 service rules | Expected-event model + gap register |
| FR-18 §65B / BSA certificate | **G13** vault + DocumentGen; P04 CA/TSA; P05 | Admissibility certificate generator |
| FR-19 LTV / evidence-record renewal | X.1 (`JOB-G12-LTV`); P04 CA/TSA | PAdES-LTV / RFC 4998 renewal |
| FR-20 Bulk corrigendum | **P01** (`WF-G12-BULK-CORRIGENDUM`); P02 SoD | Sampling-based mass-corrigendum |
| FR-21 Grievance / appeal | **P01** (`WF-G12-APPEAL`); P02; X.2 | Independent appellate escalation |

### A3. Roles mapped to RBAC v1.7 (ADDITIONS, not a parallel scheme)

| G12 actor | Expressed as (RBAC v1.7) | Notes |
|---|---|---|
...
| **Appellate Authority** | New role (read + decision) | P01 approver in `WF-G12-APPEAL`; independent of the upholding custodian. |
...
| **Pension Officer** | New entity-scoped module-admin role (analogous to Payroll Admin) | Read SR + consume pull-feed/extract (G11); no write. |
...
|---|---|---|---|
| 1 | **SR ledger reframed as a NET-NEW gov engine OWNED BY G12, built ON the P05 substrate**; its hash/status/anchor stack stated to **realise platform `OPEN-PLAT-03`** at the statutory-record level (not a standalone "blockchain", not a platform primitive). | `MODULE_RECONCILIATION.md` §A/§C/§D; Platform §P05/§Z | Re-grounding note; §4 trust layering; §4.1 (P05); Alignment §A1/§A2; Final Reconciliation |
| 2 | **Module + ids re-keyed `M12 → G12`** and all collaborating gov modules `M0x/M1x → G0x/G1x` (G04/G05/G06/G08/G09/G10/G11/G13/G14); `WF-/JOB-/MSG-` families re-keyed to `G12`. | `PLATFORM_FOUNDATION.md` §9.7; Reconciliation §B | Document-wide |
| 3 | **Ingestion contract named as the write-port** other gov modules (G04/G05/G06/G08/G09/G10) call; G11/G14 are consumers. | Reconciliation §A/§D | Re-grounding note; FR-02; Alignment §A2 |
| 4 | **Maker-checker / corrigendum / appeal / identity / legacy-promote / gap-resolve flows = configured P01 / W.1 flows** (`WF-G12-*`); SoD enforced by P01/P02, not re-coded. | Platform §P01; Reconciliation §C | §4.1; FR-05/06/10/14/17/20/21 |
| 5 | **Certified-extract rendering/signing/storage moved onto the G13 document vault + DocumentGen** (PrimeSoft M11 Documents extension); `VAL-M11-SIGNER`/`VAL-M11-RETENTION` cited by id, not re-authored. | Reconciliation §A; Foundation §2 | BR-10.4; FR-10/FR-18; Alignment §A1 |
...
| 7 | **`tenant_id` + `entity_id` on the ledger and every G12 entity**; data-layer scoping rejects unscoped queries; chain scoped to `(tenant_id, employee_id)`. | Platform §0.1; Reconciliation §C | E8 table; §4.1; data model |
...
| 9 | **Notifications via X.2** with statutory `MSG-G12-*` **mandatory / non-suppressible**; copy lives once in the catalogue. | Platform §X.2; Foundation §5 | §11 |
...
| 13 | **Final Reconciliation Table extended with explicit platform rows** (P01/P02/P05/P06/X.1/X.2/X.3, G-code adoption, error envelope, cursor pagination, tenant/entity scoping, RBAC additions, G13 vault). | Authoring rule §9.6 | §14.4 |
...
| 2 | **Published canonical `sr_event_type` catalog (FR-01.A)** — every code any writer emits now has a row with `event_category`, `allowed_source_modules`, `fact_correlation_rule`, and `payload_schema` (G01 identity/qualification/personal/deceased; G04 leave; G05 transfer/relieving/joining + cancellations; G06 promotion/officiating/MACP/confirmation/posting; G08 `APAR_FINAL_GRADE`; G09 disciplinary; G10 pay/increment deferred; G11 separation/superannuation/retirement/family-pension). Writers cite the exact strings. | D1 / F-04,F-07,F-08 | FR-01.A; FR-01 AC4/AC7 |
| 3 | **`APPRAISAL` `event_category` added** (18 categories) for G08 `APAR_FINAL_GRADE`. | D1 / F-08 | §5.5 enum; FR-01 AC4 |
| 4 | **Canonical SR writer matrix published (FR-01.B)** — single roster (source_module → categories → allowed) mirroring `MODULE_RECONCILIATION.md` §D; supersedes the four divergent rosters. **G11 IS a writer** for separation/superannuation (consumes-only wording withdrawn); **G03 is NOT a writer** (G04 posts leave); **G02 is NOT an SR source**. Pay-fixation: G06 posts the establishment event, G10 the pay event (no double-claim). | D2 / F-05,F-06 | FR-01.B; Feature Module Map; §2.2 out-of-scope; Re-grounding note |
...
| 6 | **D3 ID hygiene** — G12 module error codes registered under the `ERR-G12-*` namespace mapped onto the platform 8-code table; the `SIGNATURE_INVALID` collision with G13 resolved by namespacing as **`ERR-G12-SIGNATURE-INVALID`** (G13 owns `ERR-G13-SIGNATURE-INVALID`). | D3 | §8.2 |
...
|---|---|---|---|---|
| E1 | `employees` | Canonical (referenced) | G01 | Employee master; subject of SR entries |
...
| E6 | `documents` | Canonical (referenced) | G13 | Scans, order copies, signed extract PDFs, anchor export |
...
| **E8** | **`service_register_events`** | **G12-OWNED (canonical ledger)** | G12 | Append-only, hash-chained statutory SR ledger |
| **E9** | **`sr_event_type`** | **G12-owned** | G12 | Versioned taxonomy + payload schema + cadence + fact-correlation rules |
| **E10** | **`sr_corrections`** | **G12-owned** | G12 | Corrigendum / annotation / dispute / supersession records |
| **E11** | **`sr_attestations`** | **G12-owned** | G12 | Custodian/employee qualified signatures + timestamps |
| **E12** | **`sr_verification_cycles`** | **G12-owned** | G12 | Periodic + bulk/assisted/heir verification cycles |
| **E13** | **`sr_ingestion_requests`** | **G12-owned** | G12 | Append-only provenance/idempotency/semantic-dedup log |
| **E14** | **`sr_certified_extracts`** | **G12-owned** | G12 | Issued certified copies (signed, redacted, QR/offline-verifiable) |
| **E15** | **`sr_access_log`** | **G12-owned** | G12 | Append-only custody/access log |
| **E16** | **`sr_subscriptions`** | **G12-owned** | G12 | Downstream pull-feed subscriptions + cursor |
| **E17** | **`sr_legacy_digitisation_batch`** | **G12-owned** | G12 | Legacy digitisation batch header |
| **E18** | **`sr_legacy_digitisation_record`** | **G12-owned** | G12 | Staged legacy entry (with confidence flag) |
| **E19** | **`sr_status_events`** | **G12-owned (NEW v2)** | G12 | Append-only, hash-chained status/supersession sub-ledger |
| **E20** | **`sr_anchors`** | **G12-owned (NEW v2)** | G12 | External Merkle-root anchors (RFC 3161-timestamped, WORM) |
| **E21** | **`sr_expected_event_rule`** | **G12-owned (NEW v2)** | G12 | Expected-event model per cadre/service rules (completeness) |
| **E22** | **`sr_gap_register`** | **G12-owned (NEW v2)** | G12 | Detected completeness gaps + corroboration lifecycle |
| **E23** | **`sr_appeals`** | **G12-owned (NEW v2)** | G12 | Grievance/appeal escalation beyond custodian uphold |
| **E24** | **`sr_authenticity_certificates`** | **G12-owned (NEW v2)** | G12 | §65B / BSA electronic-record authenticity certificates |
| **E25** | **`sr_ltv_renewals`** | **G12-owned (NEW v2)** | G12 | PAdES-LTV / RFC 4998 evidence-record renewal events |
| **E26** | **`sr_bulk_corrigendum_batch`** | **G12-owned (NEW v2)** | G12 | Cadre-wide/pay-commission bulk corrigendum batch |
...
#### E8 — `service_register_events` (THE statutory ledger — G12-owned, append-only, hash-chained) — **amended v2**
...
| `entity_id` | UUID FK→entities | N | **NEW v3** — entity/directorate scope (Platform §0.1); every G12 entity carries `tenant_id` + `entity_id` per the platform tenancy contract |
| `employee_id` | UUID FK→employees | N | Subject of the event |
| `service_no` | varchar(32) | N | Denormalised business key (golden in G01) |
...
| `source_module` | varchar(16) | N | Provenance: G01..G14, `G12_MANUAL`, `G12_LEGACY` |
...
| `document_ids` | uuid[] | Y | Supporting documents (G13) |
...
|---|---|---|---|---|---|---|---|---|---|
| sr…0001 | GOV-100245 | 1 | APPOINTMENT | 2008-07-14 | G01 | VERIFIED | ACTIVE | EMPLOYEE_VERIFIED | QUALIFYING |
| sr…0042 | GOV-100245 | 42 | PROMOTION | 2019-06-01 | G06 | VERIFIED | SUPERSEDED | ATTESTED | QUALIFYING |
| sr…0043 | GOV-100245 | 43 | PROMOTION_CORRIGENDUM | 2019-06-01 | G12_MANUAL | VERIFIED | ACTIVE | ATTESTED | QUALIFYING |
| sr…0119 | GOV-088120 | 7 | INCREMENT | 1996-07-01 | G12_LEGACY | RECONSTRUCTED | ACTIVE | UNATTESTED | QUALIFYING |
...
#### E19 — `sr_status_events` (status/supersession sub-ledger — G12-owned, append-only, hash-chained) — **NEW v2**
...
#### E20 — `sr_anchors` (external Merkle-root anchors — G12-owned, append-only) — **NEW v2**
...
| `worm_document_id` | UUID FK→documents | N | Immutable WORM export of the anchor + head list (G13 object-lock) |
...
| `source_rule_ref` | varchar(120) | Y | Pointer to G06/G10/service-rule master authority |
...
| `workflow_instance_id` | UUID FK→workflow_instances | Y | `WF-G12-APPEAL` |
...
| `document_id` | UUID FK→documents | N | Signed certificate PDF in G13 |
...
| `workflow_instance_id` | UUID FK→workflow_instances | Y | `WF-G12-BULK-CORRIGENDUM` |
...
```
employees (G01) 1───∞ service_register_events (E8)            [subject of every entry]
employees (G01) 1───∞ sr_verification_cycles (E12)
...
sr_certified_extracts ∞───1 employees ; 1───1 documents (G13); 1───1 sr_attestations
...
service_register_events ∞───∞ documents (G13)
...
|---|---|---|---|
| `service_register_events` (E8) | **G12** | **G12 ingestion only** (source modules + manual + legacy); status projections via materialiser | G01..G14, Employee (self), Auditor |
| `sr_status_events` (E19) | G12 | G12 status-materialiser only (FR-03/05/06/07/08) | Auditor, Custodian, FR-04 verifier |
| `sr_anchors` (E20) | G12 | `JOB-G12-ANCHOR` only | Auditor, Custodian, FR-04, extract offline-verify |
| `sr_expected_event_rule` (E21) | G12 | Sys Admin (draft), Custodian (publish) | `JOB-G12-GAPSCAN` |
| `sr_gap_register` (E22) | G12 | `JOB-G12-GAPSCAN`, Custodian, Employee (corroborate) | Custodian, Employee, Auditor, G11 |
| `sr_appeals` (E23) | G12 | Employee/Heir (raise), Appellate Authority (decide) | Custodian, Auditor, Appellate Authority |
| `sr_authenticity_certificates` (E24) | G12 | Custodian (issue) | Courts, Auditor, Employee |
| `sr_ltv_renewals` (E25) | G12 | `JOB-G12-LTV`, Sys Admin (migration) | Auditor, Custodian |
| `sr_bulk_corrigendum_batch` (E26) | G12 | Custodian (maker), 2nd Custodian (sampling approve) | Custodian, Auditor |
| *(E9–E18 unchanged from v1 §5.4)* | G12 | per v1 | per v1 |

### 5.5 Enum & reference catalog (v2 — additions/changes in **bold**)

| Enum | Values |
|---|---|
| `event_category` | APPOINTMENT, CONFIRMATION, PROMOTION, TRANSFER, POSTING, PAY, INCREMENT, LEAVE, TRAINING, AWARD, PUNISHMENT, SUSPENSION, DEPUTATION, IDENTITY, QUALIFICATION, **APPRAISAL** *(v3.1 — G08 `APAR_FINAL_GRADE`)*, SEPARATION, OTHER **(18 categories; `APPRAISAL` added v3.1 per D1/F-08)** |
...
| `source_module` | G01..G14, G12_MANUAL, G12_LEGACY |
...
5. **Mandatory external anchor (NEW).** `JOB-G12-ANCHOR` periodically computes a Merkle root over all `(employee → content head, status head)` leaves, obtains an RFC 3161 timestamp, and writes it to independent WORM (`sr_anchors`). **A current anchor MUST exist within the configured cadence; FR-04 compares live chain heads to the latest anchor and a mismatch is a non-suppressible FAIL.**
...
13. **Completeness reconciliation (NEW).** `JOB-G12-GAPSCAN` evaluates `sr_expected_event_rule` against recorded events and raises `sr_gap_register` findings; a CRITICAL (pension-affecting) gap blocks pre-retirement cycle completion (FR-08) until EXPLAINED or CLOSED.
...
16. **Referential integrity** — `employee_id` in G01; `document_ids` in G13; FK constraints enforced.
...
|---|---|---|---|---|---|
| `APPOINTMENT` | APPOINTMENT | `[G01]` | Y | `EMP\|CAT:APPOINTMENT\|EFF:event_date` | post, cadre, pay_level, order_no |
| `NAME_CHANGE` | IDENTITY | `[G01]` | N | `EMP\|CAT:IDENTITY\|ATTR:NAME\|EFF:event_date` | old_name, new_name, order_no |
| `DOB_CHANGE` | IDENTITY | `[G01]` | N | `EMP\|CAT:IDENTITY\|ATTR:DOB\|EFF:event_date` | old_dob, new_dob, order_no (dual sign-off) |
| `GENDER_CHANGE` | IDENTITY | `[G01]` | N | `EMP\|CAT:IDENTITY\|ATTR:GENDER\|EFF:event_date` | old_gender, new_gender, order_no |
| `CATEGORY_CHANGE` | IDENTITY | `[G01]` | N | `EMP\|CAT:IDENTITY\|ATTR:CATEGORY\|EFF:event_date` | old_category, new_category, order_no |
| `DECEASED` | IDENTITY | `[G01]` | N | `EMP\|CAT:IDENTITY\|ATTR:DECEASED\|EFF:event_date` | date_of_death, source_ref (master flag only) |
| `QUALIFICATION_ADDED` | QUALIFICATION | `[G01]` | N | `EMP\|CAT:QUALIFICATION\|QUAL:qualification_code\|EFF:event_date` | qualification_code, institution |
| `DEPARTMENTAL_EXAM_PASSED` | QUALIFICATION | `[G01]` | N | `EMP\|CAT:QUALIFICATION\|EXAM:exam_code\|EFF:event_date` | exam_code, result, order_no |
| `EL_AVAILED` | LEAVE | `[G04]` | Y | `EMP\|CAT:LEAVE\|LINEAGE:leave_spell_lineage_id` | leave_spell_lineage_id, from, to, days |
| `HPL_AVAILED` | LEAVE | `[G04]` | Y | `EMP\|CAT:LEAVE\|LINEAGE:leave_spell_lineage_id` | leave_spell_lineage_id, from, to, days |
| `COMMUTED_LEAVE` | LEAVE | `[G04]` | Y | `EMP\|CAT:LEAVE\|LINEAGE:leave_spell_lineage_id` | leave_spell_lineage_id, from, to, days |
| `STUDY_LEAVE` | LEAVE | `[G04]` | Y | `EMP\|CAT:LEAVE\|LINEAGE:leave_spell_lineage_id` | leave_spell_lineage_id, from, to, days |
| `MATERNITY_LEAVE` | LEAVE | `[G04]` | Y | `EMP\|CAT:LEAVE\|LINEAGE:leave_spell_lineage_id` | leave_spell_lineage_id, from, to, days |
| `LWP_SPELL` | LEAVE | `[G04]` | Y | `EMP\|CAT:LEAVE\|LINEAGE:leave_spell_lineage_id` | leave_spell_lineage_id, from, to, days *(non-qualifying spell; completeness suppressor)* |
| `EOL_SPELL` | LEAVE | `[G04]` | Y | `EMP\|CAT:LEAVE\|LINEAGE:leave_spell_lineage_id` | leave_spell_lineage_id, from, to, days |
| `TRANSFER` | TRANSFER | `[G05]` | Y | `EMP\|CAT:TRANSFER\|ORDER:order_no\|EFF:event_date` | from_unit, to_unit, order_no |
| `RELIEVING` | TRANSFER | `[G05]` | Y | `EMP\|CAT:TRANSFER\|ATTR:RELIEVING\|ORDER:order_no` | relieved_unit, relieved_on, order_no |
| `JOINING` | TRANSFER | `[G05]` | Y | `EMP\|CAT:TRANSFER\|ATTR:JOINING\|ORDER:order_no` | joined_unit, joined_on, order_no |
| `MUTUAL_TRANSFER` | TRANSFER | `[G05]` | Y | `EMP\|CAT:TRANSFER\|ATTR:MUTUAL\|ORDER:order_no` | counterpart_emp, from_unit, to_unit |
| `TRANSFER_CANCELLED` | TRANSFER | `[G05]` | Y | *(reversal — see `reverses_source_reference_id`)* | reverses_source_reference_id, order_no |
| `RELIEVING_CANCELLED` | TRANSFER | `[G05]` | Y | *(reversal)* | reverses_source_reference_id, order_no |
| `JOINING_CANCELLED` | TRANSFER | `[G05]` | Y | *(reversal)* | reverses_source_reference_id, order_no |
| `PROMOTION` | PROMOTION | `[G06]` | Y | `EMP\|CAT:PROMOTION\|EFF:event_date\|POST:to_post` | from_post, to_post, pay_level, order_no |
| `OFFICIATING` | PROMOTION | `[G06]` | Y | `EMP\|CAT:PROMOTION\|ATTR:OFFICIATING\|EFF:event_date\|POST:to_post` | officiating_post, from, to, order_no |
| `MACP` | PROMOTION | `[G06]` | Y | `EMP\|CAT:PROMOTION\|ATTR:MACP\|EFF:event_date` | macp_level, financial_upgradation, order_no *(establishment event; pay event posted by G10)* |
| `CONFIRMATION` | CONFIRMATION | `[G06]` | Y | `EMP\|CAT:CONFIRMATION\|EFF:event_date` | confirmed_post, probation_from, order_no |
| `POSTING` | POSTING | `[G06]` | Y | `EMP\|CAT:POSTING\|ORDER:order_no\|EFF:event_date` | posted_unit, role, order_no |
| `REVERSION` | PROMOTION | `[G06]` | Y | `EMP\|CAT:PROMOTION\|ATTR:REVERSION\|EFF:event_date` | from_post, to_post, order_no |
| `PROMOTION_CANCELLED` | PROMOTION | `[G06]` | Y | *(reversal)* | reverses_source_reference_id, order_no |
| `OFFICIATING_CANCELLED` | PROMOTION | `[G06]` | Y | *(reversal)* | reverses_source_reference_id, order_no |
| `APAR_FINAL_GRADE` | APPRAISAL | `[G08]` | N | `EMP\|CAT:APPRAISAL\|PERIOD:apar_period` | apar_period, final_grade, reporting_year |
| `MINOR_PENALTY` | PUNISHMENT | `[G09]` | N | `EMP\|CAT:PUNISHMENT\|CASE:case_id` | case_id, penalty, order_no |
| `MAJOR_PENALTY` | PUNISHMENT | `[G09]` | Y | `EMP\|CAT:PUNISHMENT\|CASE:case_id` | case_id, penalty, order_no |
| `SUSPENSION` | SUSPENSION | `[G09]` | Y | `EMP\|CAT:SUSPENSION\|CASE:case_id\|EFF:event_date` | case_id, from, order_no |
| `SUSPENSION_REVOKED` | SUSPENSION | `[G09]` | Y | `EMP\|CAT:SUSPENSION\|ATTR:REVOKED\|CASE:case_id` | case_id, revoked_on, order_no |
| `CENSURE` | PUNISHMENT | `[G09]` | N | `EMP\|CAT:PUNISHMENT\|CASE:case_id` | case_id, order_no |
| `MINOR_PENALTY_REVERSAL` | PUNISHMENT | `[G09]` | N | *(reversal)* | reverses_source_reference_id, tribunal_order |
| `MAJOR_PENALTY_REVERSAL` | PUNISHMENT | `[G09]` | Y | *(reversal)* | reverses_source_reference_id, tribunal_order |
| `PAY_FIXATION` | PAY | `[G10]` | Y | `EMP\|CAT:PAY\|EFF:event_date\|CAUSE:cause_ref` | cause_ref, pay_level, fixed_pay, order_no *(deferred build)* |
| `ANNUAL_INCREMENT` | INCREMENT | `[G10]` | Y | `EMP\|CAT:INCREMENT\|EFF:event_date` | increment_date, new_pay *(deferred build)* |
| `INCREMENT_WITHHELD` | INCREMENT | `[G10]` | Y | `EMP\|CAT:INCREMENT\|ATTR:WITHHELD\|EFF:event_date` | reason, order_no *(deferred build; completeness suppressor)* |
| `PAY_PROTECTION` | PAY | `[G10]` | Y | `EMP\|CAT:PAY\|ATTR:PROTECTION\|EFF:event_date` | protected_pay, cause_ref *(deferred build)* |
| `SEPARATION` | SEPARATION | `[G11]` | Y | `EMP\|CAT:SEPARATION\|EFF:event_date` | mode, last_working_day, order_no |
| `SUPERANNUATION` | SEPARATION | `[G11]` | Y | `EMP\|CAT:SEPARATION\|ATTR:SUPERANNUATION\|EFF:event_date` | retirement_date, order_no |
| `RETIREMENT` | SEPARATION | `[G11]` | Y | `EMP\|CAT:SEPARATION\|ATTR:RETIREMENT\|EFF:event_date` | retirement_date, order_no |
| `VOLUNTARY_RETIREMENT` | SEPARATION | `[G11]` | Y | `EMP\|CAT:SEPARATION\|ATTR:VRS\|EFF:event_date` | vrs_date, order_no |
| `RESIGNATION` | SEPARATION | `[G11]` | Y | `EMP\|CAT:SEPARATION\|ATTR:RESIGNATION\|EFF:event_date` | resigned_on, order_no |
| `DEATH_IN_SERVICE` | SEPARATION | `[G11]` | Y | `EMP\|CAT:SEPARATION\|ATTR:DEATH\|EFF:event_date` | date_of_death, order_no *(separation/benefit consequence; G01 posts the `DECEASED` master flag)* |
| `FAMILY_PENSION_SANCTIONED` | SEPARATION | `[G11]` | Y | `EMP\|CAT:SEPARATION\|ATTR:FAMILY_PENSION\|EFF:event_date` | beneficiary, pension_order_no |

> **Manual / legacy lanes:** `source_module=G12_MANUAL` (custodian maker-checker ingestion, FR-02 BR-02.2) and `source_module=G12_LEGACY` (digitisation, FR-14) may post any published code subject to its `allowed_source_modules` being widened to include them at publish time; corrigendum/annotation/reversal partner codes (`*_CORRIGENDUM`, `*_CANCELLED`, `*_REVERSAL`) are emitted by G12 itself (FR-05) or by the originating writer per the reversal envelope.

#### FR-01.B — Canonical SR writer matrix (v3.1, authoritative — mirrors `MODULE_RECONCILIATION.md` §D)

> This **single roster** supersedes every divergent writer list previously stated across G12 and the reconciliation doc (resolves F-05 and F-06). It is the only authoritative answer to "who calls the write-port".

| `source_module` | Posts (event categories) | Allowed? | Notes |
|---|---|---|---|
| **G01** | IDENTITY / QUALIFICATION / APPOINTMENT / personal-data life events (name/DOB/category change, `DECEASED`) | **Writer** | G01 owns the master, so G01 posts identity/qualification life events on commit. **G02 is NOT an SR source** — it is the approval workflow whose committed change *causes* G01 to post. |
| **G03** | — | **NOT a writer** | G03 feeds G04 (exposes `leave_spell_lineage_id` on the approved-leave event); G03 does **not** post to SR. |
| **G04** | LEAVE spells affecting service / qualifying service | **Writer** | The leave→SR writer (`EL_AVAILED`/`HPL_AVAILED`/`COMMUTED_LEAVE`/`STUDY_LEAVE`/`MATERNITY_LEAVE`/`LWP_SPELL`/`EOL_SPELL`). |
| **G05** | TRANSFER / RELIEVING / JOINING / POSTING-transfer family (+ `*_CANCELLED`) | **Writer** | `TRANSFER`/`RELIEVING`/`JOINING`/`MUTUAL_TRANSFER` + cancellations. |
| **G06** | PROMOTION / POSTING / OFFICIATING / MACP / CONFIRMATION | **Writer** | Posts the **establishment** event for a promotion/MACP/posting. **Pay-fixation: G06 posts the establishment event (`PROMOTION`/`MACP`); G10 posts the pay event (`PAY_FIXATION`) — no double-claim.** |
| **G08** | APPRAISAL final grade (`APAR_FINAL_GRADE`) | **Writer** | `event_category = APPRAISAL` (v3.1). |
| **G09** | Disciplinary penalty / suspension / exoneration (+ `*_REVERSAL`) | **Writer** | **Reference implementation — already conformant; do not regress.** |
| **G10** | PAY / INCREMENT events | **Writer (deferred build)** | SR-posting FR authored now (endpoint, codes, `fact_key`, dedup tuple, `source_module=G10`) but enabled when G10 ships. |
| **G11** | SEPARATION / SUPERANNUATION / RETIREMENT / family-pension life events | **Writer** | **G11 IS a writer** for the separation/superannuation event (resolves F-05); the prior "consumes only" wording is withdrawn. G11 also **consumes** SR (pull-feed/extract) for pension computation. |
| **G12** (this) | manual records, corrigenda (single/bulk/reversal), legacy digitisation, annotations, anchors, §65B certificates | **Owner** | `G12_MANUAL` / `G12_LEGACY` provenance. |
| **G13 / G14** | — | **NOT writers** | G13 stores bytes; G14 consumes the pull-feed. |
...
4. Invalid payload/provenance/missing-mandate rejects with a specific G12 code and records REJECTED; no entry created.
...
- BR-02.2 Manual custodian ingestion uses `source_module=G12_MANUAL` + maker-checker.
...
**Data Model References.** `sr_ingestion_requests` (write, incl. `fact_key`/`semantic_dedup_result`), `service_register_events` (append via FR-03), `sr_event_type`, `employees` (G01), `documents` (G13), `sr_corrections` (reversal spawn).
...
| Dependencies | FR-01, FR-03, FR-05 (reversal), G01, G13 |
...
- BR-05.2 DOB/name corrigenda require dual custodian sign-off + govt order/court reference (`WF-G12-IDENTITY-CHANGE`); PII ceiling applies.
- BR-05.3 Correcting a qualifying-service impact re-emits to the pull-feed (FR-13) so G11 re-reads.
- **BR-05.4 (v2)** Separation-of-duties: maker ≠ checker ≠ second custodian for routine corrigenda (`WF-G12-CORRIGENDUM`).
- **BR-05.5 (v2)** Cadre-wide/pay-commission corrections route to the **bulk-corrigendum** workflow (FR-20), not one-by-one.

**Data Model References.** `sr_corrections`, `service_register_events` (append corrigendum), `sr_status_events` (SUPERSESSION), `workflow_instances`, `documents` (G13).
...
| Aspect | Detail |
|---|---|
| Components | `CorrectionService`, `AppendEngine`, `StatusLedger`, `WorkflowAdapter` → P01 (`WF-G12-CORRIGENDUM`/`WF-G12-IDENTITY-CHANGE`), `SoDGuard` (NEW) |
...
7. Pre-retirement cycle incomplete at retirement → blocks G11 pension finalisation with an explicit flag.
...
- **BR-08.5 (v2)** Heir eligibility requires nominee/heir identity verification against G11/G01 nominee records.
...
| Failure handling | Overdue escalation; incomplete pre-retirement blocks G11; missing nominee → documented note |
| Dependencies | Scheduler, FR-06, FR-07, FR-17 gap gate, G11 gate, G11/G01 nominee records |
...
**Data Model References.** `sr_certified_extracts` (incl. redaction/anchor/LTV/certificate fields), `service_register_events` (read), `sr_attestations`, `sr_anchors` (ref), `sr_authenticity_certificates` (FR-18), `documents` (G13), `sr_access_log`.
...
| Backend flow | Gather entries → apply redaction (P02) → render → digest → co-sign (FULL_SR) → qualified sign + TSA + PAdES-LTV → embed anchor ref → store PDF (G13) → record extract + token → generate §65B → log |
| Data operations | SELECT entries; INSERT extract + attestation + §65B; G13 store |
...
| State changes | New extract row; PDF in G13; §65B cert; access_log; possible prior-extract revoke |
| Failure handling | Sign/store failure → no persistence; revoke idempotent |
| Dependencies | G13, PKI, TSA, FR-11, FR-18, FR-19 |
...
- **User Story:** *As the Pension module (G11), I want to pull authoritative SR change events for relevant categories from a single authenticated feed, so I consume facts reliably without G12 maintaining three delivery stacks for three internal consumers.*

**Description.** **v2 simplifies v1 (R17):** at launch, downstream modules consume a **single authenticated pull-feed** (`GET /api/v1/sr/feed?since_seq=`) with a per-subscriber durable cursor (`last_delivered_seq`); the feed is materialised by `JOB-G12-FEED`. **WEBHOOK and MESSAGE_BUS modes (and the DLQ apparatus) are deferred** behind a documented real-time requirement (e.g., a committed G14 real-time-dashboard SLA); the enum and entity retain the modes but only PULL_FEED is enabled. Corrigenda/reversals re-emit so consumers re-read corrected facts; subscribers dedupe by `sr_event_id`.
...
| Aspect | Detail |
|---|---|
| Components | `FeedMaterialiser` (`JOB-G12-FEED`), `SubscriptionRepository`, `FeedController` |
...
3. Records must match an G01 `employee_id` (or AMBIGUOUS/UNMATCHED) before promotion; unmatched cannot promote.
...
- **BR-14.4 (v2)** A LEGACY_UNVERIFIABLE fact never silently counts as a confirmed qualifying-service fact; pension (G11) sees the confidence flag.

**Data Model References.** `sr_legacy_digitisation_batch`, `sr_legacy_digitisation_record` (incl. confidence/corroboration fields), `service_register_events` (append via FR-03), `documents` (G13), `workflow_instances`.
...
| Aspect | Detail |
|---|---|
| Components | `DigitisationService` (P06 ETL+V), `MasterMatcher`, `ReconciliationService`, `ConfidenceClassifier` (NEW), `SemanticDedupService`, `AppendEngine`, `WorkflowAdapter` → P01 (`WF-G12-LEGACY-PROMOTE`); writes `migration_runs` |
...
| Dependencies | G13 scans, G01 match, FR-03, FR-08/FR-17, workflow |
...
**Data Model References.** `service_register_events` (continuity fields; archival metadata external), `sr_anchors` (DR reconciliation), `audit_log`, `employees` (G01 status trigger).
...
| Dependencies | Storage tiering; G01 status; `sr_anchors` |
...
5. Analytics aggregates exposed to G14 and exportable, respecting RBAC.
...
**UI Behavior Notes.** Forensics dashboard: content/status/anchor integrity banners, lineage graph, coverage + gap gauges, as-of date picker, access-anomaly list. Analytics cards feed G14.
...
| Dependencies | FR-04/05/07/08/12/17; G14 |
...
- BR-17.3 The expected-event model **references** G06/G10/service-rule authority; it does not redefine substantive service rules.
...
**Data Model References.** `sr_expected_event_rule` (read), `sr_gap_register` (write), `service_register_events` (read/resolve), `workflow_instances` (`WF-G12-GAP-RESOLVE`), `notifications`.
...
| Aspect | Detail |
|---|---|
| Components | `ExpectedEventProjector`, `GapScanJob` (`JOB-G12-GAPSCAN`), `GapResolutionService`, `GapGate` (FR-08), `NotificationAdapter` |
...
| Dependencies | FR-01 cadence rules, G06/G10 service rules, FR-08 gate, FR-02/05/14 recording |
...
**Data Model References.** `sr_authenticity_certificates` (write), `sr_certified_extracts` (read), `sr_anchors` (ref), `sr_attestations`/`sr_ingestion_requests` (chain of custody), `documents` (G13), `sr_access_log`.
...
| State changes | New cert row; PDF in G13; access_log |
| Failure handling | Provisional/digest-mismatch → blocked; sign failure → no persistence |
| Dependencies | FR-10, FR-11 (anchor), FR-19, G13, PKI/TSA |
...
**Data Model References.** `sr_ltv_renewals` (write), `sr_certified_extracts`/`sr_attestations`/`sr_anchors` (subjects), `documents` (G13).
...
| Aspect | Detail |
|---|---|
| Components | `LtvWrapper`, `EvidenceRecordRenewer` (`JOB-G12-LTV`), `CryptoMigrationService`, `AnchorClient`, `RenewalRepository` |
...
| Aspect | Detail |
|---|---|
| Components | `BulkCorrigendumService`, `TargetSelectorEngine`, `SamplingApprovalService`, `CorrectionService` (FR-05), `WorkflowAdapter` (`WF-G12-BULK-CORRIGENDUM`) |
...
**Data Model References.** `sr_appeals` (write), `sr_corrections` (FR-06 source dispute; FR-05 resulting corrigendum), `service_register_events` (status via chain), `workflow_instances` (`WF-G12-APPEAL`), `notifications`.
...
| Aspect | Detail |
|---|---|
| Components | `AppealService`, `WorkflowAdapter` (`WF-G12-APPEAL`), `CorrectionService` (FR-05 on overturn), `StatusLedger`, `NotificationAdapter` |
...
- **Platform error envelope:** `{ "error": { "code", "message", "field", "details" } }`; user-facing copy resolves via plain-language `ERR-G12-*` ids.
...
### 8.2 Error-code catalog (G12-specific; v1 codes retained, v2 additions in **bold**)

> **Namespace (v3.1 / D3).** Every G12 module error code is registered in the platform error registry under the **`ERR-G12-*` namespace** and maps onto exactly one of the **platform 8 standard HTTP codes** (422/401/403/404/409/412/429/500). The `SR_*` machine codes below are the canonical aliases (e.g. `SR_TYPE_NOT_FOUND` ≡ `ERR-G12-TYPE-NOT-FOUND` → 404); plain-language user copy resolves via the matching `ERR-G12-*` id (§11/§15). The signature-validity failure is namespaced **`ERR-G12-SIGNATURE-INVALID`** to resolve the cross-module `SIGNATURE_INVALID` collision with G13 (which owns `ERR-G13-SIGNATURE-INVALID`); no bare/unnamespaced `SIGNATURE_INVALID` is used.
...
| `SR_EMPLOYEE_NOT_FOUND` | 404 | `employee_id` not in G01 |
...
| **`ERR-G12-SIGNATURE-INVALID`** | 422 | Qualified signature on a certified extract / attestation fails validation (FR-10/FR-11/FR-18) — **namespaced to resolve the `SIGNATURE_INVALID` collision with G13** |
...
Idempotency-Key: G06:ord-9912:v1
{
  "source_module": "G06",
...
```
*Response spawns `WF-G12-CORRIGENDUM` and returns the correction id.*
...
|---|---|---|
| G03/G04 (leave) | inbound | Ingestion contract (leave spell events) |
| G05 (transfer) | inbound | Ingestion contract (posting/transfer/relieving/joining + **cancellations**) |
| G06 (promotion) | inbound + **pull-feed** | Ingestion + pull-feed subscription |
| G08 (appraisal) | inbound | Ingestion (appraisal-recorded) |
| G09 (disciplinary) | inbound | Ingestion (punishment/suspension + **reversal on quash**) |
| G10 (payroll) | inbound | Ingestion (pay-fixation/increment); **service-rule source for completeness (FR-17)** |
| G11 (pension) | outbound | **Pull-feed** + certified-extract consumption; pre-retirement verification + **CRITICAL-gap** gate; **nominee records for heir verification** |
| G01 (employee) | reference | Existence/identity checks; status triggers archival; cadre for expected-event rules |
| G13 (documents) | reference | Scans, order copies, signed extract PDFs, **anchor WORM export, §65B certificates** |
| G14 (analytics) | outbound | Analytics aggregates + pull-feed |
...
| **Longevity** | **PAdES-LTV envelopes + RFC 4998 evidence-record renewal (`JOB-G12-LTV`); crypto-migration re-anchors without rewriting history; §65B/BSA certificate on statutory extracts — signed records verifiable decades later** |
...
| From | Event | To | Guard |
|---|---|---|---|
| CREATED | Scans linked | SCANNING | G13 documents attached |
...
| RECONCILIATION | VERIFIED facts discrepancy=0; flagged facts have provenance+note+corroboration plan | READY_FOR_PROMOTION | All matched to G01; semantic dedup clear |
...
| From | Event | To | Guard |
|---|---|---|---|
| (none) | Generate + redact + (FULL_SR co-sign) + qualified-sign + LTV | ISSUED (revoked=false) | Attestation gating; redaction entitlement; signed PDF in G13; §65B available |
...
| **Completeness gap flagged** | `JOB-G12-GAPSCAN` finding | Employee, SR Custodian | In-app, email (CRITICAL) |
| **CRITICAL gap blocks pension** | Pre-retirement finalise blocked | Employee, Custodian, G11 | In-app, email |
...
| Corrigendum / reversal / bulk corrigendum issued | Correction approved/applied | Employee, G11 (pull-feed) | In-app + pull-feed |
...
| **LTV renewal backlog / crypto migration** | `JOB-G12-LTV` / migration | Sys Admin, Auditor | In-app |
| Legacy batch ready / promoted | Status transition | HR maker, Custodian | In-app |
| Pre-retirement verification due | N months before superannuation | Employee, Custodian, G11 | In-app, email |

**Platform alignment (X.2 / Foundation §5).** Each row maps to a plain-language `MSG-G12-*` id (e.g., `MSG-G12-ENTRY-RECORDED`, `MSG-G12-SEMCONFLICT`, `MSG-G12-ATTEST-DUE`, `MSG-G12-PKI-DOWN`, `MSG-G12-VERIFY-OPEN`, `MSG-G12-VERIFY-OVERDUE`, `MSG-G12-GAP-FLAGGED`, `MSG-G12-GAP-PENSION-BLOCK`, `MSG-G12-DISPUTE`, `MSG-G12-APPEAL`, `MSG-G12-CORRIGENDUM`, `MSG-G12-ANCHOR-FAIL`, `MSG-G12-EXTRACT-ISSUED`, `MSG-G12-65B-ISSUED`, `MSG-G12-LTV-BACKLOG`, `MSG-G12-PRERETIRE-VERIFY`); copy lives once in the Message Catalogue. **IN_APP + EMAIL in parallel** for approval-bearing events; **statutory notifications (verification due/overdue, integrity/anchor FAIL, CRITICAL gap, pre-retirement verification, appeal outcome) are mandatory and not user-suppressible**; non-urgent IN_APP supports digest mode and quiet-hours. Retry with exponential backoff up to 5 attempts + DLQ; every dispatch audit-logged. Sensitive content referenced, not embedded. **All user-facing copy is plain-language (council Improvement 20).** Optional Microsoft Teams actionable cards for SR approvals (additive channel, never the system of record).
...
| Event volume analytics | G14 | Events by category/time/org_unit (feeds dashboards) |

Aggregates are exposed to G14 via `GET /api/v1/sr/analytics/summary` and the pull-feed; all respect RBAC and access logging. Exports are paginated/async for large populations.
...
3. **Master alignment:** confirm G01 golden source; build the legacy matcher and **semantic dedup keys**.
...
7. **Source-module cutover (FR-02):** enable ingestion principals for the canonical writer set **G01/G04/G05/G06/G08/G09/G10/G11** (FR-01.B; G03 feeds G04 and is not a writer) with idempotent + semantic-dedup posting and reversal support; dual-write/verification window with G04 leave reconciliation.
8. **Pull-feed (FR-13):** register G11/G06/G14 pull-feed subscriptions before pension/seniority cutover.
9. **Gap scan + pre-retirement verification (FR-17/FR-08):** run `JOB-G12-GAPSCAN` and open cycles for the near-retirement cohort to surface gaps and validate digitised history before pension processing.
...
| **Completeness piloted** | **`JOB-G12-GAPSCAN` run; CRITICAL gaps triaged; pension gate verified** |
...
| **SR ledger = NET-NEW gov engine OWNED BY G12, ON the P05 substrate; realises OPEN-PLAT-03** | Re-grounding note; §4; §4.1 (P05); Alignment §A1/§A2; FR-03/FR-04 | ✅ |
| **Ingestion contract = single frozen write-port `/api/v1/sr/ingest` for the canonical writer set G01/G04/G05/G06/G08/G09/G10/G11 (FR-01.B; G03 NOT a writer, G11 IS); G11/G14 also consume** | FR-01.A/FR-01.B; FR-02; Alignment §A2; §8.4; Appendix C | ✅ |
| **Maker-checker / corrigendum / appeal / legacy-promote = configured P01 / W.1 flows (`WF-G12-*`); SoD via P01/P02** | §4.1; FR-05/06/10/14/17/20/21; Alignment §A1 | ✅ |
...
| **Background jobs on X.1 (`JOB-G12-INTEGRITY/ANCHOR/VERIFY-GEN/GAPSCAN/LTV/FEED/RETENTION`)** | §4.1; FR-04/08/13/15/17/19 | ✅ |
| **Notifications on X.2 / W.3; statutory `MSG-G12-*` mandatory / non-suppressible** | §11; §4.1 | ✅ |
| **Legacy digitisation on P06 ETL+V (3 dry runs, `migration_runs`, `legacy_source_id`)** | FR-14; §13.1; §4.1 | ✅ |
| **Certified extract / §65B on G13 vault + DocumentGen (`VAL-M11-SIGNER`/`VAL-M11-RETENTION` cited)** | FR-10/FR-18; BR-10.4; Alignment §A1 | ✅ |
...
| **G-code re-key `M12 → G12` and all collaborating gov modules `M0x/M1x → G0x/G1x`** | document-wide; Amendments v2→v3 #2 | ✅ |
...
|---|---|
| G01 | APPOINTMENT; IDENTITY (NAME_CHANGE/DOB_CHANGE/GENDER_CHANGE/CATEGORY_CHANGE/DECEASED); QUALIFICATION (QUALIFICATION_ADDED/DEPARTMENTAL_EXAM_PASSED) — golden change records on commit |
| G04 | LEAVE spells (EL_AVAILED/HPL_AVAILED/COMMUTED_LEAVE/STUDY_LEAVE/MATERNITY_LEAVE/LWP_SPELL/EOL_SPELL). **G03 does NOT post to SR** — it feeds G04 (`leave_spell_lineage_id`) |
| G05 | TRANSFER, RELIEVING, JOINING, MUTUAL_TRANSFER, **TRANSFER_CANCELLED/RELIEVING_CANCELLED/JOINING_CANCELLED** |
| G06 | PROMOTION, OFFICIATING, MACP, CONFIRMATION, POSTING, REVERSION, **PROMOTION_CANCELLED/OFFICIATING_CANCELLED**; **establishment** event for pay-fixation (G10 posts the pay event); service-rule source for increment/seniority cadence (FR-17) |
| G08 | **APAR_FINAL_GRADE** (`event_category=APPRAISAL`) |
| G09 | MINOR_PENALTY/MAJOR_PENALTY, SUSPENSION/SUSPENSION_REVOKED, CENSURE, **MINOR/MAJOR_PENALTY_REVERSAL (on quash)** |
| G10 | PAY_FIXATION, ANNUAL_INCREMENT, INCREMENT_WITHHELD, PAY_PROTECTION **(deferred build)**; increment-cadence source (FR-17) |
| G11 | **WRITER** for SEPARATION, SUPERANNUATION, RETIREMENT, VOLUNTARY_RETIREMENT, RESIGNATION, DEATH_IN_SERVICE, FAMILY_PENSION_SANCTIONED; **also consumes** SR (pull-feed/extract + pre-retirement + CRITICAL-gap gate; provides nominee records for heir verification) |
| G12 (this) | manual records, corrigenda (single/bulk/reversal), legacy digitisation, annotations, anchors, §65B certificates |
...
*End of G12 Digital Service Register BRD v3.0 (platform-grounded). Supersedes v2.0 (which incorporated all 20 council Adopted Improvements and Risk Register items R1–R17); v3 preserves that content in full and re-grounds it onto the PrimeSoft platform (P01–P06 / X.1–X.3 / W.1–W.3) with the SR ledger as the NET-NEW gov engine owned by G12 on the P05 substrate. Final reconciliation: 0 unresolved gaps, including platform rows.*
