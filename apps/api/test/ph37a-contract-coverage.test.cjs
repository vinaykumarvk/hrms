...
test("PH-37A ratchet floor: coverage does not regress below the recorded baseline", () => {
  const record = coverage();
  // Ratchet floor recorded in docs/reviews/contract-coverage-20260703.md (PH-50A raised it to 482 / 36.4%).
  assert.ok(record.implementedTotal >= 482, `implemented routes ${record.implementedTotal} < floor 482`);
  assert.ok(record.totalPct >= 36.4, `coverage ${record.totalPct}% < floor 36.4%`);
});
